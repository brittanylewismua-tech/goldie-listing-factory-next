...
export default function ContextHelp({ label, title, intro, sections }: { label: string; title: string; intro: string; sections: HelpSection[] }) {
  ...
  useEffect(() => {
    if (!open) return;
    // Help currently has one interactive element. Keep keyboard focus on it
    // while the modal is open, including Shift+Tab from the first control.
    ...
    return () => { window.removeEventListener("keydown", close); document.body.style.overflow = previousOverflow; triggerRef.current?.focus({ preventScroll: true }); };
  }, [open]);
  const dialog = open ? createPortal(<div className="context-help-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }}>
    <section className="context-help-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      <button ref={closeRef} type="button" className="context-help-close" aria-label="Close instructions" onClick={() => setOpen(false)}>×</button>
  ...
  return <>
    <button ref={triggerRef} type="button" className="context-help-trigger" aria-label={label} aria-haspopup="dialog" onClick={() => setOpen(true)}>?</button>
    {dialog}
  </>;
}

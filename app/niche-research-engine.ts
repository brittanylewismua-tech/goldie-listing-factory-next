...
import {listingPhoto,listingPrice,type EtsyDisplayListing} from '@/app/etsy-listing-display';
...
import {putEvidence} from '@/app/niche-research-store';
import {matchesNiche,qualifies,nicheMetrics,snapshot,type NicheProject,type NicheShop,type NicheListing,type NicheReview} from '@/app/niche-research-model';
type EtsyBody={count?:number;results?:Array<Record<string,unknown>>;shop_name?:string;url?:string};
...
async function checkShop(user:string,p:NicheProject,s:NicheShop){
 ...
 for(const raw of page){const r=raw as EtsyDisplayListing,id=Number(r.listing_id),title=decodeEntities(String(r.title??'')),tags=(r.tags??[]).map(String);if(!Number.isSafeInteger(id)||id<=0)continue;if(!matchesNiche({title,tags},p.phrases))continue;const old=s.listings.find(l=>l.id===id);const l:NicheListing={id,shopId:s.id,title,tags,image:listingPhoto(r),displayAt:Math.floor(Date.now()/1000),price:listingPrice(r),currency:r.price?.currency_code??'',product:productFamily(title)||'other',createdAt:r.original_creation_timestamp??null,seenAt:s.cycleAt,active:true};if(old)Object.assign(old,l);else s.listings.push(l);changed.push(l);}
 ...
 // Retain non-matching IDs too: a subsequent catalog page or refresh may identify them.
 for(const row of page){const transactionId=Number(row.transaction_id),listingId=Number(row.listing_id),at=Number(row.create_timestamp??row.created_timestamp);if(!Number.isSafeInteger(transactionId)||transactionId<=0||!Number.isSafeInteger(listingId)||!Number.isFinite(at))continue;const r:NicheReview={transactionId,listingId,at,rating:typeof row.rating==='number'?row.rating:null,text:decodeEntities(String(row.review??'')).slice(0,700)};const old=s.reviews.find(r=>r.transactionId===transactionId);if(old)Object.assign(old,r);else s.reviews.push(r);changed.push(r);}
...
export async function advanceResearch(user:string,p:NicheProject){
 ...
 finish(p,now);return;}
 const pending=p.shops.find(s=>s.checkedAt<s.cycleAt);if(pending){await checkShop(user,p,pending);return;}
 ...
 const b=await etsy(p,`shops/${candidate.id}`);p.shops.push({id:candidate.id,name:b.shop_name??`Shop ${candidate.id}`,url:b.url??`https://www.etsy.com/shop/${encodeURIComponent(b.shop_name??'')}`,catalogOffset:0,catalogTotal:null,catalogDone:false,reviewOffset:0,reviewsDone:false,listings:[],reviews:[],checkedAt:0,cycleAt:now,reviewSince:now-365*86400});
}

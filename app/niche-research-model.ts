...
export function analyzeNiche(shops:NicheShop[],at:number){
 ...
 const buyerThemes=buyerPatterns.map(pattern=>{const matches=reviews.filter(r=>pattern.re.test(r.text));return {name:pattern.name,count:matches.length,shops:new Set(matches.map(r=>byId.get(r.listingId)!.shopId)).size,examples:matches.sort((a,b)=>b.at-a.at).filter((r,index,all)=>all.findIndex(other=>other.text.trim().toLowerCase().replace(/\s+/g,' ')===r.text.trim().toLowerCase().replace(/\s+/g,' '))===index).slice(0,3)};}).filter(p=>p.count>=2).sort((a,b)=>b.shops-a.shops||b.count-a.count);

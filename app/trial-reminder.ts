...
import { planAmount, type PlanKey, type BillingInterval } from "@/app/plan-limits";
...
function money(plan: PlanKey, interval: BillingInterval, amount?: number, currency = "usd") { return new Intl.NumberFormat("en-US", { style: "currency", currency }).format((amount ?? planAmount(plan, interval)) / 100); }
...
export function trialReminderHtml({ plan, chargeAt, logoUrl, interval = "month", amount, currency }: { plan: PlanKey; chargeAt: number; logoUrl?: string; interval?: BillingInterval; amount?: number; currency?: string }) {
...
export async function scheduleTrialReminder(input: { email: string; plan: PlanKey; trialEnd: number; test?: boolean; interval?: BillingInterval; amount?: number; currency?: string }) {
  ...
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ from: "Listing Factory <hello@mail.thegoldiesuite.com>", to: [input.email], subject: input.test ? "Goldie launch test — no subscription or charge" : "Your Listing Factory trial ends tomorrow", html: input.test ? "<p><strong>Goldie launch verification only. No subscription is being started or charged.</strong></p>" + trialReminderHtml({ plan: input.plan, chargeAt: input.trialEnd, interval: input.interval, amount: input.amount, currency: input.currency }) : trialReminderHtml({ plan: input.plan, chargeAt: input.trialEnd, interval: input.interval, amount: input.amount, currency: input.currency }), scheduled_at: new Date(scheduledAt * 1000).toISOString(), tags: [{ name: "email_type", value: "trial_reminder" }, { name: "plan", value: input.plan }] }), signal: AbortSignal.timeout(15000) });

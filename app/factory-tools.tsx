...
export function SavedWorkflow(props: WorkflowProps) {
  ...
  async function save() {
    ...
    try{
      ...
      const response = await fetch("/api/product-recipes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: editingId || undefined, name, templateUrl: props.templateUrl, description:existing?.description, keywordListId, normalizePadding:true,etsyShippingProfileId:shippingProfileId,defaultColorIds:existing?.defaultColorIds,printifyImageIndices:existing?.printifyImageIndices,etsyDefaults:existing?.etsyDefaults,defaultMockupTheme:existing?.defaultMockupTheme,mockupIds:existing?.mockupIds,setupComplete,defaultProfitTarget:existing?.defaultProfitTarget }) });
      const result = await response.json() as { id?: string; error?: string };
      if (!response.ok){setMessage(result.error || "The product could not be saved.");return}
      const saved:Recipe={id:result.id||editingId,name:name.trim(),templateUrl:props.templateUrl,description:existing?.description||"",defaultTitle:"",keywordListId,normalizePadding:true,etsyShippingProfileId:shippingProfileId,defaultColorIds:existing?.defaultColorIds,printifyImageIndices:existing?.printifyImageIndices,etsyDefaults:existing?.etsyDefaults,defaultMockupTheme:existing?.defaultMockupTheme,mockupIds:existing?.mockupIds,setupComplete,defaultProfitTarget:existing?.defaultProfitTarget};
      const selected=await props.onUseRecipe(saved);
      if(!selected){setMessage("The product was saved, but its Printify details could not be loaded. Check the product link and try again.");await reload();return}
      setActiveId(saved.id);setName(""); setEditingId(""); setMessage(editingId ? "Product updated and selected." : "Product saved and selected. You will not need to paste this Printify link again."); setEditing(false); await reload();

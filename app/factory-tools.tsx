...
type WorkflowProps = {
  ...
  onTemplateUrl: (value: string) => void; onUseRecipe: (recipe: Recipe) => Promise<boolean>;onUseBundle:(bundle:ProductBundle,recipeIds:string[])=>Promise<boolean|"canceled">; onStartNewProduct: () => boolean | Promise<boolean>; onChangeProduct: () => boolean | Promise<boolean>; onVerifyTemplate: (url: string) => Promise<{shippingTemplateId:string;shippingProfileNeedsSelection?:boolean}|null>;
};
...
export function SavedWorkflow(props: WorkflowProps) {
  ...
  async function chooseBundle(bundle:ProductBundle){
    ...
    try{
      const outcome=await props.onUseBundle(bundle,bundle.recipeIds);
      if(outcome===true){setEditing(false);setBundleForm(false);setShowLibrary(false);return}
      setActiveId(props.selectedProductId);
      if(outcome==="canceled")return;
      setMessage("Some saved products in this bundle could not be loaded. Edit the bundle or refresh the page, then try again.");
    }catch(error){
      console.error("Bundle selection failed",error);
      setActiveId(props.selectedProductId);setMessage("This bundle could not be loaded. Try again in a moment.");
    }finally{actionLock.current=false;setPendingAction("")}
  }
...
export function KeywordBank({ onAdd=()=>undefined,onSelect,title="Choose a keyword bank",copy="Only phrases from this validated bank will be used.",compact=false,selectionOnly=false,initialId="" }: { onAdd?: (keyword: string) => void;onSelect?:(list:KeywordList|null)=>void;title?:string;copy?:string;compact?:boolean;selectionOnly?:boolean;initialId?:string }) {
  ...
  useEffect(()=>{
    ...
    if(active&&!current){setActive("");onSelectRef.current?.(null);return}
    const initial=lists.find(list=>list.id===initialId);
    if(initial&&!active){setActive(initial.id);onSelectRef.current?.(initial)}
  },[lists,initialId,active]);
  const chosen = lists.find((list) => list.id === active);
  return <section className={`keyword-bank keyword-workspace ${compact?"compact-keywords":""}`}><div className="keyword-workspace-heading"><div><b>{title}</b>{copy&&<span>{copy}</span>}</div>{!compact&&<a href="/keywords" target="_blank" rel="noopener noreferrer">Upload or manage keyword banks ↗</a>}</div><div className="keyword-list-picker"><select aria-label={title} value={active} onChange={(e) => {const id=e.target.value;setActive(id);onSelect?.(lists.find(list=>list.id===id)||null)}}><option value="">Choose a keyword bank</option>{lists.map((list) => <option value={list.id} key={list.id}>{list.name}</option>)}</select></div>{chosen ? selectionOnly?(()=>{

...
export function draftStageLabel(stageId:string,tasks:string[],optionsLabel='sizes'){
  ...
  if(colors&&sizes)return `Artwork, colors & ${optionsLabel}`;
  if(colors)return 'Artwork & colors';
  if(sizes)return `Artwork & ${optionsLabel}`;

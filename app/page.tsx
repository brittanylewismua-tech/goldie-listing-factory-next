...
function PricingReview({variants,pricing,prices,shippingPercent,approved,onPricing,onPrices,onShippingPercent,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;shippingPercent:number;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onShippingPercent:(value:number)=>void;onApprove:()=>void}){
  ...
  return <section className={`variant-pricing ${approved?"approved":""}`}><div className="variant-pricing-head"><div><p className="mini-label">PRICING REVIEW</p><h3>See and approve every enabled variant</h3><p>Goldie pulled the exact Printify cost and US shipping for each enabled size and color. Edit any item price before approval.</p></div>{approved&&<span>✓ Approved</span>}</div><div className="pricing-cost-snapshot"><div><span>Printify product cost</span><b>{moneyRange(productCostMin,productCostMax)}</b><small>Varies by enabled size and color</small></div><div><span>Printify shipping</span><b>{moneyRange(shippingMin,referenceShipping)}</b><small>${referenceShipping.toFixed(2)} highest rate protects every variant</small></div><div><span>Your Etsy fees</span><b>{pricing.etsyFeePercent.toFixed(1)}% + ${(pricing.fixedFee+pricing.listingFee).toFixed(2)}</b><small>${pricing.fixedFee.toFixed(2)} payment + ${pricing.listingFee.toFixed(2)} listing</small></div><div><span>Profit goal</span><b>${pricing.targetProfit.toFixed(2)}</b><small>Minimum target for one item</small></div></div><div className="pricing-controls"><label>Profit target<input type="number" min="0" step="0.01" value={pricing.targetProfit} onChange={event=>changeProfit(Number(event.target.value))}/></label><div className="shipping-split"><span>Choose the shipping price buyers will see on Etsy</span><p>Printify shipping for this product is {moneyRange(shippingMin,referenceShipping)}. Goldie uses the ${referenceShipping.toFixed(2)} highest rate so no variant is underpriced.</p><div className="shipping-presets"><button type="button" className={normalizedPercent===0?"active":""} onClick={()=>changeShipping(0)}><b>Free shipping</b><small>Buyer pays $0.00</small></button><button type="button" className={normalizedPercent===50?"active":""} onClick={()=>changeShipping(50)}><b>Split it 50/50</b><small>Buyer pays ${(referenceShipping/2).toFixed(2)}</small></button><button type="button" className={normalizedPercent===100?"active":""} onClick={()=>changeShipping(100)}><b>Buyer pays all</b><small>Buyer pays ${referenceShipping.toFixed(2)}</small></button></div><label className="custom-shipping"><span>Or enter the exact shipping price buyers should pay</span><span className="money-input">$<input aria-label="Custom buyer shipping price" type="number" min="0" max={referenceShipping.toFixed(2)} step="0.01" value={buyerShipping(referenceShipping).toFixed(2)} onChange={event=>changeBuyerShippingAmount(Number(event.target.value))}/></span></label><small>Goldie puts the unpaid portion of each variant’s actual Printify shipping into that variant’s item price.</small></div><button onClick={()=>recalculate()}>Reset recommendations</button></div><div className="shipping-api-note"><b>Shipping split is calculated, not yet applied at Etsy.</b><span>Printify can only publish its full shipping template. When Etsy access is connected, Goldie can assign the exact partial-shipping profile automatically. Until then, confirm the shipping profile on Etsy after publishing.</span></div><div className="fee-profile-summary" aria-label="Etsy fees included in every calculation"><span>{pricing.etsyFeePercent.toFixed(1)}% Etsy transaction + payment percentage</span><span>${pricing.fixedFee.toFixed(2)} fixed payment fee</span><span>${pricing.listingFee.toFixed(2)} listing / renewal fee</span><a href="/usage" target="_blank" rel="noopener noreferrer">Review or change Etsy fee profile ↗</a></div><div className="variant-table-wrap"><table className="variant-table"><thead><tr><th>Enabled variant</th><th>Printify cost</th><th>Printify shipping</th><th>Buyer pays</th><th>Built into price</th><th>Etsy % fee</th><th>Fixed payment fee</th><th>Listing fee</th><th>Total Etsy fees</th><th>Item price</th><th>Projected profit<br/>(1-item order)</th></tr></thead><tbody>{variants.map(variant=>{const shipping=Number(variant.shipping||0),charged=buyerShipping(shipping),included=Math.max(0,shipping-charged),itemCents=prices[String(variant.id)]??recommendedPrice(variant.cost,{...pricing,shippingCost:shipping,shippingCharged:charged}),item=itemCents/100,percentFee=(item+charged)*pricing.etsyFeePercent/100,fixedFee=pricing.fixedFee,listingFee=pricing.listingFee,fees=percentFee+fixedFee+listingFee,profit=item+charged-variant.cost/100-shipping-fees;return <tr key={variant.id}><td><b>{variant.title}</b></td><td>${(variant.cost/100).toFixed(2)}</td><td>${shipping.toFixed(2)}</td><td>${charged.toFixed(2)}</td><td>${included.toFixed(2)}</td><td>${percentFee.toFixed(2)}</td><td>${fixedFee.toFixed(2)}</td><td>${listingFee.toFixed(2)}</td><td><b>${fees.toFixed(2)}</b></td><td><label aria-label={`Price for ${variant.title}`}>$<input type="number" min={(variant.cost/100).toFixed(2)} step="0.01" value={(itemCents/100).toFixed(2)} onChange={event=>onPrices({...prices,[String(variant.id)]:Math.round(Math.max(variant.cost/100,Number(event.target.value)||0)*100)})}/></label></td><td className={profit+0.005>=pricing.targetProfit?"profit-pass":"profit-low"}>${profit.toFixed(2)}</td></tr>})}</tbody></table></div><p className="pricing-footnote">Every recommendation shows a single-item order and includes the complete Etsy fee profile above. Offsite Ads and order-specific sales tax processing are excluded because they cannot be known for every order.</p><button className="approve-pricing" onClick={onApprove}>{approved?"✓ Pricing approved":"Approve all variant prices"}</button></section>
}

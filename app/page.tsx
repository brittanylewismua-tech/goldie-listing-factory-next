    ...
    <main className="app-shell">
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="finish"&&finishPhase==="mockups"?"mockup-workspace":""}`}>
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          {templateDetails && <><div className="template-proof recipe-proof"><div className="product-thumb"><span>YOUR<br/>ART</span></div><div className="template-info"><b>{templateDetails.blueprintTitle}</b><span>{templateDetails.provider} · {templateDetails.enabledVariants} enabled variants</span><span>✓ Product, placement, variants, and shipping profile imported</span></div><span className="template-badge">Product ready</span></div><button className="workflow-next" onClick={()=>goToStep("designs")}>Next step <span>→</span></button></>}</div>
          ...
          <article className={`step-card designs-step workflow-panel ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="finish"&&finishPhase==="details")?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              ...
              {files.length>0&&!complete&&workflowStep==="designs"&&<button className="workflow-next" disabled={!designsFinished} onClick={()=>goToStep("review")}>{designsFinished?"Next step":`Preparing ${designsPreparing} ${designsPreparing===1?"design":"designs"}…`} {designsFinished&&<span>→</span>}</button>}

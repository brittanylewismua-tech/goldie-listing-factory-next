  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <div className="pricing-controls">
        ...
        <section className="profit-recommendation-panel"><div><h4>Want Goldie to calculate the item prices?</h4><p>Enter the profit you want from each sale. Goldie will calculate a separate price for every size using that variant’s Printify cost and shipping, the buyer’s domestic shipping charge, and your saved Etsy fees.</p></div><label>Profit goal per sale<span className="money-input">$<input aria-label="Profit goal" type="number" min="0" step="0.01" value={pricing.targetProfit} onChange={event=>changeProfit(Number(event.target.value))}/></span></label><button onClick={()=>recalculate()}>Calculate every size for ${pricing.targetProfit.toFixed(2)} profit</button>{recommendationMessage&&<p className="recommendation-result" role="status">{recommendationMessage}</p>}</section>
      </div>
      ...
      {selectedProfile&&<div className="shipping-profile-proof"><div className="selected-profile-name"><span>Selected shipping profile</span><b title={selectedProfile.title}>{selectedProfile.title}</b></div><div><span>Domestic — {selectedProfile.originCountry}</span><b>${selectedProfile.domesticPrimary.toFixed(2)}</b><small>${selectedProfile.domesticAdditional.toFixed(2)} for each additional item</small></div><div><span>International</span><b>{selectedProfile.international.length?`${selectedProfile.international.length} destination ${selectedProfile.international.length===1?"rate":"rates"}`:"Not included"}</b><small>Open the custom-profile section below only if these rates need to change.</small></div></div>}

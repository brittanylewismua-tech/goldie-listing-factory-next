    ...
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <div className="pricing-controls">
        ...
        <div className="shipping-split">
          <div className="shipping-profile-select"><span>Domestic shipping charged by your imported profile</span><p>Confirm the first-item US charge. Goldie uses it only to calculate Etsy fees and profit; it does not change the imported profile.</p></div>
      ...
      <div className="shipping-api-note"><b>Shipping profile imported from your Printify product.</b><span>Goldie copies that saved profile to every draft. Its international and additional-item rates stay inside the profile.</span></div>
...
export default function Home() {
  ...
  useEffect(()=>{if(!snapshotReady.current||restoringBatch||(!files.length&&!drafts.length))return;const timer=window.setTimeout(()=>{const id=batchIdRef.current||crypto.randomUUID();batchIdRef.current=id;window.localStorage.setItem("goldie-active-batch",id);const designs=files.map(({file:ignoredFile,previewUrl:ignoredPreview,...design})=>design);void fetch("/api/batches",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,status:running?"processing":complete?drafts.some(draft=>draft.status==="Failed")?"needs_attention":"complete":"draft",step:workflowStep,setupName:activeRecipe?.name||"",productTitle:templateDetails?.blueprintTitle||"",designCount:files.length,state:{template,templateDetails,description,pricing,variantPrices,shippingPercent,pricingApproved,mockupTheme,activeRecipe,designs,drafts,complete,bulkTitles,printifyImageIndices,sizeGuideName}})})},700);return()=>window.clearTimeout(timer);// eslint-disable-next-line react-hooks/exhaustive-deps
  },[restoringBatch,workflowStep,template,templateDetails,description,pricing,variantPrices,shippingPercent,pricingApproved,mockupTheme,activeRecipe,files.map(file=>`${file.id}:${file.title}:${file.tags.join("|")}:${JSON.stringify(file.etsy||{})}`).join(";"),drafts,complete,running,bulkTitles,printifyImageIndices,sizeGuideName]);
  ...
  async function publishAll(){const ids=drafts.filter(draft=>draft.status==="Created"&&draft.id).map(draft=>draft.id!);if(!ids.length)return;setPublishConfirmOpen(false);setPublishing(true);setPublishMessage("");try{const response=await fetch("/api/printify/drafts/publish",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productIds:ids,printifyImageIndices})}),payload=await response.json() as {published?:string[];finished?:Array<{etsyListingId:number;url:string}>;error?:string};if(!response.ok)throw new Error(payload.error||"The batch could not be published.");setPublishMessage(`${payload.finished?.length||ids.length} listings are live on Etsy with their Etsy-only details and saved listing images applied.`)}catch(error){setPublishMessage(error instanceof Error?error.message:"The batch could not be published.")}finally{setPublishing(false)}}
  ...
  function createDrafts() {const issues=requiredForStep("review");if(issues.length)return void stopWith("This batch isn’t ready to create.",issues);if(!pricingApproved)return void stopWith("Approve pricing before creating drafts.",["Review every enabled variant and click “Approve all variant prices.”"]);setPreflightOpen(true);}

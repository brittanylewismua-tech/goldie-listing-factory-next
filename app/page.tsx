          ...
          <article className={`step-card workflow-panel ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="finish"&&finishPhase==="details")?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              ...
              {files.length > 0 && complete && <div className="listing-editor">
                ...
                <section className="batch-title-builder"><div><p className="mini-label">AI TITLE BUILDER</p><h3>Auto-create every title</h3><p>Each design gets its own title and matching tags. Goldie cannot invent keywords in this mode.</p></div><div className="title-style-toggle"><span>Title style</span><button className={titleJoiner===", "?"active":""} onClick={()=>changeTitleJoiner(", ")}>Title with commas</button><button className={titleJoiner===" "?"active":""} onClick={()=>changeTitleJoiner(" ")}>Title without commas</button></div><KeywordBank selectionOnly initialId={activeRecipe?.keywordListId||""} onSelect={setAutoTitleBank} title="Keyword bank" copy="Goldie selects the best phrases for each individual design."/><button className="ai-title-button" disabled={titleBuilding||!autoTitleBank||!files.length} onClick={()=>void buildBatchTitle()}>{titleBuilding?`Creating ${files.length} titles…`:"Auto-create all titles"}</button>{titleBuildMessage&&<p className="title-build-message" role="status">{titleBuildMessage}</p>}</section>

...
function PricingReview({variants,pricing,prices,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string,international:InternationalShippingRate[])=>Promise<void>;onApprove:()=>void}){
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <div className="pricing-section-heading shipping-section-heading"><div><h4>2. Shipping</h4><p>Review the Etsy shipping profile that will be used for every listing in this batch.</p></div></div>
...
export default function Home() {
  ...
  const belowRecommendedPixels=useMemo(()=>{const scale=isRigidPaperProduct(templateDetails)?Math.min(templateDetails?.placementScale||1,1):templateDetails?.placementScale||0,recommendedWidth=Math.round((templateDetails?.maxPrintWidth||0)*scale),recommendedHeight=Math.round((templateDetails?.maxPrintHeight||0)*scale);if(!recommendedWidth||!recommendedHeight)return [];return files.filter(file=>Boolean(file.width&&file.height&&(file.width<recommendedWidth||file.height<recommendedHeight)))},[files,templateDetails]);
...
<FinalListingReview drafts={drafts} files={files} selections={printifyImageSelections} defaultIndices={printifyImageIndices} batchSizeGuide={sizeGuideName} onEdit={setFinishPhase}/><div className="final-review-actions"><button onClick={()=>setFinishPhase("details")}>Review titles + description</button><button onClick={()=>setFinishPhase("etsy")}>Review Etsy details</button><button onClick={()=>setFinishPhase("mockups")}>Review images + mockups</button></div><div className="publish-live-warning"><b>These listings will be published live on Etsy.</b><span>They will not go to Etsy drafts. Nothing happens until you confirm again in the next window.</span></div><button className="publish-all-button" disabled={publishing||drafts.some(draft=>draft.status==="Failed")} onClick={openPublishConfirmation}>{publishing?"Publishing…":"Publish all live on Etsy"}</button>{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}</>}</div></article>}
      ...
      {pixelWarningOpen&&<div className="publish-confirm-backdrop" role="presentation"><section className="publish-confirm pixel-warning-modal" role="alertdialog" aria-modal="true" aria-labelledby="pixel-warning-title"><span className="publish-confirm-icon">!</span><p className="mini-label">PRINT RESOLUTION CHECK</p><h2 id="pixel-warning-title">{belowRecommendedPixels.length===1?"One uploaded design is":"Some uploaded designs are"} below Printify’s recommended pixel size.</h2><p>These designs may still print, but they may show a lower resolution inside the Printify editor at the largest enabled size. Would you like to proceed anyway?</p><div className="publish-missing"><b>Review recommended:</b><ul>{belowRecommendedPixels.slice(0,5).map(file=><li key={file.id}>{file.name} · {file.width} × {file.height}px</li>)}{belowRecommendedPixels.length>5&&<li>{belowRecommendedPixels.length-5} more designs</li>}</ul></div><div className="publish-confirm-actions"><button autoFocus onClick={()=>setPixelWarningOpen(false)}>Go back and review</button><button className="pixel-proceed" onClick={()=>{setPixelWarningOpen(false);goToStep("review")}}>Proceed anyway</button></div></section></div>}

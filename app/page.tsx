...
function PrintifyImagePicker({ images,indices,onApplyOne,onApplyAll,onSaveRecipe }: { images: string[];indices:number[];onApplyOne:(indices:number[])=>void;onApplyAll:(indices:number[])=>void;onSaveRecipe?:(indices:number[])=>void|Promise<void> }) {
  const [selected,setSelected]=useState<Set<number>>(new Set(indices)),[expanded,setExpanded]=useState<string>(""),[action,setAction]=useState<"clear"|"all"|"future"|"">(""),[feedback,setFeedback]=useState("");
  ...
  function applyAll(){onApplyAll(chosen);setAction("all");setFeedback("✓ These Printify photos are now selected on every listing in this batch.")}
  async function saveFuture(){if(!onSaveRecipe)return;setFeedback("Saving your preference…");try{await onSaveRecipe(chosen);setAction("future");setFeedback("✓ These Printify photos will be preselected for future batches using this product.")}catch(error){setAction("");setFeedback(error instanceof Error?error.message:"These preferences could not be saved.")}}
  return <details className="printify-image-picker"><summary>Choose Printify flatlays ({selected.size} selected)</summary><p>Choose the Printify images for this listing. Click any photo to view it larger. Changes are saved automatically.</p><div>{images.map((src,index)=><label className={selected.has(index)?"selected":""} key={src}><input type="checkbox" checked={selected.has(index)} onChange={()=>toggle(index)}/><button type="button" className="printify-photo-expand" onClick={event=>{event.preventDefault();setExpanded(src)}} aria-label={`View Printify photo ${index+1} larger`}><img src={src} alt={`Printify product mockup ${index+1}`}/></button></label>)}</div><div className="image-pref-actions"><button type="button" className={`clear ${action==="clear"?"confirmed":""}`} onClick={deselect}>{action==="clear"&&<span className="action-check">✓</span>}<b>{action==="clear"?"Selections cleared":"Clear selections"}</b><small>Remove every selected Printify photo from this listing only.</small></button><button type="button" className={action==="all"?"confirmed":""} onClick={applyAll}>{action==="all"&&<span className="action-check">✓</span>}<b>{action==="all"?"Applied to every listing":"Apply to every listing"}</b><small>Use these same Printify photos across the entire batch.</small></button>{onSaveRecipe&&<button type="button" className={action==="future"?"confirmed":""} onClick={()=>void saveFuture()}>{action==="future"&&<span className="action-check">✓</span>}<b>{action==="future"?"Saved for future batches":"Remember for future batches"}</b><small>Preselect these photos whenever you use this saved product again.</small></button>}</div>{feedback&&<p className="image-pref-feedback" role="status">{feedback}</p>}{expanded&&<div className="printify-photo-lightbox" role="dialog" aria-modal="true" aria-label="Expanded Printify photo" onMouseDown={event=>{if(event.target===event.currentTarget)setExpanded("")}}><button type="button" onClick={()=>setExpanded("")} aria-label="Close expanded photo">×</button><img src={expanded} alt="Expanded Printify product mockup"/></div>}</details>;
}
...
function PricingReview({variants,pricing,prices,productName,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprovalChange}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;productName:string;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string,international:InternationalShippingRate[])=>Promise<void>;onApprovalChange:(ready:boolean)=>void}){
  ...
  const wholePrice=(cents:number)=>wholeNumberPricing?Math.round(cents/100)*100:cents;
  ...
  function toggleWholeNumberPricing(checked:boolean){setWholeNumberPricing(checked);if(!checked)return;const rounded=Object.fromEntries(variants.map(variant=>{const current=prices[String(variant.id)]??variant.templatePrice;return[String(variant.id),Math.max(Math.round(current/100)*100,variant.cost)]}));onPrices(normalizePricesByCost(variants,rounded));setRecommendationMessage("✓ Every item price is now a whole number. Matching Printify-cost groups still share one price.")}
...
export default function Home() {
  ...
  useEffect(()=>{if(imageStepError&&(printifyImageIndices.length||Object.values(printifyImageSelections).some(values=>values.length)||Object.values(preparedMockupCounts).some(count=>count>0)))setImageStepError("")},[imageStepError,printifyImageIndices,printifyImageSelections,preparedMockupCounts]);
...
<FinalListingReview drafts={drafts} files={files} selections={printifyImageSelections} defaultIndices={printifyImageIndices} batchSizeGuide={sizeGuideName} onEdit={setFinishPhase}/><div className="publish-live-warning"><b>These listings will be published live on Etsy.</b><span>They will not go to Etsy drafts. Nothing happens until you confirm again in the next window.</span></div><button className="publish-all-button" aria-busy={publishing} disabled={publishing||drafts.some(draft=>draft.status==="Failed")} onClick={openPublishConfirmation}>{publishing?"Publishing…":"Publish all live on Etsy"}</button>{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}</>}</div></article>}
      ...
      {complete && workflowStep==="finish" && finishPhase==="mockups" && <section className="post-draft-workspace">
        ...
        <button className="workflow-next mockup-next" onClick={()=>{const missing=drafts.filter(item=>item.status==="Created"&&item.id).filter(item=>{const selected=printifyImageSelections[item.id!]??printifyImageIndices;return !selected.length&&!(preparedMockupCounts[item.id!]||0)});if(missing.length){const message="Please add at least one image to every listing before going to the next step.";setImageStepError(message);stopWith("Every listing needs a photo.",[message]);return}setImageStepError("");setFinishPhase("final");window.scrollTo({top:0,behavior:"smooth"})}}>Next step <span>→</span></button>
      </section>}

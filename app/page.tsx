...
type DraftResult = { id?: string; clientId: string; name: string; title?: string; tags?: string[]; previewUrl?: string; printifyImages?: string[]; shopId?: number; editorUrl?: string; status: "Created" | "Failed"; error?: string; placement?:{x:number;y:number;scale:number} };
...
function PlacementEditor({draft,design,template,onSaved}:{draft:DraftResult;design:DesignFile;template:TemplateDetails|null;onSaved:(draft:DraftResult)=>void}){const initial=draft.placement||{x:.5,y:.5,scale:template?.placementScale||1},[placement,setPlacement]=useState(initial),[saving,setSaving]=useState(false),[message,setMessage]=useState("");const quality=design.width&&template?.maxPrintWidth?printifyDpi(design.width,template.maxPrintWidth,placement.scale):null;async function save(){if(!draft.id)return;setSaving(true);setMessage("");try{const response=await fetch("/api/printify/drafts/update",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:draft.id,placement})}),payload=await response.json() as {draft?:DraftResult;error?:string};if(!response.ok)throw new Error(payload.error||"Placement could not be saved.");onSaved({...draft,...payload.draft,placement});setMessage("Placement saved in Printify.")}catch(error){setMessage(error instanceof Error?error.message:"Placement could not be saved.")}finally{setSaving(false)}}return <details className="placement-editor"><summary>Resize or reposition this design</summary><div className="placement-grid"><label>Size <input type="range" min="0.1" max="2.5" step="0.01" value={placement.scale} onChange={e=>setPlacement({...placement,scale:Number(e.target.value)})}/></label><label>Left / right <input type="range" min="0" max="1" step="0.01" value={placement.x} onChange={e=>setPlacement({...placement,x:Number(e.target.value)})}/></label><label>Up / down <input type="range" min="0" max="1" step="0.01" value={placement.y} onChange={e=>setPlacement({...placement,y:Number(e.target.value)})}/></label></div><div className={`live-dpi ${quality&&quality.dpi>=300?"pass":"check"}`}><b>{quality?`${quality.dpi} DPI live estimate`:"DPI unavailable"}</b><span>{quality?.dpi&&quality.dpi<300?"Below 300 DPI—reduce the design size or use Adjust in Printify.":"300 DPI target met."}</span></div><button onClick={()=>void save()} disabled={saving}>{saving?"Saving…":"Save placement to Printify"}</button>{message&&<small role="status">{message}</small>}</details>}

function PricingReview({variants,pricing,prices,shippingMode,approved,onPricing,onPrices,onShippingMode,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;shippingMode:"buyer"|"free";approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onShippingMode:(value:"buyer"|"free")=>void;onApprove:()=>void}){
  const recalculate=(nextPricing=pricing,nextMode=shippingMode)=>onPrices(Object.fromEntries(variants.map(variant=>[String(variant.id),recommendedPrice(variant.cost,{...nextPricing,shippingCost:Number(variant.shipping||0),shippingCharged:nextMode==="buyer"?Number(variant.shipping||0):0})])));
  function changeProfit(value:number){const next={...pricing,targetProfit:Math.max(0,value)};onPricing(next);recalculate(next);}
  function changeShipping(value:"buyer"|"free"){onShippingMode(value);recalculate(pricing,value)}
  return <section className={`variant-pricing ${approved?"approved":""}`}><div className="variant-pricing-head"><div><p className="mini-label">PRICING REVIEW</p><h3>See and approve every enabled variant</h3><p>Goldie pulled the exact Printify cost and US shipping for each enabled size and color. Edit any item price before approval.</p></div>{approved&&<span>✓ Approved</span>}</div><div className="pricing-controls"><label>Profit target<input type="number" min="0" step="0.01" value={pricing.targetProfit} onChange={event=>changeProfit(Number(event.target.value))}/></label><label>Shipping shown to buyer<select value={shippingMode} onChange={event=>changeShipping(event.target.value as "buyer"|"free")}><option value="buyer">Buyer pays Printify shipping</option><option value="free">Free shipping—include it in price</option></select></label><button onClick={()=>recalculate()}>Reset recommendations</button></div><div className="variant-table-wrap"><table className="variant-table"><thead><tr><th>Enabled variant</th><th>Printify cost</th><th>Shipping</th><th>Etsy fees*</th><th>Item price</th><th>Projected profit</th></tr></thead><tbody>{variants.map(variant=>{const itemCents=prices[String(variant.id)]??recommendedPrice(variant.cost,{...pricing,shippingCost:Number(variant.shipping||0),shippingCharged:shippingMode==="buyer"?Number(variant.shipping||0):0}),item=itemCents/100,shipping=Number(variant.shipping||0),charged=shippingMode==="buyer"?shipping:0,fees=(item+charged)*pricing.etsyFeePercent/100+pricing.fixedFee+pricing.listingFee,profit=item+charged-variant.cost/100-shipping-fees;return <tr key={variant.id}><td><b>{variant.title}</b></td><td>${(variant.cost/100).toFixed(2)}</td><td>${shipping.toFixed(2)}<small>{shippingMode==="buyer"?" buyer-paid":" included"}</small></td><td>${fees.toFixed(2)}</td><td><label aria-label={`Price for ${variant.title}`}>$<input type="number" min={(variant.cost/100).toFixed(2)} step="0.01" value={(itemCents/100).toFixed(2)} onChange={event=>onPrices({...prices,[String(variant.id)]:Math.round(Math.max(0,Number(event.target.value))*100)})}/></label></td><td className={profit+0.005>=pricing.targetProfit?"profit-pass":"profit-low"}>${profit.toFixed(2)}</td></tr>})}</tbody></table></div><p className="pricing-footnote">* Estimate uses your saved Etsy transaction/payment profile plus the listing fee. Offsite Ads and order-specific sales tax processing are intentionally excluded.</p><button className="approve-pricing" onClick={onApprove}>{approved?"✓ Pricing approved":"Approve all variant prices"}</button></section>
}
...
export default function Home() {
  ...
  const [shippingMode,setShippingMode]=useState<"buyer"|"free">("buyer");
  ...
  useEffect(()=>{if(!snapshotReady.current||restoringBatch||(!template&&!files.length&&!drafts.length))return;const timer=window.setTimeout(()=>{const id=batchIdRef.current||crypto.randomUUID();batchIdRef.current=id;window.localStorage.setItem("goldie-active-batch",id);const designs=files.map(({file:ignoredFile,previewUrl:ignoredPreview,...design})=>design);void fetch("/api/batches",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,status:running?"processing":complete?drafts.some(draft=>draft.status==="Failed")?"needs_attention":"complete":"draft",step:workflowStep,setupName:activeRecipe?.name||"",productTitle:templateDetails?.blueprintTitle||"",designCount:files.length,state:{template,templateDetails,description,pricing,variantPrices,shippingMode,pricingApproved,mockupTheme,activeRecipe,designs,drafts,complete,bulkTitles,printifyImageIndices}})})},700);return()=>window.clearTimeout(timer);// eslint-disable-next-line react-hooks/exhaustive-deps
  },[restoringBatch,workflowStep,template,templateDetails,description,pricing,variantPrices,shippingMode,pricingApproved,mockupTheme,activeRecipe,files.map(file=>`${file.id}:${file.title}:${file.tags.join("|")}:${JSON.stringify(file.etsy||{})}`).join(";"),drafts,complete,running,bulkTitles,printifyImageIndices]);
  ...
  function startNewProduct(){
    if((template||files.length>0||drafts.length>0||complete)&&!window.confirm("Add a new product and clear the current product setup? Any designs and unfinished work in this batch will be removed."))return false;
  ...
  function addBatchKeyword(keyword:string){const current=files.length&&files.every(file=>file.title===files[0].title)?files[0].title:"";const phrases=current.split(/[,|]/).map(value=>value.trim().toLocaleLowerCase());if(phrases.includes(keyword.trim().toLocaleLowerCase()))return;applyBatchTitle(current?`${current}${titleJoiner}${keyword}`:keyword)}
  async function buildBatchTitle(){const current=files.length&&files.every(file=>file.title===files[0].title)?files[0].title:"",keywords=current.split(/[,|]/).map(value=>value.trim()).filter(Boolean);if(!keywords.length){setTitleBuildMessage("Choose keyword phrases first.");return}setTitleBuilding(true);setTitleBuildMessage("");try{const response=await fetch("/api/listing-intelligence",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({mode:"title",image:await safeImagePreviewDataUrl(files[0].file,1200,false),product:{blueprintTitle:templateDetails?.blueprintTitle,brand:templateDetails?.brand,model:templateDetails?.model},keywords,useCommas:titleJoiner===", "})}),payload=await response.json() as {title?:string;error?:string};if(!response.ok||!payload.title)throw new Error(payload.error||"Goldie could not build this title.");applyBatchTitle(payload.title);setTitleBuildMessage("Title applied to every listing. Matching tags were rebuilt too.")}catch(error){setTitleBuildMessage(error instanceof Error?error.message:"Goldie could not build this title.")}finally{setTitleBuilding(false)}}

  async function publishAll(){const ids=drafts.filter(draft=>draft.status==="Created"&&draft.id).map(draft=>draft.id!);if(!ids.length)return;const missing:string[]=[];if(files.some(file=>!file.title.trim()))missing.push("one or more titles");if(files.some(file=>!file.tags.length))missing.push("one or more tag sets");if(!description.trim())missing.push("the permanent product description");if(files.some(file=>!file.etsy))missing.push("one or more Etsy detail reviews");const warning=missing.length?`Still blank or unfinished: ${missing.join(", ")}.\n\n`:"";if(!window.confirm(`${warning}Publish ${ids.length} ${ids.length===1?"listing":"listings"} through Printify now? They may become live on Etsy immediately.`))return;setPublishing(true);setPublishMessage("");try{const response=await fetch("/api/printify/drafts/publish",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productIds:ids})}),payload=await response.json() as {published?:string[];error?:string};if(!response.ok)throw new Error(payload.error||"The batch could not be published.");setPublishMessage(`${payload.published?.length||ids.length} listings were sent to Etsy through Printify.`)}catch(error){setPublishMessage(error instanceof Error?error.message:"The batch could not be published.")}finally{setPublishing(false)}}
      ...
      try {
        ...
        for (let pipelineAttempt = 1; pipelineAttempt <= 3; pipelineAttempt += 1) {
          ...
          try {
            ...
            const baseScale=templateDetails?.placementScale,printWidth=templateDetails?.maxPrintWidth,maxPlacementScale=baseScale&&printWidth&&design.width?Math.max(baseScale,design.width/printWidth):baseScale;
            const response = await fetchWithDeadline("/api/printify/drafts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ batchId: templateDetails?.batchId, title: design.title || undefined, tags: design.tags, pricing, variantPrices, description:fullDescription, visibleBounds:design.visibleBounds, maxPlacementScale, fileName: upload.fileName, stagedId: staged.stagedId, supportReference: staged.reference, clientId: design.id }) }, 4 * 60 * 1000);
    ...
    <main className="app-shell">
      ...
      <section className="workspace">
        ...
        <aside className={`launch-panel workflow-panel ${workflowStep==="review"?"active-panel":"hidden-panel"}`}>
          ...
          {pricedVariants.length>0&&<PricingReview variants={pricedVariants} pricing={pricing} prices={variantPrices} shippingMode={shippingMode} approved={pricingApproved} onPricing={value=>{setPricing(value);setPricingApproved(false)}} onPrices={value=>{setVariantPrices(value);setPricingApproved(false)}} onShippingMode={value=>{setShippingMode(value);setPricingApproved(false)}} onApprove={()=>setPricingApproved(true)}/>}

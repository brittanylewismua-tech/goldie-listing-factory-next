...
export default function Home() {
  ...
  function canOpenStep(step:WorkflowStep){if(localPreview)return true;if(step==="connect")return true;if(step==="setup")return connected&&etsyConnected;if(step==="designs")return connected&&etsyConnected&&templateLoaded;if(step==="review")return etsyConnected&&ready;return etsyConnected&&complete}
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          <article className={`step-card connect-step workflow-panel ${connected ? "done" : ""} ${workflowStep==="connect"?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              ...
              {(localPreview||(connected&&etsyConnected))&&<button className="workflow-next" onClick={()=>goToStep("setup",false,localPreview)}>{localPreview&&!connected?"Preview without connecting":"Choose a product"} <span>→</span></button>}
            </div>
          </article>

...
export default function Home() {
  ...
  },[restoringBatch,workflowStep,finishPhase,template,templateDetails,description,pricing,variantPrices,etsyShippingProfileId,pricingApproved,mockupTheme,activeRecipe,files.map(file=>`${file.id}:${file.title}:${file.tags.join("|")}:${file.blurb||""}:${file.descriptionOverride??""}:${file.sizeGuideName||""}:${JSON.stringify(file.etsy||{})}`).join(";"),drafts,complete,running,bulkTitles,batchKeywords,titleJoiner,titleBuilderMode,autoTitleBankId,manualKeywordBankId,sharedMockups,preparedMockupCounts,printifyImageIndices,printifyImageSelections,sizeGuideName]);
  ...
  async function publishAll(){const ids=drafts.filter(draft=>draft.status==="Created"&&draft.id).map(draft=>draft.id!);if(!ids.length)return;setPublishConfirmOpen(false);setPublishing(true);setPublishMessage("");setBatchReceipt(null);try{const response=await fetch("/api/printify/drafts/publish",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productIds:ids,printifyImageIndices,printifyImageSelections,etsyShippingProfileId})}),payload=await response.json() as {published?:string[];finished?:Array<{etsyListingId:number;url:string}>;error?:string};if(!response.ok)throw new Error(payload.error||"The batch could not be published.");const count=payload.finished?.length||ids.length;setBatchReceipt({publishedCount:count,etsyUrls:(payload.finished||[]).map(item=>item.url).filter(Boolean),completedAt:new Date().toISOString()});setPublishMessage("")}catch(error){setPublishMessage(error instanceof Error?error.message:"The batch could not be published.")}finally{setPublishing(false)}}
  ...
  const workflowHero = {
    connect: { eyebrow: "THE LISTING FACTORY", title: "Turn finished designs into ready-to-publish Etsy listings.", copy: "Connect Printify once. Goldie handles the repetitive setup from here—without rebuilding every product by hand." },
    ...
    finish: finishPhase==="details" ? { eyebrow: "STEP 6 · TITLES + DESCRIPTION", title: "Finish the words once—not listing by listing.", copy: "Build titles from validated keyword banks, generate matching tags, and apply one reusable description with optional listing-specific edits." } : finishPhase==="etsy" ? { eyebrow: "STEP 7 · ETSY DETAILS", title: "Goldie prepared the product details Etsy needs.", copy: "Review the category and product-specific fields. Goldie leaves optional details blank unless they genuinely apply." } : finishPhase==="mockups" ? { eyebrow: "STEP 8 · IMAGES + MOCKUPS", title: "Build the final image set.", copy: "Review the real Printify placement, choose flatlays, add an optional size guide, and prepare your own lifestyle mockups." } : { eyebrow: "STEP 9 · FINAL REVIEW", title: "Everything is ready for one final check.", copy: "Confirm the words, pricing, Etsy details, and images. Publishing will send these listings live—not to Etsy drafts." },
  }[workflowStep];
    ...
    <main className="app-shell">
      <header className="topbar">
        <div className="brand-lockup">
          <div className="powered-wordmark"><span className="powered-label">Powered by</span><img src="/goldie-wordmark-clean.png?v=2" alt="Goldie" className="wordmark" /><span className="gpt-label">GPT</span></div>
        </div>
        ...
        <div className="steps-column">
          ...
          <article className={`step-card workflow-panel ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="finish"&&finishPhase==="details")?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              ...
              {files.length > 0 && complete && <div className={`listing-editor ${titlePulseIds.size?"titles-resolving":""}`}>

...
export default function Home() {
  ...
  async function selectRecipe(recipe:Recipe){setActiveRecipe(recipe);setPrintifyImageIndices(recipe.printifyImageIndices||[]);setEtsyShippingProfileId(Number(recipe.etsyShippingProfileId)||0);setTemplate(recipe.templateUrl);setMockupTheme("");const nextPricing={...pricing,targetProfit:DEFAULT_PRICING.targetProfit,shippingCost:0,shippingCharged:0};setPricing(nextPricing);setTemplateDetails(null);return Boolean(await loadTemplateUrl(recipe.templateUrl,nextPricing,Number(recipe.etsyShippingProfileId)||0,recipe.defaultColorIds||[]))}
  async function useRecipe(recipe: Recipe) { const changingProduct=Boolean((activeRecipe?.id&&activeRecipe.id!==recipe.id)||(template&&template!==recipe.templateUrl));if(changingProduct&&(files.length>0||drafts.length>0||complete)){const count=files.length;if(!window.confirm(`Switch to “${recipe.name}” and start a new batch? This removes ${count} ${count===1?"design":"designs"} and all work from the current batch on this page.`))return false;clearCurrentBatch(false)}setActiveBundle(null);setBundleRecipes([]);setBundleIndex(0);return await selectRecipe(recipe); }
...
<div className="final-safety-readiness" aria-label="Final publishing readiness"><span>{files.every(file=>file.etsy)&&files.every(file=>!personalizationIssue(file))?"✓":"!"} Etsy details and personalization {files.every(file=>file.etsy)&&files.every(file=>!personalizationIssue(file))?"are ready":"still need review"}</span><span>{allCreatedListingsHaveImages()?"✓":"!"} {allCreatedListingsHaveImages()?"Every listing has at least one photo":"One or more listings still need a photo"}</span></div><FinalListingReview drafts={drafts} files={files} selections={printifyImageSelections} defaultIndices={printifyImageIndices} batchSizeGuide={sizeGuideName} onEdit={setFinishPhase}/><div className="publish-live-warning"><b>These listings will be published live on Etsy.</b><span>They will not go to Etsy drafts. Nothing happens until you confirm again in the next window.</span><small>Etsy charges its standard $0.20 USD listing fee for each listing created. This fee is charged by Etsy and is separate from your Goldie subscription.</small></div><button className="publish-all-button" aria-busy={publishing} disabled={publishing||drafts.some(draft=>draft.status==="Failed")} onClick={openPublishConfirmation}>{publishing?"Publishing…":"Publish all live on Etsy"}</button>{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}</>}</div></article>}
      ...
      {complete && workflowStep==="finish" && finishPhase==="mockups" && <section className="post-draft-workspace">
        <div className="post-draft-heading"><div><p className="mini-label">STEP 8 · IMAGES + MOCKUPS</p><h2>Review placement and choose listing images.</h2><p>The large preview below is the real Printify placement Goldie uses as the required reference for lifestyle mockups.</p></div>{drafts.filter(draft=>draft.status==="Created").length>1&&<button className="open-all-button" onClick={openAllDrafts}>Open all in Printify</button>}</div>

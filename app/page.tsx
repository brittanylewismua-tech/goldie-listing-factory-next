...
export default function Home() {
  ...
  function useRecipe(recipe: Recipe) { setActiveRecipe(recipe);setPrintifyImageIndices(recipe.printifyImageIndices||[]);setTemplate(recipe.templateUrl);setMockupTheme(recipe.defaultMockupTheme || ""); setPricing({ ...DEFAULT_PRICING, ...(recipe.pricing || {}) }); setTemplateDetails(null); void loadTemplateUrl(recipe.templateUrl); }
      ...
      try {
        ...
        for (let pipelineAttempt = 1; pipelineAttempt <= 3; pipelineAttempt += 1) {
          ...
          try {
            ...
            const response = await fetchWithDeadline("/api/printify/drafts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ batchId: templateDetails?.batchId, title: design.title || listingTitle.trim() || undefined, tags: design.tags, pricing, description:fullDescription, visibleBounds:design.visibleBounds, fileName: upload.fileName, stagedId: staged.stagedId, supportReference: staged.reference, clientId: design.id }) }, 4 * 60 * 1000);
  ...
  async function prepareOne(design:DesignFile){try{const response=await fetch("/api/listing-intelligence",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({image:await fileDataUrl(design.file),product:{blueprintTitle:templateDetails?.blueprintTitle,brand:templateDetails?.brand,model:templateDetails?.model,description},title:design.title,tags:design.tags})}),payload=await response.json() as {details?:EtsyDetails;error?:string};if(!response.ok||!payload.details)throw new Error(payload.error||"Etsy details could not be prepared.");updateDesign(design.id,{etsy:payload.details,etsyError:""});const draft=drafts.find(item=>item.clientId===design.id);if(draft?.id)await fetch("/api/printify/drafts/update",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId:draft.id,title:design.title,tags:design.tags,description:[payload.details.blurb,description].filter(Boolean).join("\n\n")})});return payload.details}catch(error){updateDesign(design.id,{etsyError:error instanceof Error?error.message:"Etsy details could not be prepared."});return null}}
  async function finishListing(id:string){const design=files.find(file=>file.id===id);if(!design)return;setPreparingEtsy(true);await prepareOne(design);setPreparingEtsy(false)}
  ...
  return (
    <main className="app-shell">
      ...
      {preflightOpen && <div className="preflight-backdrop" role="presentation" onMouseDown={(e)=>{if(e.target===e.currentTarget)setPreflightOpen(false)}}><section className="preflight" role="dialog" aria-modal="true" aria-labelledby="preflight-title"><p className="mini-label">FINAL CHECK</p><h2 id="preflight-title">Ready to create {files.length} Printify {files.length===1?"draft":"drafts"}?</h2><div className="preflight-list"><div><span>Printify product</span><b>✓ {templateDetails?.blueprintTitle}</b></div><div><span>Titles and tags</span><b>{files.filter(f=>!f.tags.length).length?`${files.filter(f=>!f.tags.length).length} need review`:"✓ Complete"}</b></div><div><span>Descriptions</span><b>{files.filter(f=>!f.etsy?.blurb).length?`${files.filter(f=>!f.etsy?.blurb).length} introductions need review`:"✓ Product description + unique introductions"}</b></div><div><span>Etsy details</span><b>{files.filter(f=>!f.etsy).length?`${files.filter(f=>!f.etsy).length} need review`:`✓ ${files.length} completed for ${templateDetails?.blueprintTitle}`}</b></div><div><span>Pricing</span><b>✓ Calculated per enabled variant</b></div></div>{files.some(f=>!f.etsy||!f.tags.length)&&<p className="preflight-warning">Some optional or automatically prepared information needs review. Drafts can still be created now.</p>}<div className="preflight-actions"><button className="preflight-cancel" onClick={()=>setPreflightOpen(false)}>Go back and review</button><button className="preflight-confirm" onClick={confirmDrafts}>Create drafts anyway →</button></div></section></div>}

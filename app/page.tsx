...
type EtsyShippingProfile={id:number;title:string;originCountry:string;currency:string;domesticPrimary:number;domesticAdditional:number;international:Array<{label:string;primary:number;additional:number}>};
...
const PROGRESS_STEPS = ["Connect Printify","Choose product","Add designs","Review pricing","Create drafts","Titles + descriptions","Images + mockups","Final review"];
...
function PricingReview({variants,pricing,prices,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string)=>Promise<void>;onApprove:()=>void}){
  const selectedProfile=profiles.find(profile=>profile.id===selectedProfileId);
  const [customCharge,setCustomCharge]=useState(""),[customAdditional,setCustomAdditional]=useState(""),[customProfileName,setCustomProfileName]=useState(""),[savingProfile,setSavingProfile]=useState(false),[profileMessage,setProfileMessage]=useState("");
  useEffect(()=>{setCustomCharge(selectedProfile?selectedProfile.domesticPrimary.toFixed(2):"");setCustomAdditional(selectedProfile?selectedProfile.domesticAdditional.toFixed(2):"");setCustomProfileName(selectedProfile?selectedProfile.title:"");setProfileMessage("")},[selectedProfileId]);
  const enteredCharge=Number(customCharge),enteredAdditional=Number(customAdditional),buyerShipping=Number.isFinite(enteredCharge)&&enteredCharge>=0?enteredCharge:selectedProfile?.domesticPrimary||0,customDirty=Boolean(selectedProfile&&(Math.abs(buyerShipping-selectedProfile.domesticPrimary)>.004||Math.abs(enteredAdditional-selectedProfile.domesticAdditional)>.004||customProfileName.trim()!==selectedProfile.title));
  const recalculate=(nextPricing=pricing,nextBuyerShipping=buyerShipping)=>onPrices(Object.fromEntries(variants.map(variant=>{const shipping=Number(variant.shipping||0);return[String(variant.id),recommendedPrice(variant.cost,{...nextPricing,shippingCost:shipping,shippingCharged:nextBuyerShipping})]})));
  function changeProfit(value:number){onPricing({...pricing,targetProfit:Math.max(0,value)});}
  function chooseProfile(id:number){onSelectProfile(id);const profile=profiles.find(item=>item.id===id);setCustomCharge(profile?profile.domesticPrimary.toFixed(2):"");setCustomAdditional(profile?profile.domesticAdditional.toFixed(2):"");setCustomProfileName(profile?profile.title:"")}
  async function createProfile(){if(!selectedProfile)return;const charge=Number(customCharge),additional=Number(customAdditional),title=customProfileName.trim();if(!Number.isFinite(charge)||charge<0||!Number.isFinite(additional)||additional<0||!title)return setProfileMessage("Name the profile and enter valid first-item and additional-item charges.");setSavingProfile(true);setProfileMessage("");try{await onCreateProfile(selectedProfile.id,charge,additional,title);setProfileMessage("✓ New Etsy shipping profile saved and selected.")}catch(error){setProfileMessage(error instanceof Error?error.message:"The shipping profile could not be saved.")}finally{setSavingProfile(false)}}
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      <div className="variant-pricing-head">
        <div><p className="mini-label">PRICING</p><h3>Review the prices already on your template.</h3><p>Goldie starts with your template’s exact variant prices. Change them yourself or ask Goldie to recommend prices for your profit goal.</p></div>
        {approved&&<span>✓ Approved</span>}
      </div>
      <div className="pricing-controls">
        <label className="shipping-profile-select"><span>Start with one of this seller’s Etsy shipping profiles</span><select value={selectedProfileId||""} disabled={profilesLoading} onChange={event=>chooseProfile(Number(event.target.value))}><option value="">{profilesLoading?"Loading this seller’s Etsy shipping profiles…":"Choose a starting Etsy profile"}</option>{profiles.map(profile=><option key={profile.id} value={profile.id}>{profile.title}</option>)}</select><small>This supplies the processing and international settings. The domestic amount is editable below.</small></label>
        <label>How much profit do you want from each sale?<span className="money-input">$<input aria-label="Profit goal" type="number" min="0" step="0.01" value={pricing.targetProfit} onChange={event=>changeProfit(Number(event.target.value))}/></span></label>
        <button onClick={()=>recalculate()}>Use Goldie’s recommended prices</button>
      </div>
      ...
      {selectedProfile&&<div className="shipping-profile-proof"><div><span>Starting profile</span><b>{selectedProfile.title}</b></div><div><span>Original domestic charge</span><b>${selectedProfile.domesticPrimary.toFixed(2)}</b><small>${selectedProfile.domesticAdditional.toFixed(2)} with another item</small></div><div><span>International settings retained</span><b>{selectedProfile.international.length?`${selectedProfile.international.length} destination ${selectedProfile.international.length===1?"rate":"rates"}`:"Not included"}</b><small>{selectedProfile.international.slice(0,3).map(rate=>`${rate.label}: $${rate.primary.toFixed(2)}`).join(" · ")||"This profile has no international destination."}</small></div></div>}
      {selectedProfile&&<section className="custom-shipping-builder"><h4>Create a custom shipping profile for this seller</h4><p>Edit any field. This changes only the new profile created for this seller and product—not every seller or every shipping profile.</p><label>Shipping profile name<input value={customProfileName} maxLength={60} onChange={event=>{setCustomProfileName(event.target.value);setProfileMessage("")}}/></label><div className="shipping-amount-presets"><button onClick={()=>setCustomCharge(selectedProfile.domesticPrimary.toFixed(2))}>Full: ${selectedProfile.domesticPrimary.toFixed(2)}</button><button onClick={()=>setCustomCharge((selectedProfile.domesticPrimary/2).toFixed(2))}>Half: ${(selectedProfile.domesticPrimary/2).toFixed(2)}</button><button onClick={()=>setCustomCharge("0.00")}>Free</button></div><label>First item — buyer pays domestically<span className="money-input">$<input inputMode="decimal" value={customCharge} onChange={event=>{setCustomCharge(event.target.value);setProfileMessage("")}}/></span></label><label>Each additional item — buyer pays<span className="money-input">$<input inputMode="decimal" value={customAdditional} onChange={event=>{setCustomAdditional(event.target.value);setProfileMessage("")}}/></span></label>{customDirty?<button disabled={savingProfile||!customCharge||!customAdditional||!customProfileName.trim()} onClick={()=>void createProfile()}>{savingProfile?"Saving this seller’s profile in Etsy…":"Save this custom profile for this product"}</button>:<div className="shipping-saved-state">✓ These values are already saved in the selected Etsy profile.</div>}{profileMessage&&<small role="status">{profileMessage}</small>}</section>}
      ...
      <button className="approve-pricing" disabled={!selectedProfile||customDirty} onClick={onApprove}>{approved?"✓ Pricing + shipping approved":!selectedProfile?"Choose an Etsy shipping profile first":customDirty?"Save the domestic shipping amount first":"Approve pricing + shipping"}</button>
    </section>
  );
}
...
export default function Home() {
  ...
  function openProgressStep(index:number){if(!confirmUploadInterruption())return;if(index===0)return goToStep("connect");if(index===1)return goToStep("setup");if(index===2)return goToStep("designs");if(index===3)return goToStep("review");if(index===4){goToStep("review");return createDrafts()}if(!complete)return stopWith("Finish the earlier steps first.",requiredForStep("finish"));setFinishPhase(index===5?"details":index===6?"mockups":"final");goToStep("finish",false,true)}
  ...
  useEffect(()=>{if(!snapshotReady.current||restoringBatch||(!files.length&&!drafts.length))return;const timer=window.setTimeout(()=>{const id=batchIdRef.current||crypto.randomUUID();batchIdRef.current=id;window.localStorage.setItem("goldie-active-batch",id);const designs=files.map(({file:ignoredFile,previewUrl:ignoredPreview,...design})=>design);void fetch("/api/batches",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,status:running?"processing":complete?drafts.some(draft=>draft.status==="Failed")?"needs_attention":"complete":"draft",step:workflowStep,setupName:activeRecipe?.name||"",productTitle:templateDetails?.blueprintTitle||"",designCount:files.length,state:{template,templateDetails,description,pricing,variantPrices,etsyShippingProfileId,pricingApproved,mockupTheme,activeRecipe,designs,drafts,complete,bulkTitles,printifyImageIndices,sizeGuideName}})})},700);return()=>window.clearTimeout(timer);// eslint-disable-next-line react-hooks/exhaustive-deps
  },[restoringBatch,workflowStep,template,templateDetails,description,pricing,variantPrices,etsyShippingProfileId,pricingApproved,mockupTheme,activeRecipe,files.map(file=>`${file.id}:${file.title}:${file.tags.join("|")}:${JSON.stringify(file.etsy||{})}`).join(";"),drafts,complete,running,bulkTitles,printifyImageIndices,sizeGuideName]);
  ...
  async function createCustomShippingProfile(baseProfileId:number,domesticPrimary:number,domesticAdditional:number,title:string){const response=await fetch("/api/etsy/shipping-profiles",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({baseProfileId,domesticPrimary,domesticAdditional,title})}),result=await response.json() as {id?:number;error?:string};if(!response.ok||!result.id)throw new Error(result.error||"The Etsy shipping profile could not be saved.");await loadEtsyShippingProfiles(result.id);if(activeRecipe){const updated={...activeRecipe,etsyShippingProfileId:result.id};await fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(updated)});setActiveRecipe(updated)}setPricingApproved(false)}
          ...
          <article className={`step-card workflow-panel ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="finish"&&finishPhase==="details")?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              ...
              {files.length > 0 && complete && <div className="listing-editor">
                <div className="editor-heading"><div><b>1. Build titles for the whole batch—or customize exceptions</b><span>Start with one shared title for speed. Every keyword click updates all Printify drafts and regenerates their matching tags. Open an individual listing only when it needs different wording.</span></div><span>{files.length} listings</span></div>
                ...
                <button className="workflow-next" onClick={()=>{const missing=missingPublishFields();if(missing.length)stopWith("Complete the required listing details.",missing.map(field=>`${field} must be completed before continuing.`));else setFinishPhase("mockups")}}>Choose images and mockups <span>→</span></button>
              </div>}
            </div>
          </article>
          {workflowStep==="finish"&&finishPhase==="final"&&<article className="step-card final-review active-panel"><div className="step-number">08</div><div className="step-content"><div className="step-heading"><div><p className="mini-label">FINAL REVIEW</p><h2>Your batch is ready for its final check</h2></div><span className="done-mark">✓ {drafts.filter(draft=>draft.status==="Created").length} drafts</span></div><p className="step-copy">Confirm the checklist below. Nothing is published until you use the final button.</p><div className="final-checklist"><span>✓ Every enabled variant price was reviewed and approved</span><span>✓ {etsyShippingProfiles.find(profile=>profile.id===etsyShippingProfileId)?.title||"Etsy shipping profile"} will be applied automatically</span><span>{files.every(file=>file.title.trim())?"✓":"!"} Printify titles are complete</span><span>{files.every(file=>file.tags.length)?"✓":"!"} Printify tags are complete</span><span>{description.trim()?"✓":"!"} Printify description {description.trim()?"is attached":"is blank"}</span><span>{files.every(file=>file.etsy)?"✓":"!"} Etsy categories and product attributes {files.every(file=>file.etsy)?"are ready":"still need review"}</span><span>✓ Printify placement and listing images were reviewed</span>{sizeGuideName&&<span>✓ {sizeGuideName} will be applied to every Etsy listing</span>}</div><div className="final-review-actions"><button onClick={()=>goToStep("review")}>Review titles + pricing</button><button onClick={()=>setFinishPhase("mockups")}>Review images + mockups</button></div><div className="publish-live-warning"><b>These listings will be published live on Etsy.</b><span>Printify does not send them to Etsy drafts. Goldie checks every required field before the final confirmation.</span></div><button className="publish-all-button" disabled={publishing||drafts.some(draft=>draft.status==="Failed")} onClick={openPublishConfirmation}>{publishing?"Publishing…":"Publish all live on Etsy"}</button>{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}</div></article>}
        </div>

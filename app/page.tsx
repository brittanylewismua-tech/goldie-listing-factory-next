  ...
  return (
    <main className="app-shell">
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="finish"&&finishPhase==="mockups"?"mockup-workspace":""}`}>
        ...
        />}
        {activeBundle&&bundleRecipes.length>1&&<section className="bundle-progress" aria-label={`Product bundle ${activeBundle.name}`}><div><span>PRODUCT BUNDLE</span><b>{activeBundle.name}</b><small>Upload once. Goldie carries your designs and titles into each product.</small></div><ol>{bundleRecipes.map((recipe,index)=><li className={index<bundleIndex?"complete":index===bundleIndex?"current":""} key={recipe.id}><span>{index<bundleIndex?"✓":index+1}</span><b>{recipe.name}</b><small>{index<bundleIndex?"Published":index===bundleIndex?"You are here":"Up next"}</small></li>)}</ol></section>}

...
function PricingReview({variants,pricing,prices,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string,international:InternationalShippingRate[])=>Promise<void>;onApprove:()=>void}){
  ...
  useEffect(()=>{setCustomCharge(selectedProfile?selectedProfile.domesticPrimary.toFixed(2):"");setCustomAdditional(selectedProfile?selectedProfile.domesticAdditional.toFixed(2):"");setCustomInternational(selectedProfile?selectedProfile.international.map(rate=>({...rate,primary:rate.primary.toFixed(2),additional:rate.additional.toFixed(2)})):[]);setCustomProfileName(selectedProfile?selectedProfile.title:"");setProfileMessage("")},[selectedProfileId]);
  ...
  function chooseProfile(id:number){onSelectProfile(id);const profile=profiles.find(item=>item.id===id);setCustomCharge(profile?profile.domesticPrimary.toFixed(2):"");setCustomAdditional(profile?profile.domesticAdditional.toFixed(2):"");setCustomInternational(profile?profile.international.map(rate=>({...rate,primary:rate.primary.toFixed(2),additional:rate.additional.toFixed(2)})):[]);setCustomProfileName(profile?profile.title:"")}
  function changeInternational(index:number,field:"primary"|"additional",value:string){setCustomInternational(current=>current.map((rate,i)=>i===index?{...rate,[field]:Number(value)}:rate));setProfileMessage("")}
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      {selectedProfile&&<section className="custom-shipping-builder"><h4>Create a custom shipping profile for this seller</h4><p>Name the new profile and review every rate. This creates a separate Etsy profile for this seller and product; it never changes the starting profile.</p><label>Shipping profile name<input value={customProfileName} maxLength={60} onChange={event=>{setCustomProfileName(event.target.value);setProfileMessage("")}}/></label><h5>Domestic shipping</h5><div className="shipping-amount-presets"><button onClick={()=>setCustomCharge(selectedProfile.domesticPrimary.toFixed(2))}>Full: ${selectedProfile.domesticPrimary.toFixed(2)}</button><button onClick={()=>setCustomCharge((selectedProfile.domesticPrimary/2).toFixed(2))}>Half: ${(selectedProfile.domesticPrimary/2).toFixed(2)}</button><button onClick={()=>setCustomCharge("0.00")}>Free</button></div><div className="shipping-rate-row"><b>{selectedProfile.originCountry}</b><label>First item<span className="money-input">$<input inputMode="decimal" value={customCharge} onChange={event=>{setCustomCharge(event.target.value);setProfileMessage("")}}/></span></label><label>Additional item<span className="money-input">$<input inputMode="decimal" value={customAdditional} onChange={event=>{setCustomAdditional(event.target.value);setProfileMessage("")}}/></span></label></div><h5>International shipping</h5>{customInternational.length?<div className="international-rate-list">{customInternational.map((rate,index)=><div className="shipping-rate-row" key={rate.key}><b>{rate.label}</b><label>First item<span className="money-input">$<input aria-label={`${rate.label} first item`} inputMode="decimal" value={rate.primary} onChange={event=>changeInternational(index,"primary",event.target.value)}/></span></label><label>Additional item<span className="money-input">$<input aria-label={`${rate.label} additional item`} inputMode="decimal" value={rate.additional} onChange={event=>changeInternational(index,"additional",event.target.value)}/></span></label></div>)}</div>:<p className="no-international-rates">The starting profile has no international destinations.</p>}{customDirty?<button disabled={savingProfile||!customCharge||customAdditional===""||!customProfileName.trim()} onClick={()=>void createProfile()}>{savingProfile?"Saving this seller’s profile in Etsy…":"Save this custom profile for this product"}</button>:<div className="shipping-saved-state">✓ These values are already saved in the selected Etsy profile.</div>}{profileMessage&&<small role="status">{profileMessage}</small>}</section>}

  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <div className="variant-table-wrap"><table className="variant-table">
        ...
        <tbody>{variants.map(variant=>{const shipping=Number(variant.shipping||0),charged=buyerShipping,itemCents=prices[String(variant.id)]??variant.templatePrice,item=itemCents/100,percentFee=(item+charged)*pricing.etsyFeePercent/100,fees=percentFee+pricing.fixedFee+pricing.listingFee,profit=item+charged-variant.cost/100-shipping-fees;return <tr key={variant.id}><td><b>{variant.title}</b></td><td className="printify-product-cost"><b>${(variant.cost/100).toFixed(2)}</b><small>Product only</small></td><td><PriceField value={itemCents} minimum={variant.cost/100} label={"Price for "+variant.title} onCommit={cents=>changeVariantPrice(variant,cents)}/></td><td className={profit+0.005>=pricing.targetProfit?"profit-pass":"profit-low"}><b>${profit.toFixed(2)}</b><small className="profit-fee-note">All Etsy fees included</small></td></tr>})}</tbody>
      </table></div>

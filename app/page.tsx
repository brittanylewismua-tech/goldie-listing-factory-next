...
import { KeywordBank, SavedWorkflow, type KeywordList, type Pricing, type Recipe } from "./factory-tools";
...
type DesignFile = { name: string; size: number; id: string; file: File; previewUrl: string; title: string; tags: string[]; blurb?:string; width?: number; height?: number; visibleBounds?:VisibleBounds; hasTransparency?:boolean; paddingStatus?:"checking"|"trimmed"|"full";etsy?:EtsyDetails;etsyError?:string };
...
const PROGRESS_STEPS = ["Connect Printify","Choose product","Add designs","Review pricing","Create drafts","Titles + description","Etsy listing details","Images + mockups","Final review"];
...
function PriceField({value,minimum,label,onCommit}:{value:number;minimum:number;label:string;onCommit:(cents:number)=>void}){const [draft,setDraft]=useState((value/100).toFixed(2));useEffect(()=>setDraft((value/100).toFixed(2)),[value]);function commit(){const amount=Number(draft);if(!Number.isFinite(amount)){setDraft((value/100).toFixed(2));return}const cents=Math.round(Math.max(minimum,amount)*100);onCommit(cents);setDraft((cents/100).toFixed(2))}return <label aria-label={label}>$<input type="text" inputMode="decimal" value={draft} onChange={event=>setDraft(event.target.value)} onBlur={commit} onKeyDown={event=>{if(event.key==="Enter"){event.currentTarget.blur()}if(event.key==="Escape"){setDraft((value/100).toFixed(2));event.currentTarget.blur()}}}/></label>}
...
function IndividualAutoTitle({design,template,useCommas,initialBankId,onApply}:{design:DesignFile;template:TemplateDetails|null;useCommas:boolean;initialBankId?:string;onApply:(title:string,tags:string[])=>void}){const [bank,setBank]=useState<KeywordList|null>(null),[building,setBuilding]=useState(false),[message,setMessage]=useState("");async function build(){if(!bank)return;setBuilding(true);setMessage("");try{const result=await autoTitleForDesign(design,bank.keywords,useCommas,template);onApply(result.title,tagsFromTitle(result.keywords.join(", ")));setMessage("✓ New title and matching tags applied to this listing only.")}catch(error){setMessage(error instanceof Error?error.message:"Goldie could not create this title.")}finally{setBuilding(false)}}return <details className="individual-title-builder" onClick={event=>event.stopPropagation()}><summary>Create a different title with AI</summary><KeywordBank compact selectionOnly initialId={initialBankId||""} title="Keyword bank" copy="Goldie will choose the phrases that best match this design." onSelect={setBank}/><button className="ai-title-button" disabled={!bank||building} onClick={()=>void build()}>{building?"Creating this title…":"Create title for this design"}</button>{message&&<p className="title-build-message" role="status">{message}</p>}</details>}

function PricingReview({variants,pricing,prices,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string,international:InternationalShippingRate[])=>Promise<void>;onApprove:()=>void}){
  ...
  function resetProfileEditor(profile=selectedProfile){setCustomCharge(profile?profile.domesticPrimary.toFixed(2):"");setCustomAdditional(profile?profile.domesticAdditional.toFixed(2):"");setCustomInternational(profile?profile.international.map(rate=>({...rate,primary:rate.primary.toFixed(2),additional:rate.additional.toFixed(2)})):[]);setCustomProfileName(profile?profile.title:"");setProfileMessage("")}
  useEffect(()=>{resetProfileEditor()},[selectedProfileId,selectedProfile?.title]);
  const enteredCharge=Number(customCharge),enteredAdditional=Number(customAdditional),buyerShipping=Number.isFinite(enteredCharge)&&enteredCharge>=0?enteredCharge:selectedProfile?.domesticPrimary||0,internationalDirty=Boolean(selectedProfile&&customInternational.some((rate,index)=>Math.abs(Number(rate.primary)-selectedProfile.international[index]?.primary)>.004||Math.abs(Number(rate.additional)-selectedProfile.international[index]?.additional)>.004)),customDirty=Boolean(selectedProfile&&(Math.abs(buyerShipping-selectedProfile.domesticPrimary)>.004||Math.abs(enteredAdditional-selectedProfile.domesticAdditional)>.004||internationalDirty||customProfileName.trim()!==selectedProfile.title));
  ...
  function changeProfit(value:number){onPricing({...pricing,targetProfit:Math.max(0,value)});setRecommendationMessage("");}
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      {selectedProfile&&<details className="custom-shipping-builder"><summary>{customDirty?"⚠ Unsaved shipping changes":"Change shipping prices (optional)"}</summary><div className="custom-shipping-body"><label>New profile name<input value={customProfileName} maxLength={60} onChange={event=>{setCustomProfileName(event.target.value);setProfileMessage("")}}/></label><h5>{selectedProfile.originCountry}</h5><div className="shipping-rate-row"><b>Domestic</b><label>First item<span className="money-input">$<input inputMode="decimal" value={customCharge} onChange={event=>{setCustomCharge(event.target.value);setProfileMessage("")}}/></span></label><label>Additional<span className="money-input">$<input inputMode="decimal" value={customAdditional} onChange={event=>{setCustomAdditional(event.target.value);setProfileMessage("")}}/></span></label></div><details className="international-shipping-editor"><summary>International rates ({customInternational.length})</summary>{customInternational.length?<div className="international-rate-list">{customInternational.map((rate,index)=><div className="shipping-rate-row" key={rate.key}><b>{rate.label}</b><label>First item<span className="money-input">$<input aria-label={`${rate.label} first item`} inputMode="decimal" value={rate.primary} onChange={event=>changeInternational(index,"primary",event.target.value)}/></span></label><label>Additional<span className="money-input">$<input aria-label={`${rate.label} additional item`} inputMode="decimal" value={rate.additional} onChange={event=>changeInternational(index,"additional",event.target.value)}/></span></label></div>)}</div>:<p className="no-international-rates">No international destinations.</p>}</details>{customDirty?<div className="custom-shipping-actions"><button disabled={savingProfile||!customCharge||customAdditional===""||!customProfileName.trim()} onClick={()=>void createProfile()}>{savingProfile?"Saving…":"Save new profile"}</button><button onClick={()=>resetProfileEditor()}>Discard changes</button></div>:<div className="shipping-saved-state">No changes made.</div>}{profileMessage&&<small role="status">{profileMessage}</small>}</div></details>}
      <section className="item-pricing-section"><div className="item-pricing-heading"><div><h4>2. Item prices</h4><p>The profit column includes Printify costs, shipping, and Etsy fees.</p></div><div className="profit-goal-control"><label>Profit goal<span className="money-input">$<input aria-label="Profit goal" type="number" min="0" step="0.01" value={pricing.targetProfit} onChange={event=>changeProfit(Number(event.target.value))}/></span></label><button onClick={()=>recalculate()}>Update prices</button></div></div>{recommendationMessage&&<p className="recommendation-result" role="status">{recommendationMessage}</p>}
      <div className="variant-table-wrap"><table className="variant-table">
        <thead><tr><th>Size / color</th><th>Your item price</th><th>Your estimated profit</th></tr></thead>
        <tbody>{variants.map(variant=>{const shipping=Number(variant.shipping||0),charged=buyerShipping,itemCents=prices[String(variant.id)]??variant.templatePrice,item=itemCents/100,percentFee=(item+charged)*pricing.etsyFeePercent/100,fees=percentFee+pricing.fixedFee+pricing.listingFee,profit=item+charged-variant.cost/100-shipping-fees;return <tr key={variant.id}><td><b>{variant.title}</b></td><td><PriceField value={itemCents} minimum={variant.cost/100} label={"Price for "+variant.title} onCommit={cents=>onPrices({...prices,[String(variant.id)]:cents})}/></td><td className={profit+0.005>=pricing.targetProfit?"profit-pass":"profit-low"}><b>${profit.toFixed(2)}</b></td></tr>})}</tbody>
      </table></div></section>
      ...
      <button className="approve-pricing" disabled={!selectedProfile||customDirty} onClick={onApprove}>{approved?"✓ Pricing + shipping approved":!selectedProfile?"Choose a shipping profile first":customDirty?"Save or discard shipping changes first":"Approve pricing + shipping"}</button>
    </section>
  );
}
...
export default function Home() {
  ...
  function goToStep(step:WorkflowStep,replace=false,force=false){if(!force){const issues=requiredForStep(step);if(issues.length)return stopWith("Finish this step before continuing.",issues);if(!canOpenStep(step))return;}setWorkflowStep(step);const url=new URL(window.location.href);url.searchParams.set("step",step);window.history[replace?"replaceState":"pushState"]({},"",url);window.scrollTo({top:0,behavior:"smooth"})}
  ...
  useEffect(()=>{void(async()=>{try{const url=new URL(window.location.href);const id=url.searchParams.get("batch")||"";if(!id)return;const response=await fetch(`/api/batches?id=${encodeURIComponent(id)}`);if(!response.ok)return;const payload=await response.json() as {batch?:{id:string;step:WorkflowStep;status:string;state?:Record<string,unknown>}};if(!payload.batch?.state)return;const state=payload.batch.state as {template?:string;templateDetails?:TemplateDetails;description?:string;pricing?:Pricing;mockupTheme?:string;activeRecipe?:Recipe;designs?:Array<Omit<DesignFile,"file"|"previewUrl">>;drafts?:DraftResult[];complete?:boolean;finishPhase?:FinishPhase;bulkTitles?:string;printifyImageIndices?:number[];variantPrices?:Record<string,number>;etsyShippingProfileId?:number;pricingApproved?:boolean;sizeGuideName?:string};const cached=await loadBatchFiles(id).catch(()=>[]);const designs=(state.designs||[]).map((design,index)=>{const file=cached[index];return file?{...design,file,previewUrl:URL.createObjectURL(file)}:null}).filter(Boolean) as DesignFile[];batchIdRef.current=id;setTemplate(state.template||"");setTemplateDetails(state.templateDetails||null);setDescription(state.description||"");if(state.pricing)setPricing(state.pricing);setVariantPrices(state.variantPrices||{});setEtsyShippingProfileId(Number(state.etsyShippingProfileId)||0);setPricingApproved(Boolean(state.pricingApproved));setMockupTheme(state.mockupTheme||"");setActiveRecipe(state.activeRecipe||null);setFiles(designs);setDrafts(state.drafts||[]);setComplete(Boolean(state.complete));setFinishPhase(state.finishPhase||"details");setBulkTitles(state.bulkTitles||"");setPrintifyImageIndices(state.printifyImageIndices||[]);setSizeGuideName(state.sizeGuideName||"");setResumeProcessing(payload.batch.status==="processing"&&designs.length>0);const step=state.complete?"finish":payload.batch.step;setWorkflowStep(step);url.searchParams.set("step",step);window.history.replaceState({},"",url);if(payload.batch.status==="processing"&&state.template)void loadTemplateUrl(state.template)}finally{snapshotReady.current=true;setRestoringBatch(false)}})()},[]);
  ...
  useEffect(()=>{if(!snapshotReady.current||restoringBatch||(!files.length&&!drafts.length))return;const timer=window.setTimeout(()=>{const id=batchIdRef.current||crypto.randomUUID();batchIdRef.current=id;window.localStorage.setItem("goldie-active-batch",id);const designs=files.map(({file:ignoredFile,previewUrl:ignoredPreview,...design})=>design);void fetch("/api/batches",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id,status:running?"processing":complete?drafts.some(draft=>draft.status==="Failed")?"needs_attention":"complete":"draft",step:workflowStep,setupName:activeRecipe?.name||"",productTitle:templateDetails?.blueprintTitle||"",designCount:files.length,state:{template,templateDetails,description,pricing,variantPrices,etsyShippingProfileId,pricingApproved,mockupTheme,activeRecipe,designs,drafts,complete,finishPhase,bulkTitles,printifyImageIndices,sizeGuideName}})})},700);return()=>window.clearTimeout(timer);// eslint-disable-next-line react-hooks/exhaustive-deps
  },[restoringBatch,workflowStep,finishPhase,template,templateDetails,description,pricing,variantPrices,etsyShippingProfileId,pricingApproved,mockupTheme,activeRecipe,files.map(file=>`${file.id}:${file.title}:${file.tags.join("|")}:${JSON.stringify(file.etsy||{})}`).join(";"),drafts,complete,running,bulkTitles,printifyImageIndices,sizeGuideName]);
  ...
  function useRecipe(recipe: Recipe) { const changingProduct=Boolean((activeRecipe?.id&&activeRecipe.id!==recipe.id)||(template&&template!==recipe.templateUrl));if(changingProduct&&(files.length>0||drafts.length>0||complete)){const count=files.length;if(!window.confirm(`Switch to “${recipe.name}” and start a new batch? This removes ${count} ${count===1?"design":"designs"} and all work from the current batch on this page.`))return false;clearCurrentBatch(false)}setActiveRecipe(recipe);setPrintifyImageIndices(recipe.printifyImageIndices||[]);setEtsyShippingProfileId(Number(recipe.etsyShippingProfileId)||0);setTemplate(recipe.templateUrl);setMockupTheme("");const nextPricing={...pricing,targetProfit:DEFAULT_PRICING.targetProfit};setPricing(nextPricing);setTemplateDetails(null);void loadTemplateUrl(recipe.templateUrl,nextPricing);return true; }
  ...
  async function loadTemplateUrl(productUrl = template, pricingOverride?:Pricing) {
    ...
    try {
      ...
      setTemplateDetails(result.product);setDescription(result.product.description||"");if(result.product.standardShipping!=null)setPricing(current=>({...current,shippingCost:result.product!.standardShipping!,shippingCharged:0}));setVariantPrices(Object.fromEntries((result.product.variants||[]).map(variant=>[String(variant.id),variant.templatePrice])));setPricingApproved(false); return true;
      ...
      try {
        ...
        for (let pipelineAttempt = 1; pipelineAttempt <= 3; pipelineAttempt += 1) {
          ...
          try {
            ...
            const response = await fetchWithDeadline("/api/printify/drafts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ batchId: templateDetails?.batchId, title: design.title || undefined, tags: design.tags, pricing, etsyBuyerShipping:etsyShippingProfiles.find(profile=>profile.id===etsyShippingProfileId)?.domesticPrimary||0, variantPrices, description:fullDescription, maxPlacementScale:isRigidPaperProduct(templateDetails)?1:undefined, fileName: upload.fileName, stagedId: staged.stagedId, supportReference: staged.reference, clientId: design.id }) }, 4 * 60 * 1000);
  ...
  const workflowHero = {
    ...
    finish: finishPhase==="details" ? { eyebrow: "STEP 6 · TITLES + DESCRIPTION", title: "Finish the listing words.", copy: "Create the titles and tags, review each listing, then confirm the reusable description." } : finishPhase==="etsy" ? { eyebrow: "STEP 7 · ETSY LISTING DETAILS", title: "Review what Etsy needs.", copy: "Goldie chooses the category and product-specific details. Sizes, colors, and prices carry over from Printify automatically." } : finishPhase==="mockups" ? { eyebrow: "STEP 8 · IMAGES + MOCKUPS", title: "Choose the listing images.", copy: "Review the real Printify previews, choose flatlays, and add your own mockups." } : { eyebrow: "STEP 9 · FINAL REVIEW", title: "Review the finished batch.", copy: "Confirm the words, pricing, Etsy details, images, and mockups before anything goes live." },
  }[workflowStep];
    ...
    <main className="app-shell">
      ...
      <section className={`workspace ${complete&&workflowStep==="finish"&&finishPhase==="mockups"?"mockup-workspace":""}`}>
        ...
        <div className="steps-column">
          ...
          <div className={`workflow-panel ${workflowStep==="setup"?"active-panel":"hidden-panel"}`}><SavedWorkflow connected={connected} templateUrl={template} templateVerified={templateLoaded} loadingTemplate={loadingTemplate} onTemplateUrl={(value) => { templateLoadVersion.current+=1;setLoadingTemplate(false);setTemplate(value);setTemplateDetails(null);setTemplateError(""); }} onUseRecipe={useRecipe} onStartNewProduct={startNewProduct} onVerifyTemplate={loadTemplateUrl} />
          ...
          <article className={`step-card workflow-panel ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="finish"&&finishPhase==="details")?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              ...
              <p className="step-copy">{workflowStep==="finish"?"Create the batch titles, review each listing, then confirm the description used across the batch.":"Build one focused batch of up to 20 finished designs. Upload a folder or select individual images."}</p>
              ...
              {files.length>0&&!complete&&workflowStep==="designs"&&<button className="workflow-next" disabled={!designsFinished} onClick={()=>goToStep("review")}>{designsFinished?"Review this batch":`Preparing ${designsPreparing} ${designsPreparing===1?"design":"designs"}…`} {designsFinished&&<span>→</span>}</button>}
              {files.length > 0 && complete && <div className="listing-editor">
                <div className="editor-heading"><div><b>1. Create titles and tags</b><span>Goldie analyzes every design separately and chooses only relevant phrases from your validated keyword bank. Review or edit any listing afterward.</span></div><span>{files.length} listings</span></div>
                <section className="batch-title-builder"><div><p className="mini-label">BATCH TITLE BUILDER</p><h3>Create titles for the whole batch</h3><p>Let Goldie choose the best validated keywords for each design, or choose the exact keywords yourself.</p></div><div className="title-builder-choice" role="group" aria-label="How do you want to create batch titles?"><button className={titleBuilderMode==="ai"?"active":""} onClick={()=>setTitleBuilderMode("ai")}><b>Goldie chooses keywords</b><span>Creates a different title for each design</span></button><button className={titleBuilderMode==="manual"?"active":""} onClick={()=>setTitleBuilderMode("manual")}><b>I choose keywords</b><span>Uses your selections across the batch</span></button></div><div className="title-style-toggle"><span>Title style</span><button className={titleJoiner===", "?"active":""} onClick={()=>changeTitleJoiner(", ")}>Title with commas</button><button className={titleJoiner===" "?"active":""} onClick={()=>changeTitleJoiner(" ")}>Title without commas</button></div>{titleBuilderMode==="ai"?<div className="title-builder-pane"><KeywordBank selectionOnly initialId={activeRecipe?.keywordListId||""} onSelect={setAutoTitleBank} title="Choose a keyword bank" copy="Goldie selects the best phrases for each individual design."/><button className="ai-title-button" disabled={titleBuilding||!autoTitleBank||!files.length} onClick={()=>void buildBatchTitle()}>{titleBuilding?`Creating ${files.length} titles…`:"Auto-create all titles"}</button>{titleBuildMessage&&<p className="title-build-message" role="status">{titleBuildMessage}</p>}</div>:<div className="title-builder-pane manual-title-builder"><KeywordBank initialId={activeRecipe?.keywordListId||""} onAdd={addBatchKeyword} title="Choose a keyword bank" copy="Click keywords in the order you want them. Every click updates all listings below."/><div className="selected-batch-keywords"><div><b>Selected keywords</b>{batchKeywords.length>0&&<button onClick={clearBatchKeywords}>Clear all</button>}</div>{batchKeywords.length?<div className="selected-keyword-chips">{batchKeywords.map(keyword=><button key={keyword} onClick={()=>removeBatchKeyword(keyword)}>{keyword}<span>×</span></button>)}</div>:<p>No keywords selected yet.</p>}</div>{batchKeywords.length>0&&<div className="batch-title-preview"><b>Batch title preview</b><span>{batchKeywords.join(titleJoiner)}</span><small>Applied to every listing below. You can still edit any listing individually.</small></div>}</div>}</section>
                <details className="permanent-description batch-description"><summary><span><b>Batch description</b><small>Add one description to every listing</small></span><em>{description.trim()?"✓ Added":"Review"}</em></summary><div className="batch-description-body"><p>Imported from your Printify template. Edit it once here and Goldie keeps the spacing and formatting on every listing.</p><label>Reusable product description<textarea rows={9} value={description} onChange={event=>setDescription(event.target.value)} placeholder="Add sizing, materials, production, care, and shipping information"/></label><small>An optional custom opening can be added inside any listing below.</small></div></details>
                <div className="design-table" ref={listingResultsRef}>{files.map((design) => { const displayScale=isRigidPaperProduct(templateDetails)?Math.min(templateDetails?.placementScale||1,1):templateDetails?.placementScale;const quality = design.width && templateDetails?.maxPrintWidth && displayScale ? printifyDpi(design.width, templateDetails.maxPrintWidth, displayScale) : null; const qualityReady = Boolean(quality && quality.dpi >= 300); return <article className={`design-line ${activeDesign === design.id ? "active" : ""}`} key={design.id} onClick={() => setActiveDesign(design.id)}><img src={design.previewUrl} alt=""/><div className="design-fields"><label>Title <span>{design.title.length}/140</span><input value={design.title} maxLength={140} onChange={(e) => { const title = e.target.value; updateDesign(design.id, { title, tags: tagsFromTitle(title),etsy:undefined }); }}/></label><label>Tags <span>{design.tags.length}/13</span><input value={design.tags.join(", ")} onChange={(e) => updateDesign(design.id, { tags: [...new Set(e.target.value.split(",").map((tag) => tag.trim().toLowerCase()).filter((tag) => tag && tag.length <= 20))].slice(0, 13),etsy:undefined })} placeholder="Exact title phrases, separated by commas"/></label><div className="tag-row">{design.tags.map((tag) => <span key={tag}>{tag}</span>)}{!design.tags.length && <small>Goldie will create matching tags with the title.</small>}</div><IndividualAutoTitle design={design} template={templateDetails} useCommas={titleJoiner===", "} onApply={(title,tags)=>{setActiveDesign(design.id);updateDesign(design.id,{title,tags,etsy:undefined,etsyError:""})}}/><details className="individual-description" onClick={event=>event.stopPropagation()}><summary><span>Customize description for this listing</span><small>{design.blurb?.trim()?"✓ Custom opening added":"Optional"}</small></summary><div><p>Add information that belongs only to this design. It will appear above the batch description.</p><label>Custom opening<textarea rows={4} value={design.blurb||""} onChange={event=>{const blurb=event.target.value;updateDesign(design.id,{blurb,etsy:design.etsy?{...design.etsy,blurb}:undefined,etsyError:""})}} placeholder="Example: A playful western print made for colorful gallery walls…"/></label>{design.blurb?.trim()&&<button type="button" onClick={()=>updateDesign(design.id,{blurb:"",etsy:design.etsy?{...design.etsy,blurb:""}:undefined,etsyError:""})}>Remove custom opening</button>}<small>Final order: this custom opening, then the batch description.</small></div></details>{design.etsy&&<details className="etsy-auto"><summary>✓ Etsy details completed · {design.etsy.category}</summary><div className="etsy-attribute-grid">{Object.entries({...design.etsy.attributes,...design.etsy.optional}).map(([key,value])=><label key={key}>{key}<input value={value} onChange={e=>updateDesign(design.id,{etsy:{...design.etsy!,attributes:{...design.etsy!.attributes,[key]:e.target.value}}})}/></label>)}</div><small>Optional fields Goldie could not justify were left blank.</small></details>}{design.etsyError&&<small className="field-error">{design.etsyError}</small>}{design.paddingStatus==="trimmed"&&<small className="padding-note">Transparent padding detected · placement scale preserved for print quality</small>}</div><div className={`quality-pill ${qualityReady ? "pass" : "check"}`}><b>{!quality ? "Calculating Printify DPI…" : qualityReady ? `✓ ${quality.dpi} DPI in Printify` : `${quality.dpi} DPI in Printify`}</b><small>{quality ? `${quality.level} resolution · 300 DPI recommended` : design.width ? `${design.width} × ${design.height}px` : "Reading dimensions…"}</small></div></article>; })}</div>
                <button className="workflow-next" disabled={preparingEtsy} onClick={()=>void continueToEtsyDetails()}>{preparingEtsy?"Preparing Etsy details…":"Review Etsy listing details"} <span>→</span></button>
              </div>}
            </div>
          </article>
          {workflowStep==="finish"&&finishPhase==="etsy"&&<article className="step-card etsy-details-step active-panel"><div className="step-number">07</div><div className="step-content"><div className="step-heading"><div><p className="mini-label">ETSY LISTING DETAILS</p><h2>Review the details Goldie completed</h2></div><span className="done-mark">{files.filter(file=>file.etsy).length}/{files.length} ready</span></div><p className="step-copy">Goldie uses the Printify product to choose only the Etsy fields that apply. Optional fields stay blank unless they genuinely fit.</p><div className="variant-transfer-note"><span>✓</span><div><b>Sizes, colors, and individual prices are already handled</b><small>Printify sends every enabled variation and its approved price to Etsy automatically.</small></div></div><div className="etsy-detail-list">{files.map(design=><article className="etsy-detail-card" key={design.id}><img src={design.previewUrl} alt=""/><div><span className="etsy-listing-name">{design.title||design.name}</span>{design.etsy?<><label>Etsy category<input value={design.etsy.category} onChange={event=>updateDesign(design.id,{etsy:{...design.etsy!,category:event.target.value}})}/></label><label>Design-specific introduction<textarea rows={3} value={design.etsy.blurb} onChange={event=>updateDesign(design.id,{etsy:{...design.etsy!,blurb:event.target.value}})}/></label><div className="etsy-attribute-grid">{Object.entries({...design.etsy.attributes,...design.etsy.optional}).map(([key,value])=><label key={key}>{key}<input value={value} onChange={event=>updateDesign(design.id,{etsy:{...design.etsy!,attributes:{...design.etsy!.attributes,[key]:event.target.value}}})}/></label>)}</div><small className="optional-note">Goldie leaves unsupported holidays, occasions, recipients, and other optional fields blank.</small></>:<div className="etsy-detail-error"><b>Etsy details still need to be created.</b><span>{design.etsyError}</span><button onClick={()=>void prepareOne(design)}>Try this listing again</button></div>}</div></article>)}</div><button className="workflow-next" disabled={savingEtsyDetails||files.some(file=>!file.etsy)} onClick={()=>void saveAllEtsyDetails()}>{savingEtsyDetails?"Saving Etsy details…":"Save details and choose images"} <span>→</span></button></div></article>}
          {workflowStep==="finish"&&finishPhase==="final"&&<article className="step-card final-review active-panel"><div className="step-number">09</div><div className="step-content"><div className="step-heading"><div><p className="mini-label">FINAL REVIEW</p><h2>Your batch is ready for its final check</h2></div><span className="done-mark">✓ {drafts.filter(draft=>draft.status==="Created").length} drafts</span></div><p className="step-copy">Confirm the checklist below. Nothing is published until you use the final button.</p><div className="final-checklist"><span>✓ Every enabled variation and price was reviewed</span><span>✓ {etsyShippingProfiles.find(profile=>profile.id===etsyShippingProfileId)?.title||"Etsy shipping profile"} will be applied automatically</span><span>{files.every(file=>file.title.trim())?"✓":"!"} Titles are complete</span><span>{files.every(file=>file.tags.length)?"✓":"!"} Tags are complete</span><span>{description.trim()?"✓":"!"} Description {description.trim()?"is attached":"is blank"}</span><span>{files.every(file=>file.etsy)?"✓":"!"} Etsy categories and product details {files.every(file=>file.etsy)?"are ready":"still need review"}</span><span>✓ Printify placement and listing images were reviewed</span>{sizeGuideName&&<span>✓ {sizeGuideName} will be applied to every Etsy listing</span>}</div><div className="final-review-actions"><button onClick={()=>setFinishPhase("details")}>Review titles + description</button><button onClick={()=>setFinishPhase("etsy")}>Review Etsy details</button><button onClick={()=>setFinishPhase("mockups")}>Review images + mockups</button></div><div className="publish-live-warning"><b>These listings will be published live on Etsy.</b><span>They will not go to Etsy drafts. Nothing happens until you confirm again in the next window.</span></div><button className="publish-all-button" disabled={publishing||drafts.some(draft=>draft.status==="Failed")} onClick={openPublishConfirmation}>{publishing?"Publishing…":"Publish all live on Etsy"}</button>{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}</div></article>}
        </div>

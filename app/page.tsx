...
function PricingReview({variants,pricing,prices,shippingPercent,approved,onPricing,onPrices,onShippingPercent,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;shippingPercent:number;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onShippingPercent:(value:number)=>void;onApprove:()=>void}){
  ...
  const buyerShipping=(shipping:number,percent=normalizedPercent)=>shipping*percent/100;
  ...
  return <section className={`variant-pricing ${approved?"approved":""}`}><div className="variant-pricing-head"><div><p className="mini-label">PRICING REVIEW</p><h3>See and approve every enabled variant</h3><p>Goldie pulled the exact Printify cost and US shipping for each enabled size and color. Edit any item price before approval.</p></div>{approved&&<span>✓ Approved</span>}</div><div className="pricing-controls"><label>Profit target<input type="number" min="0" step="0.01" value={pricing.targetProfit} onChange={event=>changeProfit(Number(event.target.value))}/></label><div className="shipping-split"><span>How much shipping should the buyer pay?</span><div className="shipping-presets">{[0,50,100].map(percent=><button type="button" className={normalizedPercent===percent?"active":""} key={percent} onClick={()=>changeShipping(percent)}>{percent===0?"Free shipping":percent===100?"All shipping":`${percent}% of shipping`}</button>)}</div><label><b>{normalizedPercent}% paid by buyer</b><input aria-label="Percent of shipping paid by buyer" type="range" min="0" max="100" step="1" value={normalizedPercent} onChange={event=>changeShipping(Number(event.target.value))}/><input aria-label="Custom percent of shipping paid by buyer" type="number" min="0" max="100" step="1" value={normalizedPercent} onChange={event=>changeShipping(Number(event.target.value))}/><span>%</span></label><small>Goldie builds the remaining {100-normalizedPercent}% into each variant’s item price.</small></div><button onClick={()=>recalculate()}>Reset recommendations</button></div><div className="shipping-api-note"><b>Shipping split is calculated, not yet applied at Etsy.</b><span>Printify can only publish its full shipping template. When Etsy access is connected, Goldie can assign the exact partial-shipping profile automatically. Until then, confirm the shipping profile on Etsy after publishing.</span></div><div className="fee-profile-summary" aria-label="Etsy fees included in every calculation"><span>{pricing.etsyFeePercent.toFixed(1)}% Etsy transaction + payment percentage</span><span>${pricing.fixedFee.toFixed(2)} fixed payment fee</span><span>${pricing.listingFee.toFixed(2)} listing / renewal fee</span></div><div className="variant-table-wrap"><table className="variant-table"><thead><tr><th>Enabled variant</th><th>Printify cost</th><th>Printify shipping</th><th>Buyer pays</th><th>Built into price</th><th>Etsy % fee</th><th>Fixed payment fee</th><th>Listing fee</th><th>Total Etsy fees</th><th>Item price</th><th>Projected profit</th></tr></thead><tbody>{variants.map(variant=>{const shipping=Number(variant.shipping||0),charged=buyerShipping(shipping),included=shipping-charged,itemCents=prices[String(variant.id)]??recommendedPrice(variant.cost,{...pricing,shippingCost:shipping,shippingCharged:charged}),item=itemCents/100,percentFee=(item+charged)*pricing.etsyFeePercent/100,fixedFee=pricing.fixedFee,listingFee=pricing.listingFee,fees=percentFee+fixedFee+listingFee,profit=item+charged-variant.cost/100-shipping-fees;return <tr key={variant.id}><td><b>{variant.title}</b></td><td>${(variant.cost/100).toFixed(2)}</td><td>${shipping.toFixed(2)}</td><td>${charged.toFixed(2)}</td><td>${included.toFixed(2)}</td><td>${percentFee.toFixed(2)}</td><td>${fixedFee.toFixed(2)}</td><td>${listingFee.toFixed(2)}</td><td><b>${fees.toFixed(2)}</b></td><td><label aria-label={`Price for ${variant.title}`}>$<input type="number" min={(variant.cost/100).toFixed(2)} step="0.01" value={(itemCents/100).toFixed(2)} onChange={event=>onPrices({...prices,[String(variant.id)]:Math.round(Math.max(0,Number(event.target.value))*100)})}/></label></td><td className={profit+0.005>=pricing.targetProfit?"profit-pass":"profit-low"}>${profit.toFixed(2)}</td></tr>})}</tbody></table></div><p className="pricing-footnote">Every recommendation includes the complete Etsy fee profile shown above. Offsite Ads and order-specific sales tax processing are excluded because they cannot be known for every order.</p><button className="approve-pricing" onClick={onApprove}>{approved?"✓ Pricing approved":"Approve all variant prices"}</button></section>
}
...
export default function Home() {
  ...
  // Keep older saved batches compatible while storing the new exact percentage.
  const shippingMode=shippingPercent;
  const setShippingMode=(value:number|"buyer"|"free")=>setShippingPercent(typeof value==="number"?value:value==="buyer"?100:0);

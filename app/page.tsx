...
type EtsyDetails={category:string;taxonomyId?:number;properties?:EtsyPropertySelection[];attributes:Record<string,string>;optional:Record<string,string>;blurb:string;confidence:"high"|"review"};
  ...
  useEffect(()=>{
    const guardFinalActions=(event:MouseEvent)=>{
      ...
      if(element.classList.contains("publish-all-button")){
        issues=[...(!etsyConnected?["Connect the Etsy shop that will receive these listings."]:[]),...missingPublishFields().map(field=>`${field} must be completed before publishing.`),...requiredForStep("finish")];
      }
  ...
  return (
    <main className="app-shell">
      ...
      <section className={`workspace ${complete&&workflowStep==="finish"&&finishPhase==="mockups"?"mockup-workspace":""}`}>
        ...
        <button className="workflow-back" type="button" onClick={goBackOneStep}><span aria-hidden="true">←</span> Back</button>

...
function PricingReview({variants,pricing,prices,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onApprove:()=>void}){
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <div className="variant-table-wrap"><table className="variant-table">
        ...
        <tbody>{variants.map(variant=>{const shipping=Number(variant.shipping||0),charged=buyerShipping,itemCents=prices[String(variant.id)]??variant.templatePrice,item=itemCents/100,percentFee=(item+charged)*pricing.etsyFeePercent/100,fees=percentFee+pricing.fixedFee+pricing.listingFee,profit=item+charged-variant.cost/100-shipping-fees;return <tr key={variant.id}><td><b>{variant.title}</b></td><td><label aria-label={"Price for "+variant.title}>$<input type="number" min={(variant.cost/100).toFixed(2)} step="0.01" value={(itemCents/100).toFixed(2)} onChange={event=>onPrices({...prices,[String(variant.id)]:Math.round(Math.max(variant.cost/100,Number(event.target.value)||0)*100)})}/></label></td><td className={profit+0.005>=pricing.targetProfit?"profit-pass":"profit-low"}><b>${profit.toFixed(2)}</b></td></tr>})}</tbody>
      </table></div>
...
export default function Home() {
  ...
  useEffect(()=>{if(!etsyConnected)return;setShippingProfilesLoading(true);setShippingProfilesError("");fetch("/api/etsy/shipping-profiles").then(async response=>{const result=await response.json() as {profiles?:EtsyShippingProfile[];error?:string};if(!response.ok)throw new Error(result.error||"Your Etsy shipping profiles could not be loaded.");setEtsyShippingProfiles(result.profiles||[]);setEtsyShippingProfileId(current=>current&&result.profiles?.some(profile=>profile.id===current)?current:0)}).catch(error=>setShippingProfilesError(error instanceof Error?error.message:"Your Etsy shipping profiles could not be loaded.")).finally(()=>setShippingProfilesLoading(false))},[etsyConnected]);

...
function PricingReview({variants,pricing,prices,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprove}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number)=>Promise<void>;onApprove:()=>void}){
  ...
  function chooseProfile(id:number){onSelectProfile(id)}
  async function createProfile(){if(!selectedProfile)return;const charge=Number(customCharge);if(!Number.isFinite(charge)||charge<0)return setProfileMessage("Enter the domestic shipping amount the buyer should pay.");setSavingProfile(true);setProfileMessage("");try{await onCreateProfile(selectedProfile.id,charge);setProfileMessage("✓ New Etsy shipping profile saved and selected.")}catch(error){setProfileMessage(error instanceof Error?error.message:"The shipping profile could not be saved.")}finally{setSavingProfile(false)}}
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <div className="pricing-controls">
        <label className="shipping-profile-select"><span>Etsy shipping profile for this batch</span><select value={selectedProfileId||""} disabled={profilesLoading} onChange={event=>chooseProfile(Number(event.target.value))}><option value="">{profilesLoading?"Loading your Etsy shipping profiles…":"Choose an Etsy shipping profile"}</option>{profiles.map(profile=><option key={profile.id} value={profile.id}>{profile.title}</option>)}</select><small>Goldie applies this exact Etsy profile to every listing when you publish.</small></label>
      ...
      {selectedProfile&&<div className="shipping-profile-proof"><div><span>Selected Etsy profile</span><b>{selectedProfile.title}</b></div><div><span>Buyer pays domestically</span><b>${selectedProfile.domesticPrimary.toFixed(2)}</b><small>${selectedProfile.domesticAdditional.toFixed(2)} with another item</small></div><div><span>International shipping</span><b>{selectedProfile.international.length?`${selectedProfile.international.length} destination ${selectedProfile.international.length===1?"rate":"rates"}`:"Not included"}</b><small>{selectedProfile.international.slice(0,3).map(rate=>`${rate.label}: $${rate.primary.toFixed(2)}`).join(" · ")||"This profile has no international destination."}</small></div></div>}
      {selectedProfile&&<details className="custom-shipping-builder"><summary>Charge the buyer a different domestic shipping amount</summary><p>Goldie creates a new reusable Etsy profile. Your original profile and its international rates stay unchanged.</p><div className="shipping-amount-presets"><button onClick={()=>setCustomCharge((selectedProfile.domesticPrimary/2).toFixed(2))}>Half: ${(selectedProfile.domesticPrimary/2).toFixed(2)}</button><button onClick={()=>setCustomCharge("0.00")}>Free domestic shipping</button></div><label>Buyer pays domestically<span className="money-input">$<input inputMode="decimal" value={customCharge} onChange={event=>setCustomCharge(event.target.value)} placeholder={selectedProfile.domesticPrimary.toFixed(2)}/></span></label><button disabled={savingProfile||!customCharge} onClick={()=>void createProfile()}>{savingProfile?"Saving in Etsy…":"Save as a new Etsy shipping profile"}</button>{profileMessage&&<small role="status">{profileMessage}</small>}</details>}
      ...
      <button className="approve-pricing" disabled={!selectedProfile} onClick={onApprove}>{approved?"✓ Pricing + shipping approved":selectedProfile?"Approve pricing + shipping":"Choose an Etsy shipping profile first"}</button>
    </section>
  );
}
...
export default function Home() {
  ...
  async function createCustomShippingProfile(baseProfileId:number,domesticPrimary:number){const response=await fetch("/api/etsy/shipping-profiles",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({baseProfileId,domesticPrimary})}),result=await response.json() as {id?:number;error?:string};if(!response.ok||!result.id)throw new Error(result.error||"The Etsy shipping profile could not be saved.");await loadEtsyShippingProfiles(result.id);if(activeRecipe){const updated={...activeRecipe,etsyShippingProfileId:result.id};await fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(updated)});setActiveRecipe(updated)}setPricingApproved(false)}

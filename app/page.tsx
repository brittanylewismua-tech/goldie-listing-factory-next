...
function PricingReview({variants,pricing,prices,productName,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprovalChange}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;productName:string;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string,international:InternationalShippingRate[])=>Promise<void>;onApprovalChange:(ready:boolean)=>void}){
  ...
  function recalculate(nextPricing=pricing,nextBuyerShipping=buyerShipping){const calculated=Object.fromEntries(variants.map(variant=>{const shipping=Number(variant.shipping||0);return[String(variant.id),wholePrice(recommendedPrice(variant.cost,{...nextPricing,shippingCost:shipping,shippingCharged:nextBuyerShipping}))]})),next=normalizePricesByCost(variants,calculated),changed=variants.filter(variant=>next[String(variant.id)]!==(prices[String(variant.id)]??variant.templatePrice)).length;onPrices(next);setRecommendationMessage(changed?`✓ Updated ${changed} ${changed===1?"price":"prices"}. Review each cost group below before continuing.`:"✓ Your current prices already meet this profit goal. Nothing needed to change.")}
  const initialPriceSignature=variants.map(variant=>`${variant.id}:${variant.cost}:${variant.shipping||0}:${variant.templatePrice}`).join("|");
  useEffect(()=>{if(!selectedProfile||!variants.length)return;const stillUsingTemplatePrices=variants.every(variant=>(prices[String(variant.id)]??variant.templatePrice)===variant.templatePrice);if(!stillUsingTemplatePrices)return;const calculated=Object.fromEntries(variants.map(variant=>[String(variant.id),recommendedPrice(variant.cost,{...pricing,shippingCost:Number(variant.shipping||0),shippingCharged:selectedProfile.domesticPrimary})]));onPrices(normalizePricesByCost(variants,calculated));setRecommendationMessage("✓ Goldie calculated every price from your profit goal, product costs, shipping, and Etsy fees.")},[selectedProfile?.id,initialPriceSignature]);
  ...
  function chooseProfile(id:number){const profile=profiles.find(item=>item.id===id);onSelectProfile(id);resetProfileEditor(profile);if(profile)recalculate(pricing,profile.domesticPrimary)}
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <div className="price-group-list">{priceGroups.map(group=>{const groupPrices=group.items.map(variant=>prices[String(variant.id)]??variant.templatePrice),groupPrice=Math.max(...groupPrices),profits=group.items.map(variant=>estimatedProfit(groupPrice,variant.cost,{...pricing,shippingCost:Number(variant.shipping||0),shippingCharged:buyerShipping})),lowestProfit=Math.min(...profits),examples=group.items.map(item=>item.title).filter(Boolean);return <article className="price-group" key={group.cost}>
        ...
        <details className="price-group-details"><summary>View included variants or edit one separately</summary><div className="individual-variant-list">{group.items.map(variant=>{const itemCents=prices[String(variant.id)]??variant.templatePrice,profit=estimatedProfit(itemCents,variant.cost,{...pricing,shippingCost:Number(variant.shipping||0),shippingCharged:buyerShipping});return <div key={variant.id}><span><b>{variant.title}</b><small>Printify cost ${(variant.cost/100).toFixed(2)}</small></span><PriceField value={itemCents} minimum={variant.cost/100} label={`Individual price for ${variant.title}`} onCommit={cents=>changeIndividualPrice(variant,cents)}/><span className={profit+0.005>=pricing.targetProfit?"profit-pass":"profit-low"}><b>${profit.toFixed(2)} profit</b><small>All Etsy fees included</small></span></div>})}</div></details>
      </article>})}</div>
...
export default function Home() {
  ...
  async function useBundle(bundle:ProductBundle,recipes:Recipe[]){
    ...
    clearCurrentBatch(true);setActiveBundle(bundle);setBundleRecipes(recipes);setBundleIndex(0);return await selectRecipe(recipes[0]);
  }

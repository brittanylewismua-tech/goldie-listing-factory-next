    ...
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"} style={{"--active-product":`"${currentProductName.replace(/["\\]/g,"")}"`} as React.CSSProperties}>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="finish"&&finishPhase==="mockups"?"mockup-workspace":""}`}>
        <nav className="workflow-progress" aria-label="Listing Factory progress">
          ...
          {railInFinish&&<div className="rail-substeps" role="group" aria-label="Finish listings steps">{RAIL_FINISH.map(index=>{const active=progressIndex===index,done=index<progressIndex;return <button key={PROGRESS_STEPS[index]} className={`rail-substep ${active?"active":""} ${done?"done":""}`} disabled={!railAvailable(index)} aria-current={active?"step":undefined} onClick={()=>openProgressStep(index)}><span className="rail-substep-dot">{done?"✓":""}</span><span><b>{PROGRESS_SHORT_LABELS[index]}</b><small>{localPreview&&!active?"Open preview":progressStatus(index,active,done)}</small></span></button>})}</div>}

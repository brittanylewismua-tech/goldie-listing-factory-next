...
type ProductVariant={id:number;title:string;cost:number;templatePrice:number;shipping?:number|null;options?:number[];colorId?:number|null;templateEnabled?:boolean};
...
type TemplateDetails = { id: string; batchId: string; title: string; description:string; blueprintId:number;blueprintTitle:string;brand:string;model:string;provider: string; enabledVariants: number;colorOptions?:ProductColor[]; variants:ProductVariant[]; shop: string; standardShipping?:number|null;shippingCurrency?:string;shippingTemplateId:string;shippingProfileNeedsSelection?:boolean;freeShipping:boolean;maxPrintWidth?: number | null; maxPrintHeight?: number | null; placementScale?: number | null };
...
const WORKFLOW_HELP = [
  {title:"Connect Printify and Etsy",intro:"Both accounts must be connected before Goldie can build a complete listing.",sections:[{heading:"Printify connection",copy:"Connect the Printify account that contains your saved template product. Goldie uses it to read product costs and variants, upload artwork, and create unpublished product drafts."},{heading:"Etsy connection",copy:"Connect the Etsy shop linked to that Printify template. Goldie uses Etsy’s real categories, attributes, shipping profiles, and publishing connection."},{heading:"Nothing publishes here",copy:"This step only verifies access. Goldie cannot publish a listing until you reach the final review and confirm publishing a second time."}]},
  {title:"Choose or create a saved product",intro:"A saved product tells Goldie which physical product to create and exactly where the artwork belongs. You will choose the listing options inside Listing Factory.",sections:[{heading:"Set only these three things in Printify",copy:"Choose the product type, choose the print provider or manufacturer, and place temporary artwork at the size and position you want Goldie to reuse. That is all this template needs."},{heading:"Goldie handles the rest",copy:"Do not use the template to lock in colors, sizes, prices, shipping, listing photos, mockups, titles, tags, descriptions, Etsy details, or personalization. You will review and choose those inside Listing Factory for each batch."},{heading:"How to copy the correct Printify link",copy:"In Printify, open My Products and select the product. Click into the product editor so you can see the artwork placement controls. While that editor is open, copy the entire URL from the browser address bar and paste it into Listing Factory. Do not copy the Etsy listing URL, the public product URL, or only the product ID."},{heading:"Product bundles",copy:"Choose a bundle only when the same uploaded artwork should be created on two to four different saved products, such as a T-shirt, sweatshirt, and hoodie. Upload each design once. Goldie then walks you through the pricing and listing choices for each product separately."}]},
  ...
  {title:"Review prices and shipping",intro:"Set the buyer-facing item prices and confirm the Etsy shipping profile before any Printify drafts are created.",sections:[{heading:"Use the profit goal",copy:"Goldie calculates a recommended price for every exact Printify product cost using shipping and the Etsy fee settings shown in the calculation details."},{heading:"Edit matching-cost groups",copy:"Variants with the exact same Printify cost share one price field. More expensive colors, sizes, materials, finishes, or other options remain separate automatically."},{heading:"Choose shipping",copy:"Keep the shipping profile imported from the template, or create a named copy with different domestic, additional-item, or international charges."},{heading:"Approve the result",copy:"Review the lowest estimated profit in every group. Offsite Ads and sales tax are excluded because they vary by order."}]},
...
function PrintifyImagePicker({ images,indices,onApplyOne,onApplyAll,onSaveRecipe }: { images: string[];indices:number[];onApplyOne:(indices:number[])=>void;onApplyAll:(indices:number[])=>void;onSaveRecipe?:(indices:number[])=>void|Promise<void> }) {
  const [selected,setSelected]=useState<Set<number>>(new Set(indices)),[expanded,setExpanded]=useState<string>(""),[action,setAction]=useState<"clear"|"all"|"future"|"">(""),[feedback,setFeedback]=useState(""),[savingFuture,setSavingFuture]=useState(false);
  useEffect(()=>setSelected(new Set(indices)),[indices,images.length]);
  ...
  function toggle(index:number){const next=new Set(selected);if(next.has(index))next.delete(index);else next.add(index);setSelected(next);setAction("");onApplyOne([...next].sort((a,b)=>a-b))}
  ...
  return <><details className="printify-image-picker"><summary>Choose Printify flatlays ({selected.size} selected)</summary><p>Choose the Printify images for this listing. Click any photo to view it larger. Changes are saved automatically.</p><div className="image-pref-actions"><button type="button" className={`clear ${action==="clear"?"confirmed":""}`} disabled={!chosen.length} onClick={deselect}>{action==="clear"&&<span className="action-check">✓</span>}<b>{action==="clear"?"Selections cleared":"Clear this listing’s selections"}</b><small>Remove every selected Printify photo from this listing only.</small></button><button type="button" className={action==="all"?"confirmed":""} disabled={!chosen.length} onClick={applyAll}>{action==="all"&&<span className="action-check">✓</span>}<b>{action==="all"?"Applied to every listing":"Apply these photos to every listing"}</b><small>Choose the same Printify photos across the entire batch.</small></button>{onSaveRecipe&&<button type="button" aria-busy={savingFuture} disabled={!chosen.length||savingFuture} className={action==="future"?"confirmed":""} onClick={()=>void saveFuture()}>{action==="future"&&<span className="action-check">✓</span>}<b>{savingFuture?"Saving…":action==="future"?"Saved for future batches":"Use these as this product’s default"}</b><small>Preselect these photos whenever you use this saved product again.</small></button>}</div>{feedback&&<p className="image-pref-feedback" role="status">{feedback}</p>}<div>{images.map((src,index)=><div className={`printify-image-option ${selected.has(index)?"selected":""}`} key={src}><label><input type="checkbox" checked={selected.has(index)} onChange={()=>toggle(index)}/><span className="sr-only">Select Printify photo {index+1}</span></label><button type="button" className="printify-photo-expand" onClick={()=>setExpanded(src)} aria-label={`View Printify photo ${index+1} larger`}><img src={src} alt={`Printify product mockup ${index+1}`}/></button></div>)}</div></details>{lightbox}</>;
}
...
function ProductColorSelector({product,selected,onChange,onRemember,remembering,remembered}:{product:TemplateDetails;selected:number[];onChange:(ids:number[])=>void;onRemember:()=>void;remembering:boolean;remembered:boolean}){
  const colors=product.colorOptions||[],available=colors.filter(color=>color.available),selectedSet=new Set(selected);
  ...
  return <section className="product-color-selector" aria-label={`Choose colors for ${product.blueprintTitle}`}><div className="color-selector-head"><div><p className="mini-label">COLORS FOR THIS BATCH</p><h3>Choose the colors you want to offer</h3><span>Goldie starts with the colors from your Printify template. These choices affect this batch only and do not change the template.</span></div><b>{selected.length} of {available.length} selected</b></div><div className="color-choice-grid">{colors.map(color=><button type="button" key={color.id} disabled={!color.available} aria-pressed={selectedSet.has(color.id)} onClick={()=>toggle(color.id)} className={selectedSet.has(color.id)?"selected":""}><i style={{background:color.swatch||"linear-gradient(135deg,#f8e7ef,#caa4d8)"}}/><span>{color.title}</span>{selectedSet.has(color.id)&&<em>✓</em>}{!color.available&&<small>Unavailable</small>}</button>)}</div><div className="color-selector-actions"><button type="button" onClick={()=>onChange(available.map(color=>color.id))}>Select all available</button><button type="button" onClick={()=>onChange([])}>Clear all</button><button type="button" className={remembered?"remembered":""} disabled={!selected.length||remembering} onClick={onRemember}>{remembering?"Saving…":remembered?"✓ Saved for future batches":"Remember these colors for future batches"}</button></div>{!selected.length&&<p className="color-required" role="alert">Choose at least one available color before continuing.</p>}</section>
}
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      <div className="variant-pricing-head">
        <div><h3>Review item prices and shipping</h3></div>
...
export default function Home() {
  ...
  const [bundleColorChoices,setBundleColorChoices]=useState<Record<string,number[]>>({});
  ...
  const planDraftsRemaining=sidebarUsage?Math.max(0,sidebarUsage.limit-sidebarUsage.used):null,batchDesignLimit=Math.min(MAX_BATCH_FILES,planDraftsRemaining??MAX_BATCH_FILES),additionalDesignsAvailable=Math.max(0,batchDesignLimit-files.length);
  const pricedVariants=useMemo(()=>{const variants=templateDetails?.variants||[];if(!templateDetails?.colorOptions?.length)return variants;const selected=new Set(selectedColorIds);return variants.filter(variant=>variant.colorId==null||selected.has(variant.colorId))},[templateDetails,selectedColorIds]);
  ...
  function createdListingsMissingImages(){return drafts.filter(draft=>draft.status==="Created"&&draft.id&&!(printifyImageSelections[draft.id]??printifyImageIndices).length&&!(preparedMockupCounts[draft.id]||0))}
  function allCreatedListingsHaveImages(){const created=drafts.filter(draft=>draft.status==="Created"&&draft.id);return created.length>0&&createdListingsMissingImages().length===0}
  ...
  useEffect(()=>{void Promise.all([
    fetch("/api/batches").then(response=>response.ok?response.json():{batches:[]}),
    fetch("/api/product-recipes").then(response=>response.ok?response.json():{recipes:[]}),
    fetch("/api/keyword-lists").then(response=>response.ok?response.json():{lists:[]}),
    fetch("/api/mockups/library").then(response=>response.ok?response.json():{templates:[]}),
    fetch("/api/usage").then(response=>response.ok?response.json():{}),
  ]).then(([batches,recipes,keywords,mockups,usage])=>setCommandCenterData({batches:batches.batches||[],recipes:recipes.recipes||[],keywords:keywords.lists||[],mockups:mockups.templates||[],draftsThisMonth:Number(usage.usage?.drafts)||0})).catch(()=>setCommandCenterData(null))},[]);
  ...
  function removeDesign(id:string){const removed=files.find(file=>file.id===id);if(!removed)return;const next=files.filter(file=>file.id!==id);URL.revokeObjectURL(removed.previewUrl);setFiles(next);setFileError("");setFileNotice(`${removed.name} was removed.`);setComplete(false);setDrafts([]);setProcessed(0);const batchId=batchIdRef.current;if(batchId){if(next.length)void saveBatchFiles(batchId,next.map(file=>file.file)).catch(()=>undefined);else void clearBatchFiles(batchId)}}
  ...
  async function loadTemplateUrl(productUrl = template, pricingOverride?:Pricing, savedShippingProfileId=0,rememberedColorIds:number[]=[]):Promise<TemplateDetails|null> {
    ...
    try {
      ...
      const available=new Set((result.product.colorOptions||[]).filter(color=>color.available).map(color=>color.id));let sessionColors:number[]=[];try{sessionColors=JSON.parse(window.localStorage.getItem(`goldie-colors-${result.product.id}`)||"[]") as number[]}catch{/* Ignore an invalid browser preference. */}const remembered=rememberedColorIds.filter(id=>available.has(id));const session=sessionColors.filter(id=>available.has(id));const defaults=remembered.length?remembered:session.length?session:(result.product.colorOptions||[]).filter(color=>color.available&&color.templateEnabled).map(color=>color.id);setSelectedColorIds(defaults.length?defaults:[...available]);setColorsRemembered(Boolean(remembered.length));setTemplateDetails(result.product);setDescription(result.product.description||"");if(result.product.standardShipping!=null)setPricing(current=>({...current,shippingCost:result.product!.standardShipping!,shippingCharged:0}));setVariantPrices(Object.fromEntries((result.product.variants||[]).map(variant=>[String(variant.id),variant.templatePrice])));setPricingApproved(false); return result.product;
  ...
  function createDrafts() {const issues=requiredForStep("review");if(issues.length)return void stopWith("This batch isn’t ready to create.",issues);if(planDraftsRemaining!==null&&files.length>planDraftsRemaining)return void stopWith("This batch is larger than your remaining plan allowance.",[`${planDraftsRemaining} ${planDraftsRemaining===1?"listing remains":"listings remain"} this month, but this batch contains ${files.length} designs. Remove ${files.length-planDraftsRemaining} before creating drafts.`]);if(!etsyShippingProfileId)return void stopWith("Choose shipping before creating drafts.",["Choose the Etsy shipping profile Goldie should apply to every listing."]);if(!pricingApproved)return void stopWith("Finish shipping first.",["Choose a shipping profile, then save or discard any custom shipping profile changes."]);setPreflightOpen(true);}
    ...
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"} style={{"--active-product":`"${currentProductName.replace(/["\\]/g,"")}"`} as React.CSSProperties}>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="finish"&&finishPhase==="mockups"?"mockup-workspace":""}`}>
        <nav className="workflow-progress" aria-label="Listing Factory progress">
          ...
          {RAIL_TOP.map(index=>{const label=PROGRESS_STEPS[index],active=progressIndex===index||(index===RAIL_PRICING&&progressIndex===RAIL_DRAFTS),done=index<progressIndex&&!(index===RAIL_PRICING&&progressIndex===RAIL_DRAFTS),available=railAvailable(index),draftLine=index===RAIL_PRICING&&complete?` · ${createdDraftCount} drafts created`:"";return <button key={label} className={`${active?"active":""} ${done?"done":""}`} disabled={!available} aria-current={active?"step":undefined} onClick={()=>openProgressStep(index)}><em className="progress-bubble-label">{PROGRESS_SHORT_LABELS[index]}</em><span>{done?"✓":String(index+1).padStart(2,"0")}</span><span><b>{label}</b><small>{localPreview&&!active?"Open preview":`${progressStatus(index,active,done)}${draftLine}`}</small></span></button>})}
          <button className={railInFinish?"active":""} disabled={!railAvailable(RAIL_FINISH_FIRST)} aria-current={railInFinish?"step":undefined} onClick={()=>openProgressStep(RAIL_FINISH_FIRST)}><em className="progress-bubble-label">Finish</em><span>{String(RAIL_TOP.length+1).padStart(2,"0")}</span><span><b>Finish listings</b><small>{railInFinish?`${PROGRESS_SHORT_LABELS[progressIndex]} · ${progressIndex-RAIL_FINISH_FIRST+1} of ${RAIL_FINISH.length}`:complete?"Titles, photos, publish":"Complete the prior step"}</small></span></button>
          {railInFinish&&<div className="rail-substeps" role="group" aria-label="Finish listings steps">{RAIL_FINISH.map((index,position)=>{const active=progressIndex===index,done=index<progressIndex;return <button key={PROGRESS_STEPS[index]} className={`rail-substep ${active?"active":""} ${done?"done":""}`} disabled={!railAvailable(index)} aria-current={active?"step":undefined} onClick={()=>openProgressStep(index)}><span className="rail-substep-dot">{done?"✓":position+1}</span><span><b>{FINISH_RAIL_LABELS[position]}</b><small>{localPreview&&!active?"Open preview":progressStatus(index,active,done)}</small></span></button>})}</div>}
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          {workflowStep==="finish"&&finishPhase==="final"&&<div className="step-success-banner" role="status"><span aria-hidden="true">✓</span><div><b>Listing photos complete</b><small>Every listing has at least one photo and is ready for final review.</small></div></div>}
          <article className={`step-card connect-step workflow-panel ${connected ? "done" : ""} ${workflowStep==="connect"?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              <div className="step-heading"><div><div className="heading-with-help"><h2>Connect <em>Printify</em></h2><ContextHelp label="Explain account connections" title="Connect the accounts Goldie needs" intro="Goldie needs both connections before a batch can move forward. Each connection has a different job, and disconnecting either one stops new batches until it is restored." sections={[{heading:"Printify creates the products",copy:"Your Printify token lets Goldie read your shops and saved template products, upload finished artwork, and create or update product drafts. Goldie does not need access to your Printify orders."},{heading:"Etsy finishes and publishes the listings",copy:"The Etsy connection lets Goldie read the shop connected to your Printify template, load real Etsy categories and shipping profiles, and publish the completed listings after your final confirmation."},{heading:"Use matching accounts",copy:"Connect the Printify account that contains the template product and the Etsy shop where that product was published. If the template belongs to a different shop, Goldie will stop and explain the mismatch."},{heading:"Your publishing safeguard",copy:"Connecting does not publish anything. Goldie first creates unpublished Printify drafts. Listings go live on Etsy only after the final review and a second explicit confirmation."}]}/></div></div></div>
          ...
          {templateDetails&&productSelected&&<ProductColorSelector product={templateDetails} selected={selectedColorIds} onChange={ids=>{setSelectedColorIds(ids);setColorsRemembered(false);setPricingApproved(false)}} onRemember={()=>void rememberProductColors()} remembering={rememberingColors} remembered={colorsRemembered}/>} 
          {activeBundle&&bundleRecipes.length>1&&<section className="bundle-color-selectors"><div><p className="mini-label">OTHER PRODUCTS IN THIS BUNDLE</p><h3>Choose colors for each product</h3><span>Each product keeps its own valid Printify colors and variant combinations.</span></div>{bundleRecipes.slice(1).map(recipe=>{const product=bundleColorProducts[recipe.id];if(!product)return <p key={recipe.id}>Checking colors for {recipe.name}…</p>;const chosen=bundleColorChoices[recipe.id]||[];return <div className="bundle-color-product" key={recipe.id}><b>{recipe.name}</b><ProductColorSelector product={product} selected={chosen} onChange={ids=>{setBundleColorChoices(current=>({...current,[recipe.id]:ids}));window.localStorage.setItem(`goldie-colors-${product.id}`,JSON.stringify(ids))}} onRemember={()=>void fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...recipe,defaultColorIds:chosen})})} remembering={false} remembered={false}/></div>})}</section>}
          ...
          {templateDetails&&productSelected&&<button className="workflow-next" disabled={Boolean(templateDetails.colorOptions?.length&&!selectedColorIds.length)} onClick={()=>goToStep("designs")}>Next step <span>→</span></button>}</div>
          ...
          <article className={`step-card designs-step workflow-panel ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="finish"&&finishPhase==="details")?"active-panel":"hidden-panel"}`}>
            ...
            <div className="step-content">
              <div className="step-heading"><div><p className="mini-label">{workflowStep==="finish"?"TITLES, TAGS + DESCRIPTIONS":"DESIGNS"}</p><div className="heading-with-help"><h2>{workflowStep==="finish"?"Finish titles, tags, and descriptions":"Add your finished designs"}</h2>{workflowStep==="finish"?<ContextHelp label="Explain titles, tags, and descriptions" title="Finish the words shoppers will see" intro="This step uses your validated keyword banks and your saved product description to finish every listing without forcing every listing to be identical." sections={[{heading:"Choose how titles are created",copy:"Goldie can choose the best-fitting exact phrases from one keyword bank for each design, or you can select phrases yourself and apply the same structure across the batch."},{heading:"Always review AI selections",copy:"Goldie never invents keywords, but it also cannot guarantee that the keyword bank you selected fits the artwork. If the bank is mismatched, Goldie will still choose from it. Review every generated title before continuing."},{heading:"Tags follow the title",copy:"Goldie builds matching Etsy tags from the selected title phrases. You can edit the title or tags for any individual listing without changing the others."},{heading:"Use one shared description",copy:"The batch description begins with the description imported from your Printify template. Edit it once for every listing, then open an individual listing only when it needs different wording or an added blurb. Spacing and line breaks are preserved."}]}/>:<ContextHelp label="Explain design uploads" title="Upload the finished designs for this batch" intro="Upload only artwork that is ready to place on the selected product. You can add files in several rounds without replacing files that are already in the batch." sections={[{heading:"What to upload",copy:"Use PNG or JPG artwork. Choose a transparent-background PNG when the background should not print. Do not upload mockups or product photos here."},{heading:"Add more without starting over",copy:"Choose a folder or individual images as many times as needed. New files are appended to the current batch. Goldie skips an exact duplicate file instead of adding it twice."},{heading:"Watch the limits",copy:"A batch can contain up to 20 designs. Each individual file can be up to 100 MB. Large batches are processed carefully without reducing the source resolution."},{heading:"Review the resolution warning",copy:"Goldie compares each file with the selected Printify product’s recommended pixel dimensions. A warning does not automatically mean the design will print badly, but you should inspect the largest enabled product size before proceeding."}]}/>}</div></div>{files.length > 0 && <span className="done-mark">✓ {files.length} {workflowStep==="finish"?"listings":"loaded"}</span>}</div>
              ...
              <p className="batch-limits" aria-label="Batch limits"><span>20 designs maximum</span><i /> <span>100 MB per design · no combined file-size cap</span><i /> <span>Big batches upload one design at a time so nothing loses print quality</span></p>
              ...
              {files.length > 0 && <div className="batch-capacity"><div><b>{files.length}/{batchDesignLimit} designs available this batch</b><span>{planDraftsRemaining===null?"Checking plan allowance…":`${additionalDesignsAvailable} more available · ${planDraftsRemaining} left on your plan`}</span></div><div className="capacity-track"><span style={{ width: `${batchDesignLimit?Math.min(100,(files.length/batchDesignLimit)*100):100}%` }} /></div></div>}
              {files.length>0&&workflowStep==="designs"&&<div className="design-upload-review" aria-label="Review uploaded designs">{files.map(file=><article key={file.id}><img src={file.previewUrl} alt=""/><div><b title={file.name}>{file.name}</b><small>{file.width&&file.height?`${file.width} × ${file.height}px`:"Checking dimensions…"}</small></div><button type="button" onClick={()=>removeDesign(file.id)} aria-label={`Remove ${file.name}`}>Remove</button></article>)}</div>}
...
<div className="final-safety-readiness" aria-label="Final publishing readiness"><span>{files.every(file=>file.etsy)&&files.every(file=>!personalizationProblem(file.etsy))?"✓":"!"} Etsy details and personalization {files.every(file=>file.etsy)&&files.every(file=>!personalizationProblem(file.etsy))?"are ready":"still need review"}</span><span>{allCreatedListingsHaveImages()?"✓":"!"} {allCreatedListingsHaveImages()?"Every listing has at least one photo":"One or more listings still need a photo"}</span></div><FinalListingReview drafts={drafts} files={files} selections={printifyImageSelections} defaultIndices={printifyImageIndices} batchSizeGuide={sizeGuideName} onEdit={setFinishPhase}/><div className="publish-live-warning"><b>These listings will be published live on Etsy.</b><span>They will not go to Etsy drafts. Nothing happens until you confirm again in the next window.</span><small>Etsy charges its standard $0.20 USD listing fee for each listing created. This fee is charged by Etsy and is separate from your Goldie subscription.</small></div><button className="publish-all-button" aria-busy={publishing} disabled={publishing||drafts.some(draft=>draft.status==="Failed")} onClick={openPublishConfirmation}>{publishing?"Publishing…":"Publish all live on Etsy"}</button>{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}</>}</div></article>}
      ...
      {complete && workflowStep==="finish" && finishPhase==="mockups" && <section className="post-draft-workspace">
        ...
        <div className="draft-card-grid">{drafts.map(draft=>{const design=files.find(file=>file.id===draft.clientId),selectedImages=draft.id?(printifyImageSelections[draft.id]??printifyImageIndices):printifyImageIndices;return <article className={`draft-card ${draft.status==="Failed"?"failed":""}`} key={draft.clientId}>
          ...
          {draft.status==="Failed"&&<button className="error-help-link" onClick={()=>window.dispatchEvent(new CustomEvent("goldie-support",{detail:draft.error??"A design failed"}))}>Get help with this error</button>}
        </article>})}</div>
        {imageStepError&&<p className="image-step-blocker" role="alert">{imageStepError}</p>}
        <button className="workflow-next mockup-next" onClick={()=>{const missing=createdListingsMissingImages();if(missing.length){const labelForDraft=(draft:DraftResult)=>{const file=files.find(item=>item.id===draft.clientId);const label=(draft.title||file?.title||draft.name||file?.name||"").trim();return label||`Listing ${Math.max(1,drafts.findIndex(item=>item.clientId===draft.clientId)+1)}`};const names=missing.slice(0,3).map(labelForDraft).join(", ");const more=missing.length>3?` and ${missing.length-3} more`:"";const message=`Add at least one photo to ${missing.length===1?"this listing":"each of these listings"}: ${names}${more}.`;setImageStepError(message);stopWith("Every listing needs a photo.",[message]);return}setImageStepError("");setFinishPhase("final");window.scrollTo({top:0,behavior:"smooth"})}}>Next step <span>→</span></button>
      </section>}

...
export function supportResponse(query: string, turns: SupportTurn[]): SupportResponse {
  ...
  if (/^they never appear on the page$/i.test(lower)) return { text: "Check that the files are fully downloaded to your computer, then choose them again. The Listing Factory accepts PNG, JPG/JPEG and WebP. If the same files still do not appear, use Contact Support with the file type and any upload message." };
  ...
  if (/^(no,? it will not open|one or more will not open)$/i.test(lower)) return { text: "Download or export a fresh copy from the original design file, then confirm it opens on your computer before selecting it in The Listing Factory. Keep the other working files and drafts." };
  ...
  if (/^(that|this|it) (still )?(didn'?t|doesn'?t|did not|does not) work|same (thing|error)|still (failing|broken|not working)/i.test(lower)) {
    ...
    const followUps: Record<string, SupportResponse> = {
      ...
      "template-not-found": { text: "Does the template open normally inside the same Printify account you connected to The Listing Factory, or does Printify itself say product not found?", suggestions: ["It opens normally in Printify", "Printify says product not found"] },
      "template-print-area": { text: "After saving the fresh template copy, what exact red error does The Listing Factory show when you load or run it now? Paste that text here.", suggestions: [] },
      ...
      "rate-limit": { text: "Is The Listing Factory still processing the batch, or did the batch finish with specific designs marked Failed?", suggestions: ["Still processing", "Finished with failed designs"] },
      "editor-login": { text: "After signing in, is Printify using the same account whose token you connected to The Listing Factory? Check the account email in Printify before answering.", suggestions: ["Yes, same Printify account", "No, different account"] },
  ...
  if (/same size error/i.test(lower)) return { text: "That one design is still above Printify’s accepted processed size after The Listing Factory prepared it. Export only that file again as a transparent PNG if transparency is needed, otherwise as a high-quality JPG, then run it in a new one-file batch. Leave the successful drafts alone.", articleId: "image-size" };
  if (/it opens normally in printify/i.test(lower)) return { text: "Then the product exists, but The Listing Factory’s saved token is connected to a different Printify account or shop. In The Listing Factory, disconnect Printify, reconnect using an all-scopes token created inside the account where that template opens, and load the same template link again.", articleId: "template-not-found" };
  if (/printify says product not found/i.test(lower)) return { text: "That template itself is no longer available. In Printify → My Products, create or duplicate a fresh unpublished product, save its provider, variants and placement, then paste the fresh product editor link into The Listing Factory.", articleId: "template-not-found" };
  ...
  if (/no,? different account/i.test(lower)) return { text: "Sign out of Printify in that browser, sign into the account connected to The Listing Factory, then return to the Listing Factory and click Edit in Printify again. The draft lives in the account attached to the token.", articleId: "editor-login" };
  if (/blocked pop-up icon/i.test(lower)) return { text: "Click that icon in the browser address bar, choose Always allow pop-ups and redirects from The Listing Factory, then click Done and use Open all in Printify again.", articleId: "popups" };
  ...
  if (/an older token/i.test(lower)) return { text: "Create a fresh personal access token in Printify → Account → Connections with every scope enabled. Copy it when it appears, then disconnect the old Printify connection in The Listing Factory and connect the fresh token.", articleId:"token-connect" };
  ...
  if (templateIssue) return { text: "Let’s narrow down what’s happening with the template. After you click Load template, does The Listing Factory say the product wasn’t found, show a different red message, or keep loading without finishing?", suggestions: ["Product was not found", "A different red message", "It keeps loading"] };
  ...
  if (uploadIssue && !/design(s)? failed|draft creation/i.test(lower)) return { text: "Let’s find the point where the image upload stops. Do the files never appear after you choose them, does The Listing Factory reject the batch immediately, or do they load successfully and fail later while creating drafts?", suggestions: ["Files never appear", "Batch is rejected immediately", "They fail during draft creation"] };
  ...
  if (editorIssue) return { text: "Let’s check what happens when The Listing Factory hands the draft to Printify. Do you see a login screen, product not found, the wrong account, or no new tab at all?", suggestions: ["Printify login screen", "Product not found", "Wrong Printify account", "No tab opens"] };
  ...
  if (/printify login screen/i.test(lower)) return { text: "Sign into the same Printify account whose token is connected to The Listing Factory, then return and click that draft’s Edit in Printify button again. If it then says product not found, tell me that and we’ll check the account match.", articleId:"editor-login" };
  if (/wrong printify account/i.test(lower)) return { text: "Sign out of Printify in that browser and sign into the account connected to The Listing Factory. Then return to The Listing Factory and click Edit in Printify again; the draft was created inside the token owner’s account.", articleId:"editor-login" };

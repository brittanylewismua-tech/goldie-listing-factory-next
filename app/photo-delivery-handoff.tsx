...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>;onReview?:(id:string,photos:boolean)=>void;onStatusReady?:(ready:boolean)=>void}>(({targets,beforePrepare,onReview,onStatusReady},ref)=>{
 ...
 return <section className="photo-delivery-handoff" ref={panel} aria-label="Finish Etsy drafts" aria-busy={busy}>
  ...
  {!statusKnown?<p>Check saved progress before creating or updating Etsy drafts.</p>:completed===targets.length&&targets.length>0?<p>Your drafts are ready in Etsy. Open each draft below to review it and publish when you are ready.</p>:<div className="draft-handoff-intro"><p>Choose <strong>Save to Etsy Drafts</strong> to copy this batch into Etsy without publishing it.</p><p>When a result says <strong>Etsy draft verified</strong>, open it here or find it in Etsy Shop Manager → Listings → Drafts.</p></div>}
  <p className="photo-delivery-note">No download or second upload. The Listing Factory never publishes or renews a listing. Etsy charges its listing fee only when you publish in Etsy.</p>

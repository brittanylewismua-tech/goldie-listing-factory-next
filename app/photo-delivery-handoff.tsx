...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>;onReview?:(id:string,photos:boolean)=>void;onStatusReady?:(ready:boolean)=>void}>(({targets,beforePrepare,onReview,onStatusReady},ref)=>{
 ...
 const showResults=deliveries.length>0||Object.keys(preparationErrors).length>0||Boolean(error);
 return <section className={`photo-delivery-handoff ${showResults||!statusKnown||busy?'':'is-empty'}`} ref={panel} aria-label="Finish Etsy drafts" aria-busy={busy}>
  ...
  {showResults&&<details className={`draft-handoff-details ${problems.length||error?'has-problems':''}`} open={Boolean(problems.length||error)}><summary><span>{problems.length?'Etsy drafts need attention':activeDeliveries.length?'Creating Etsy drafts':completed===targets.length?'Etsy drafts ready':'Etsy draft progress'}</span><b>{completed} of {targets.length}</b></summary>
  ...
  {error&&<div className="photo-delivery-error" role="alert"><p>{error}</p>{/sign in/i.test(error)&&<a href="/account/sign-in?return_to=%2Flisting-factory" target="_blank" rel="noopener noreferrer">Sign in to Listing Factory ↗</a>}<p>Completed work is saved. Check progress before trying again.</p></div>}
 </section>;
});

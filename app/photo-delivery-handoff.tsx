...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>;onReview?:(id:string,photos:boolean)=>void}>(({targets,beforePrepare,onReview},ref)=>{
 ...
 async function refresh(){if(locked.current)return;
  ...
  },next=>{if(queryKey.current===expectedKey){setDeliveries(next);polling.current=next.some(d=>['preparing','waiting','delivering'].includes(d.status));setLoading(false);setError('')}});
 }
 ...
 async function prepareOne(target:Target,recheckId?:string){if(locked.current)return;setPreparationErrors(current=>{const next={...current};delete next[target.id];return next});locked.current=true;statusReads.current.invalidate();setBusy(true);setError('');try{await beforePrepare();const response=await fetch('/api/listing-photos/delivery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({productId:target.id,printifyImageIndices:target.indices,shippingProfileId:target.shippingProfileId,mode:'draft',automaticDraft:true,recheckId})});const payload=await response.json() as {error?:string;delivery?:Delivery};if(!response.ok||!payload.delivery)throw Error(payload.error||'Draft preparation failed.');polling.current=true;pollSchedule.current={started:Date.now(),next:0,failures:0};setLoading(false);setDeliveries(current=>[...current.filter(d=>d.productId!==target.id),payload.delivery!])}catch(e){const message=e instanceof Error?e.message:'Draft preparation failed.';recoveryFocus.current=target.id;setPreparationErrors(current=>({...current,[target.id]:message}));setError('This listing needs attention. Review its message below.')}finally{locked.current=false;setBusy(false)}}

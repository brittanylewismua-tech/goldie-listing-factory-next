...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>;onReview?:(id:string,photos:boolean)=>void}>(({targets,beforePrepare,onReview},ref)=>{
 ...
 useImperativeHandle(ref,()=>({prepare:async()=>{
  ...
  try{
   ...
   const errors=await prepareDraftBatch(targets,async target=>{
    ...
    }catch(value){const message=value instanceof Error?value.message:'Draft preparation failed.';setPreparationErrors(current=>({...current,[target.id]:message}));throw value}
 ...
 async function prepareOne(target:Target,recheckId?:string){if(locked.current)return;setPreparationErrors(current=>{const next={...current};delete next[target.id];return next});locked.current=true;setBusy(true);setError('');try{await beforePrepare();const response=await fetch('/api/listing-photos/delivery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({productId:target.id,printifyImageIndices:target.indices,shippingProfileId:target.shippingProfileId,mode:'draft',automaticDraft:true,recheckId})});const payload=await response.json() as {error?:string;delivery?:Delivery};if(!response.ok||!payload.delivery)throw Error(payload.error||'Draft preparation failed.');await refresh()}catch(e){const message=e instanceof Error?e.message:'Draft preparation failed.';setPreparationErrors(current=>({...current,[target.id]:message}));setError('This listing needs attention. Review its message below.')}finally{locked.current=false;setBusy(false)}}

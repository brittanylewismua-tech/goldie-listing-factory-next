...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>}>(({targets,beforePrepare},ref)=>{
 ...
 return <section className="photo-delivery-handoff" ref={panel} aria-label="Automatic Etsy photos" aria-busy={busy}>
  ...
  <div role="status" aria-live="polite">{busy?'Saving your photo sets. Keep this page open until Printify opens.':loading?'Checking saved photo delivery…':deliveries.length?deliveries.some(d=>['waiting','delivering'].includes(d.status))?'Delivery continues even when this page is closed.':completed===targets.length?'Your photo deliveries are complete.':'Review the delivery status below before leaving.':'Photo delivery will be prepared when you open My Products.'}</div>
  ...
  <details><summary>How delivery works</summary><small>Includes your selected mockups, custom photos and size guide. Printify’s photos may appear briefly first. Publication checks continue for up to 24 hours, every 30 seconds at first, then less often. WebP and PNG files are prepared as Etsy-ready JPGs; transparent areas use a white background. Downloads remain optional. Later syncing photos from Printify can replace your Etsy edits.</small></details>

...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>}>(({targets,beforePrepare},ref)=>{
 ...
 const polling=useRef(true),locked=useRef(false),queryKey=useRef(""),panel=useRef<HTMLElement>(null),targetKey=JSON.stringify(targets.map(t=>[t.id,t.indices,t.shippingProfileId]));
 ...
 useImperativeHandle(ref,()=>({prepare:async()=>{
  ...
  try{
   ...
   for(const target of targets){
    const response=await fetch('/api/listing-photos/delivery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({productId:target.id,printifyImageIndices:target.indices,shippingProfileId:target.shippingProfileId,mode:'draft'})});
 ...
 async function prepareOne(target:Target,recheckId?:string){if(locked.current)return;locked.current=true;setBusy(true);setError('');try{await beforePrepare();const response=await fetch('/api/listing-photos/delivery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({productId:target.id,printifyImageIndices:target.indices,shippingProfileId:target.shippingProfileId,mode:'draft',recheckId})});const payload=await response.json() as {error?:string;delivery?:Delivery};if(!response.ok||!payload.delivery)throw Error(payload.error||'Draft preparation failed.');await refresh()}catch(e){setError(e instanceof Error?e.message:'Draft preparation failed.')}finally{locked.current=false;setBusy(false)}}
 ...
 return <section className="photo-delivery-handoff" ref={panel} aria-label="Finish Etsy drafts" aria-busy={busy}>
  ...
  <ol className="draft-handoff-steps">
   ...
   <li><strong>Send a hidden draft from Printify</strong><p>For a new Etsy draft, open the product and check <b>Hide in store</b> under Publishing settings, then click Printify’s <b>Publish</b>. With Hide in store checked, it goes to Etsy Drafts. Already-linked drafts finish without another transfer.</p></li>
  ...
  <ul className="draft-handoff-results">{targets.map(target=>{const item=deliveries.find(d=>d.productId===target.id);return <li key={target.id}><strong>{target.title}</strong><span>{item?.choiceCheckUnavailable?'Current choices not verified':item?.choicesChanged&&item.status==='completed'?'Changes not sent':item?label(item.status,item.mode==='draft'):'Not prepared'}</span>{item?.choicesChanged&&<small>{item.choiceCheckUnavailable?'Current saved choices could not be checked. Reload this page before publishing.':item.status==='completed'?'Your saved choices changed. Prepare this listing again before publishing.':'This delivery uses your earlier choices. Prepare the updated choices after it finishes, or cancel while waiting.'}</small>}{item?.error&&<small role="alert">{item.error}</small>}{(!item||['completed','failed','expired','canceled','needs_attention'].includes(item.status))&&<button type="button" className="draft-prepare" disabled={busy||loading} onClick={()=>void prepareOne(target,item?.mode==='draft'&&item.status==='needs_attention'?item.id:undefined)}>{item?.mode==='draft'&&item.status==='needs_attention'?'Verify last change':item?'Prepare updated choices':'Prepare Etsy draft'}</button>}{item?.mode==='draft'&&item.status==='needs_attention'&&<button type="button" disabled={busy} onClick={()=>void prepareOne(target)}>Prepare corrected choices</button>}{item?.status==='waiting'&&<button type="button" disabled={busy} onClick={()=>void cancel(item.id)}>Cancel waiting</button>}{item?.status==='completed'&&item.listingId&&<a href={item.mode==='draft'?`https://www.etsy.com/your/shops/me/listing-editor/edit/${item.listingId}`:`https://www.etsy.com/listing/${item.listingId}`} target="_blank" rel="noopener noreferrer">{item.mode==='draft'?'Open Etsy draft':'View on Etsy'} ↗</a>}</li>})}</ul>

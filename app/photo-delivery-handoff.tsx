...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>}>(({targets,beforePrepare},ref)=>{
 const [deliveries,setDeliveries]=useState<Delivery[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState(''),[loading,setLoading]=useState(true),[prepared,setPrepared]=useState<number|null>(null);
 ...
 useImperativeHandle(ref,()=>({prepare:async()=>{
  if(locked.current)return false;locked.current=true;setBusy(true);setError('');setPrepared(null);
  try{
 ...
 async function prepareOne(target:Target,recheckId?:string){if(locked.current)return;locked.current=true;setBusy(true);setError('');try{await beforePrepare();const response=await fetch('/api/listing-photos/delivery',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({productId:target.id,printifyImageIndices:target.indices,shippingProfileId:target.shippingProfileId,mode:'draft',automaticDraft:true,recheckId})});const payload=await response.json() as {error?:string;delivery?:Delivery};if(!response.ok||!payload.delivery)throw Error(payload.error||'Draft preparation failed.');await refresh()}catch(e){setError(e instanceof Error?e.message:'Draft preparation failed.')}finally{locked.current=false;setBusy(false)}}

...
const PhotoDeliveryHandoff=forwardRef<PhotoDeliveryHandle,{targets:Target[];beforePrepare:()=>Promise<unknown>;onReview?:(id:string,photos:boolean)=>void}>(({targets,beforePrepare,onReview},ref)=>{
 ...
 async function refresh(){if(locked.current)return;
  ...
  await statusReads.current.run(expectedKey,async signal=>{
   ...
   if(response.status===401){polling.current=false;throw Error('Sign in to Goldie in another tab, then choose Check saved progress here. Your saved drafts have not been changed.')}
   const payload=await response.json() as {error?:string;deliveries?:Delivery[]};
   if(!response.ok)throw Error(payload.error||'Photo delivery status could not be loaded.');return payload.deliveries||[];
  },next=>{if(queryKey.current===expectedKey){setDeliveries(next);const resolved=next.filter(item=>uncertainProducts.current.has(item.productId)&&resolvedDeliveryUncertainty(item));if(resolved.length){for(const item of resolved)uncertainProducts.current.delete(item.productId);setPreparationErrors(current=>{const remaining={...current};for(const item of resolved)delete remaining[item.productId];return remaining})}polling.current=next.some(d=>['preparing','waiting','delivering'].includes(d.status));setLoading(false);setError('')}});
 }
 ...
 return <section className="photo-delivery-handoff" ref={panel} aria-label="Finish Etsy drafts" aria-busy={busy}>
  ...
  {completed===targets.length&&targets.length>0?<p>Your drafts are ready in Etsy. Open each draft below to review it and publish when you are ready.</p>:<ol className="draft-handoff-steps">
  ...
  <ul className="draft-handoff-results">{targets.map(target=>{const item=deliveries.find(d=>d.productId===target.id),problem=preparationErrors[target.id];return <li key={target.id}><strong>{target.title}</strong><span>{problem?'Preparation needs attention':item?.choiceCheckUnavailable?'Current choices not verified':item?.choicesChanged&&item.status==='completed'?'Changes not sent':item?label(item.status,item.mode==='draft'):'Not prepared'}</span>{item?.choicesChanged&&<small>{deliveryChoiceRecovery(item)}</small>}{item?.error&&<small role="alert">{item.error}</small>}{problem&&<small role="alert">{problem}</small>}{problem&&onReview&&<button type="button" disabled={busy} data-recovery-product={target.id} onClick={()=>onReview(target.id,/photo|image|mockup/i.test(problem))}>{/photo|image|mockup/i.test(problem)?'Review photos':'Review listing'}</button>}{(!item||['completed','failed','expired','canceled','needs_attention'].includes(item.status))&&<button type="button" className={item?.status==='completed'&&!item.choicesChanged?'draft-refresh':'draft-prepare'} disabled={busy||loading} onClick={()=>void prepareOne(target,item?.mode==='draft'&&item.status==='needs_attention'?item.id:undefined)}>{item?.mode==='draft'&&item.status==='needs_attention'?'Verify last change':item?'Update Etsy draft':'Create Etsy draft'}</button>}{item?.mode==='draft'&&item.status==='needs_attention'&&<button type="button" disabled={busy} onClick={()=>void prepareOne(target)}>Prepare corrected choices</button>}{item?.status==='waiting'&&<button type="button" disabled={busy} onClick={()=>void cancel(item.id)}>Cancel waiting</button>}{item?.status==='completed'&&item.listingId&&<a className="draft-open" href={item.mode==='draft'?`https://www.etsy.com/your/shops/me/listing-editor/edit/${item.listingId}`:`https://www.etsy.com/listing/${item.listingId}`} target="_blank" rel="noopener noreferrer">{item.mode==='draft'?'Open Etsy draft':'View on Etsy'} ↗</a>}</li>})}</ul>

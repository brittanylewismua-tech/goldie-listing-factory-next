...
let previewFace:{left:number;top:number;right:number;bottom:number}|undefined;
        ...
        void unmeasured;
        const completed=new Map<number,Result>(),jobs=measured.map((template,index)=>({template,index}));setRenderStatus(`Creating ${measured.length} ${measured.length===1?"mockup":"mockups"} in a reliable queue. The Listing Factory will retry an interrupted scene automatically.`);await runBounded(jobs,2,async({template,index})=>{/* D447 - one scene, and it always produces a mockup.
   ...
   better result, and falls back rather than losing the scene. */
        ...
        const result=await withRecovery(async()=>{
          ...
          return drawLocally();
        });return{index,result}},({index,result})=>{completed.set(index,result);const ready=[...completed.entries()].sort((a,b)=>a[0]-b[0]).map(([,item])=>item);setResults(ready);setRenderStatus(`${ready.length} of ${measured.length} finished. ${ready.length<measured.length?"The Listing Factory is creating the remaining scenes.":"Saving all finished mockups to this listing…"}`)});const made=[...completed.entries()].sort((a,b)=>a[0]-b[0]).map(([,item])=>item);if(made.length!==measured.length){const lost=measured.filter((_,index)=>!completed.has(index)).map(template=>template.name);throw new Error(`The Listing Factory could not finish ${lost.length===1?"this scene":"these scenes"}: ${lost.join(", ")}. Nothing was saved to this listing.`);}await stageForEtsy(made);const warning=made.find(item=>item.warning)?.warning;if(warning)setEtsyStatus(`✓ Mockups saved. ${warning}`)}catch(e){setError(e instanceof Error?e.message:"Mockups could not be created.")}finally{setBusy(false);setRenderStatus("")}}
 ...
 return <><div className="integrated-mockups" aria-busy={busy} aria-label="Rendering your listing mockups"><div className="mockup-limit-note"><b>Lifestyle mockups for this listing</b><span>The Listing Factory processes them in a reliable queue.</span></div>{/* D566 - her words: "it says mockups, and then you pick your mockup set. And
      ...
      {/* D618 - the scenes are chosen ONCE, for the batch, in the panel above.
          This grid repeated the identical five photographs inside every listing,
          with its own separate checkboxes - so a two-listing batch asked the same
          question three times and showed three different answers. She picked
          scenes at the top, then found them unpicked again under each product.

          The listing keeps its own Create button and its own results; what it no
          longer does is ask again which scenes to use. */}
      <div className="mockup-control-row"><span className="mockup-set-name">{theme==="__all"?"All mockups":theme||"No mockup set chosen"}</span><span className="mockup-chosen-count">{chosen.length} {chosen.length===1?"scene":"scenes"} chosen for this batch</span><a href="/mockups" target="_blank" rel="noopener noreferrer">Manage saved mockup sets ↗</a></div>{needsReference&&<p className="automatic-reference">✓ The Listing Factory will use the real Printify preview above as the placement reference.</p>}<div className="mockup-action-sequence"><div className="mockup-primary-action"><span>1</span><div><b>Create mockups for this listing</b><small>The Listing Factory creates the mockups you selected above and saves them to this listing.</small></div><button className="generate-inline" aria-busy={busy} disabled={!chosen.length||busy||needsReference&&!referenceUrl} onClick={()=>void generate()}>{busy?"The Listing Factory is creating them…":`Create ${chosen.length ? `these ${chosen.length} ${chosen.length===1?"mockup":"mockups"}` : "selected mockups"}`}</button></div>{busy&&renderStatus&&<div className="mockup-live-progress" role="status" aria-live="polite"><i/><span>{renderStatus}</span></div>}</div>{error&&<p className="field-error" role="alert">{error}</p>}{etsyStatus&&<p className="etsy-ready-status" role="status">{etsyStatus}</p>}{results.length>0&&<div className="inline-generated">{results.map((r,resultIndex)=><figure key={r.name}><button className="mockup-enlarge" onClick={()=>setExpanded(r)}><img src={r.url} alt={r.template}/><span>View larger</span></button><figcaption><span>{r.template}</span><span className={(r.adjusted||adjustedScenes[r.templateId])?"sceneState adjusted":"sceneState"}>{(r.adjusted||adjustedScenes[r.templateId])?"Adjusted":"Ready"}</span></figcaption>
      ...
      </div></figure>)}</div>}<p className="etsy-note">The Listing Factory saves these mockups for this exact listing and adds them automatically through Etsy when you publish. Individual downloads stay available as a backup.</p></div>{lightbox}
    ...
    {editing&&designUrl&&(()=>{
      ...
      const surface=(template.preparation?.corners||template.corners) as Quad;
      const mode=renderingModeFor(productName,template.preparation?.geometry);
      /* The Listing Factory's automatic answer: the scene's surface, with THIS design placed
      ...
      const finish=async(next:PlacementTransform,exported:Blob,improveScene:boolean,advance:boolean)=>{
        ...
        if(!written.ok)throw new Error("The Listing Factory could not save this placement.");

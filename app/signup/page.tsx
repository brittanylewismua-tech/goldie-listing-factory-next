...
export default async function SignupPage({searchParams}:{searchParams:Promise<{checkout?:string;offer?:string}>}){const[user,query]=await Promise.all([getChatGPTUser(),searchParams]),offer=query.offer==="trial"||query.offer==="goldie"||query.offer==="pro"||query.offer==="scale"?query.offer:undefined;return <SignupClient signedIn={Boolean(user)} signedInEmail={user?.email} checkout={query.checkout} initialOffer={offer} returnTo="/listing-factory"/>}

...
export default function SignupClient({ signedIn, checkout, returnTo = "/listing-factory", initialOffer }: { signedIn: boolean; checkout?: string; returnTo?: string; initialOffer?: OfferKey }) {
  ...
  const offers: Array<{ key: OfferKey; name: string; description: string; price: string; cadence: string; featured?: boolean; benefits: React.ReactNode; button: string }> = [
    { key: "trial", name: "Free Trial", description: "Build your first batch before you commit.", price: "$0", cadence: "for 3 days", benefits: <><li><b>10</b> listing creations</li><li><b>6</b> AI lifestyle mockups</li><li><b>2</b> saved mockup sets</li><li>Then <b>$29/month</b>. Cancel anytime.</li></>, button: "Start free trial" },
    ...((Object.keys(PLANS) as PlanKey[]).map((key) => { const plan = PLANS[key]; const descriptions:Record<PlanKey,string>={goldie:"For sellers building a consistent listing rhythm.",pro:"For active shops testing and launching at volume.",scale:"For high-output shops and growing teams."}; return { key, name: plan.name, description: descriptions[key], price: `$${plan.price}`, cadence: "/month", featured: key === "pro", benefits: <><li><b>{plan.drafts}</b> listing creations each month</li><li><b>{plan.aiMockups}</b> AI lifestyle mockups each month</li><li><b>{plan.mockupSets}</b> saved mockup sets</li><li>Up to <b>{plan.mockupsPerSet}</b> mockups in each set</li></>, button: `Choose ${plan.name}` }; })),
  ];
  ...
  return <main className="signup-page">
    ...
    <section className="signup-credit-explainer"><h2>What counts as a listing creation?</h2><p>One unique Etsy listing successfully created by Goldie uses one listing creation. Failed attempts, retries, edits, and duplicate requests do not use another credit. A product bundle uses one creation for each distinct listing it generates, so a T-shirt, sweatshirt, and hoodie bundle uses three.</p><small>Etsy fees are separate from your Goldie subscription. Etsy currently charges its standard $0.20 USD listing fee for each listing created.</small></section>

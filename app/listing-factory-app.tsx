          ...
          <article className={`step-card designs-step workflow-panel ${workflowStep==="setup"?"batch-design-drop":""} ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="setup"&&Boolean(templateDetails)&&productSelected&&!failedBundleNames().length)?"active-panel":"hidden-panel"}`}>{/* D238 · Choosing the mockup SET lived on Product while the mockups it controls are generated here on Images. Same setting, two pages — the exact split that caused the keyword-bank and shipping duplication. */}
            ...
            <div className="step-content">
              ...
              {files.length>0&&(workflowStep==="setup"||workflowStep==="designs")&&<div className="design-upload-review" aria-label="Review uploaded designs">{files.map(file=>{const colors=(templateDetails?.colorOptions||[]).filter(color=>selectedColorIds.includes(color.id)),positions=templateDetails?.printPositions||[],frontReady=positions.some(position=>/front|chest/i.test(position)),backReady=frontReady&&positions.some(position=>/back/i.test(position)),frontVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side==="front"),backVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side==="back");return <article className="design-artwork-card" key={file.id}>
                ...
                {((colors.length>1&&frontReady)||backReady)&&<div className="artwork-version-tools"><b>Artwork and print locations</b><small>Assign the artwork each garment color should receive, then add a back print only where you want one.</small><div><button type="button" className="text-action" onClick={()=>document.getElementById("batch-preferences-after-designs")?.scrollIntoView({block:"start"})}>Change product colors</button>{colors.length>1&&frontReady?<label className="secondary-action">＋ Add another front colorway<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"front",event.target.files);event.target.value=""}}/></label>:null}{backReady?<label className="secondary-action">＋ Add a back print<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"back",event.target.files);event.target.value=""}}/></label>:frontReady?<span className="print-area-unavailable">This saved Printify product does not currently expose a back print area.</span>:null}</div></div>}
...
{publishRun&&!publishing?"Queuing every listing in this batch…":publishing?(activeBundle&&bundleRecipes.length>1?(()=>{
     ...
     no longer available once publishing has started. */}{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}{publishFailures.length>0&&<section className="publish-failure-panel" role="alert"><p className="mini-label">NOTHING WAS PUBLISHED</p><h3>{publishFailures.length===1?"1 listing could not be published":`${publishFailures.length} listings could not be published`}</h3><p className="publish-failure-lede">Etsy did not create {publishFailures.length===1?"this listing":"these listings"}, so you have not been charged a listing fee for {publishFailures.length===1?"it":"them"}. Here is exactly what Etsy said:</p><ul className="publish-failure-list">{publishFailures.map(failure=>{const draft=drafts.find(item=>item.id===failure.productId);return <li key={failure.productId}><strong>{draft?.title?.slice(0,60)||draft?.name||"Listing"}</strong><span>{failure.error}</span></li>})}</ul><p className="publish-failure-lede">Goldie has emailed this to you and recorded it. You can press publish again once it is fixed.</p></section>}</div></div></>}</div></article></>)}
        ...
        {stepProductCards(bundleCardStatus("images"),null,!(workflowStep==="designs"),<aside className={`launch-panel workflow-panel ${workflowStep==="designs"?"active-panel":"hidden-panel"}`}>
          ...
          {!complete ? (
            <>
            <button className="launch-button" aria-busy={running||preparingEtsy||Boolean(bundleRun)} disabled={!ready || running||preparingEtsy||Boolean(bundleRun)} onClick={createDrafts}>

      ...
      return <div className={`factory-listing-screen focused-review-section focused-review-${focusedSection}${focusedSection==="title"||focusedSection==="description"?" focused-review-all-listings":""}`}>
        ...
        <div className="factory-listing-grid focused-review-grid">
          <div className={`factory-form-card ${focusedSection==="etsy"?"factory-etsy-details-column":"factory-listing-form"}`}>
            <h3>{focusedSection==="etsy"?"Etsy details and personalization":focusedSection==="description"?"Descriptions for every listing":"Titles and tags for every listing"}</h3>
  ...
  return (
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"} data-product-library={showProductLibrary?"true":"false"}>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="designs"?"mockup-workspace":""}`}>
        ...
        {progressIndex===3&&files.length>0&&<ActionReceipt items={[{value:`${files.length} ${files.length===1?"design":"designs"} checked`,label:"Original artwork resolution preserved"},{value:`${requestedListingCount} private ${requestedListingCount===1?"draft":"drafts"}`,label:"Prices reviewed after finished costs are known"}]}/>}
        ...
        <div className={`steps-column ${workflowStep}-column`} data-restoring={restoringBatch?"true":undefined} inert={restoringBatch}>
          ...
          <article inert={running||Boolean(bundleRun)} className={`step-card designs-step workflow-panel ${workflowStep==="setup"?"batch-design-drop":""} ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${(workflowStep==="designs"&&!complete)||(workflowStep==="setup"&&Boolean(templateDetails)&&productSelected&&!failedBundleNames().length&&!bundleCreationMode)?"active-panel":"hidden-panel"}`}>{/* D238 · Choosing the mockup SET lived on Product while the mockups it controls are generated here on Images. Same setting, two pages — the exact split that caused the keyword-bank and shipping duplication. */}
            ...
            <div className="step-content">
              ...
              {files.length > 0 && designsFinished && <div className="batch-capacity"><b>{activeBundle?`${files.length} design${files.length===1?"":"s"} · ${requestedListingCount} private drafts`:`${files.length} design${files.length===1?"":"s"} added`}</b></div>}

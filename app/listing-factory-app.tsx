...
/* D567 - loading="lazy" on an image with no intrinsic size is a deadlock: the
  ...
  function taskPanel(task:string){
    if(task==="draft-colors"&&templateDetails)return <><DraftColorSelector product={templateDetails} drafts={drafts.filter(draft=>draft.status==="Created")} selected={selectedColorIds} saving={savingDraftVariants} artworkByDraft={Object.fromEntries(files.map(file=>[file.id,file.previewUrl]))} onChange={ids=>void syncDraftVariantChoices(ids,selectedSizeIds)} onArtworkChange={(draft,color,list,reset)=>void updateDraftColorArtwork(draft,color,list,reset)}/>{draftVariantError&&<p className="field-error" role="alert">{draftVariantError}</p>}</>;

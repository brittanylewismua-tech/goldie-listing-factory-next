...
function ProductSizeSelector({product,selected,onChange,onRemember,remembering,remembered}:{product:TemplateDetails;selected:number[];onChange:(ids:number[])=>void;onRemember:()=>void;remembering:boolean;remembered:boolean}){
  ...
  return <section className="product-size-selector" aria-label={`Choose sizes for ${product.blueprintTitle}`}>
    ...
    <div className="size-selector-actions"><button type="button" onClick={()=>onChange(available.map(size=>size.id))}>Select all available</button><button type="button" onClick={()=>{const templateSizes=(product.sizeOptions||[]).filter(size=>size.available&&size.templateEnabled).map(size=>size.id);/* Never leave the seller with nothing selected and a blocked Continue. */onChange(templateSizes.length?templateSizes:available.map(size=>size.id))}}>Match Printify template</button><button type="button" className={remembered?"remembered":""} disabled={!selected.length||remembering||remembered} onClick={onRemember}>{remembering?"Saving…":remembered?"✓ Saved for this product":"Save these as this product’s default sizes"}</button></div>
...
export default function ListingFactoryApp() {
  ...
  async function loadTemplateUrl(productUrl = template, pricingOverride?:Pricing, savedShippingProfileId=0,rememberedColorIds:number[]=[],rememberedSizeIds:number[]=[]):Promise<TemplateDetails|null> {
    ...
    try {
      ...
      const available=new Set((result.product.colorOptions||[]).filter(color=>color.available).map(color=>color.id));let sessionColors:number[]=[];try{sessionColors=JSON.parse(window.localStorage.getItem(`goldie-colors-${result.product.id}`)||"[]") as number[]}catch{/* Ignore an invalid browser preference. */}const remembered=rememberedColorIds.filter(id=>available.has(id));const session=sessionColors.filter(id=>available.has(id));const defaults=remembered.length?remembered:session.length?session:(result.product.colorOptions||[]).filter(color=>color.available&&color.templateEnabled).map(color=>color.id);setSelectedColorIds(defaults.length?defaults:[...available]);setColorsRemembered(Boolean(remembered.length));
      ...
      const sizeDefaults=rememberedSizes.length?rememberedSizes:sessionSizeIds.length?sessionSizeIds:(result.product.sizeOptions||[]).filter(size=>size.available&&size.templateEnabled).map(size=>size.id);
      setSelectedSizeIds(sizeDefaults.length?sizeDefaults:[...sizeAvailable]);setSizesRemembered(Boolean(rememberedSizes.length));
    ...
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"} style={{"--active-product":`"${currentProductName.replace(/["\\]/g,"")}"`} as React.CSSProperties}>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="finish"&&finishPhase==="mockups"?"mockup-workspace":""}`}>
        <nav className="workflow-progress" aria-label="Listing Factory progress">
          ...
          {RAIL_TOP.map(index=>{const label=PROGRESS_STEPS[index],active=progressIndex===index||(index===RAIL_PRICING&&progressIndex===RAIL_DRAFTS),done=index<progressIndex&&!(index===RAIL_PRICING&&progressIndex===RAIL_DRAFTS),issues=progressGateIssues(index),available=!issues.length,draftLine=index===RAIL_PRICING&&complete?` · ${createdDraftCount} drafts created`:"";return <button key={label} className={`${active?"active":""} ${done?"done":""}`} disabled={!available} aria-current={active?"step":undefined} title={issues[0]||undefined} onClick={()=>openProgressStep(index)}><em className="progress-bubble-label">{PROGRESS_SHORT_LABELS[index]}</em><span>{done?"✓":String(index+1).padStart(2,"0")}</span><span><b>{label}</b><small>{issues[0]||`${progressStatus(index,active,done,Boolean(issues.length))}${draftLine}`}</small></span></button>})}
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          {templateDetails&&productSelected&&<div className="saved-product-batch-page"><section className="batch-products" aria-label="Products in this batch">{(activeBundle&&bundleRecipes.length>1?bundleRecipes:(activeRecipe?[activeRecipe]:[])).map((recipe,index)=>{const isActive=!activeBundle||bundleRecipes.length<2||index===bundleIndex;const product=isActive?templateDetails:bundleColorProducts[recipe.id];if(!product)return <p className="bundle-product-loading" key={recipe.id}>Loading {recipe.name}…</p>;const ready=readinessFor(product,recipe);const open=openFacet[recipe.id]??(ready.questions[0]||"");const toggle=(name:string)=>setOpenFacet(current=>({...current,[recipe.id]:current[recipe.id]===name?"":name}));const colorFacet=ready.facets.find(facet=>facet.name==="colors");const sizeFacet=ready.facets.find(facet=>facet.name==="sizes");const shownColors=(isActive?selectedColorIds:bundleColorChoices[recipe.id])||recipe.defaultColorIds||colorFacet?.suggested?.colorIds||[];const shownSizes=(isActive?selectedSizeIds:bundleSizeChoices[recipe.id])||recipe.defaultSizeIds||sizeFacet?.suggested?.sizeIds||[];return <article className={`batch-product-card ${ready.established?"is-ready":"needs-setup"}`} key={recipe.id}><header>{pickProductPhoto(product)?<img className="bundle-product-photo" src={pickProductPhoto(product)} alt="" loading="lazy" decoding="async"/>:<span className="bundle-product-photo placeholder" aria-hidden="true"/>}<span className="bundle-product-id"><b>{recipe.name}</b><small>{product.blueprintTitle}</small></span><span className={`batch-product-state ${ready.established?"":"attention"}`}>{ready.established?"Ready":`${ready.questions.length} to set`}</span></header><div className="batch-product-rows">{[...ready.facets].sort((a,b)=>(a.state==="ask"?0:1)-(b.state==="ask"?0:1)).map(facet=>{const label=({colors:"Colors",sizes:"Sizes",mockups:"Mockups",keywords:"Keywords",shipping:"Shipping",profit:"Profit",etsy:"Etsy details"} as Record<string,string>)[facet.name];const action=({colors:"Pick colors",sizes:"Pick sizes",mockups:"Pick a mockup set",keywords:"Pick a keyword bank",shipping:"Pick a shipping profile",profit:"Set a profit goal",etsy:"Add Etsy details"} as Record<string,string>)[facet.name];const needed=facet.state==="ask";const inCard=["colors","sizes","mockups","keywords"].includes(facet.name);const suggestion=(facet.suggested?.colorIds||facet.suggested?.sizeIds||[]).length;const openThis=()=>{if(inCard){toggle(facet.name);return}const block=document.querySelector<HTMLDetailsElement>(".everything-else");if(block){block.open=true;block.scrollIntoView({block:"start"})}};return <div className={`batch-product-row ${needed?"needed":"settled"} ${open===facet.name?"open":""}`} key={facet.name}><span className="row-mark" aria-hidden="true">{needed?"!":"\u2713"}</span><span className="row-label">{label}</span><span className="row-value">{needed?action:facet.label}{facet.note?<small>{facet.note}</small>:null}</span>{needed&&suggestion>0?<button type="button" className="row-shortcut" onClick={()=>{if(facet.name==="colors"){const ids=facet.suggested?.colorIds||[];if(isActive){setSelectedColorIds(ids);setPricingApproved(false)}else setBundleColorChoices(current=>({...current,[recipe.id]:ids}));void establish(recipe,{defaultColorIds:ids})}else{const ids=facet.suggested?.sizeIds||[];if(isActive){setSelectedSizeIds(ids);setPricingApproved(false)}else setBundleSizeChoices(current=>({...current,[recipe.id]:ids}));void establish(recipe,{defaultSizeIds:ids})}}}>Use Printify&rsquo;s {suggestion} {facet.name==="colors"?(suggestion===1?"color":"colors"):(suggestion===1?"size":"sizes")}</button>:null}<button type="button" className="row-open" onClick={openThis}>{open===facet.name?"Close":needed?"Choose":"Change"}</button></div>;})}</div>{open==="colors"&&<ProductColorSelector product={product} selected={shownColors} onChange={ids=>{if(isActive){setSelectedColorIds(ids);setPricingApproved(false)}else setBundleColorChoices(current=>({...current,[recipe.id]:ids}));void establish(recipe,{defaultColorIds:ids})}} onRemember={()=>{}} remembering={false} remembered={false}/>}{open==="sizes"&&<ProductSizeSelector product={product} selected={shownSizes} onChange={ids=>{if(isActive){setSelectedSizeIds(ids);setPricingApproved(false)}else setBundleSizeChoices(current=>({...current,[recipe.id]:ids}));void establish(recipe,{defaultSizeIds:ids})}} onRemember={()=>{}} remembering={false} remembered={false}/>}{open==="mockups"&&<MockupSetSelector productName={product.blueprintTitle} value={isActive?mockupTheme:(bundleMockupChoices[recipe.id]?.theme??recipe.defaultMockupTheme??"")} selectedIds={isActive?(sharedMockups?.theme===mockupTheme?sharedMockups.ids:[]):(bundleMockupChoices[recipe.id]?.ids??recipe.mockupIds??[])} savedValue={recipe.defaultMockupTheme||""} savedIds={recipe.mockupIds} onChange={(theme,ids=[])=>{if(isActive){const next=theme?{theme,ids}:undefined;setMockupTheme(theme);setSharedMockups(next);window.sessionStorage.setItem("goldie-batch-mockups",JSON.stringify(next||null))}else setBundleMockupChoices(current=>({...current,[recipe.id]:{theme,ids}}));void establish(recipe,{defaultMockupTheme:theme,mockupIds:ids})}} saving={false} onSaveDefault={()=>{}}/>}{open==="keywords"&&<div className="bundle-product-keywords"><KeywordBank compact selectionOnly initialId={isActive?(autoTitleBankId||recipe.keywordListId||""):(bundleKeywordChoices[recipe.id]??recipe.keywordListId??"")} onSelect={list=>{const id=list?.id||"";if(isActive){setAutoTitleBank(list);setAutoTitleBankId(id)}else setBundleKeywordChoices(current=>({...current,[recipe.id]:id}));void establish(recipe,{keywordListId:id})}}/></div>}</article>})}</section><MockupSetSelector value={mockupTheme} savedValue={activeRecipe?.defaultMockupTheme||""} onChange={setMockupTheme} saving={savingProductDefault==="mockups"} onSaveDefault={()=>void saveProductDefaults({defaultMockupTheme:mockupTheme},"mockups")}/><details className="everything-else"><summary><span><b>{`${activeRecipe?.name||"This product"} settings`}</b><small>{[`$${pricing.targetProfit.toFixed(0)} profit`,friendlyShippingProfileTitle(etsyShippingProfiles.find(profile=>profile.id===etsyShippingProfileId)?.title),...(activeRecipe?.keywordListId?["keyword bank saved"]:[]),...(description.trim()?["description from Printify"]:[]),`Etsy details ${Object.keys(activeRecipe?.etsyDefaults||{}).length} saved`].filter(Boolean).join(" · ")}</small>{(!activeRecipe?.keywordListId||!description.trim())&&<b className="setup-todo">{`${[!activeRecipe?.keywordListId&&"keyword bank",!description.trim()&&"description"].filter(Boolean).join(" and ")} still to set`}</b>}</span><em>Edit</em></summary><div className="everything-else-body"><section><div><b>Profit goal</b><span>Shipping stays separate because the customer pays shipping.</span></div><label>$<input type="number" min="0" max="500" step="1" value={pricing.targetProfit} onChange={event=>{setPricing(current=>({...current,targetProfit:Number(event.target.value)||0}));setPricingApproved(false)}}/></label>{pricing.targetProfit!==(Number(activeRecipe?.defaultProfitTarget)||DEFAULT_PRICING.targetProfit)&&<button type="button" className="save-product-default" disabled={savingProductDefault==="profit"} onClick={()=>void saveProductDefaults({defaultProfitTarget:pricing.targetProfit},"profit")}>{savingProductDefault==="profit"?"Saving…":`Save $${pricing.targetProfit} as the default`}</button>}</section><section><div><b>Keyword bank</b><span>Used for generated titles and matching tags.</span></div><KeywordBank compact selectionOnly initialId={autoTitleBankId||activeRecipe?.keywordListId||""} onSelect={list=>{setAutoTitleBank(list);setAutoTitleBankId(list?.id||"")}}/>{autoTitleBankId!==String(activeRecipe?.keywordListId||"")&&<button type="button" className="save-product-default" disabled={!autoTitleBankId||savingProductDefault==="keywords"} onClick={()=>void saveProductDefaults({keywordListId:autoTitleBankId},"keywords")}>{savingProductDefault==="keywords"?"Saving…":"Save this keyword bank as the default"}</button>}</section><section><div><b>Shipping profile</b><span>The amount the customer pays is separate from item profit.</span></div><select aria-label="Shipping profile for this batch" value={etsyShippingProfileId} onChange={event=>{setEtsyShippingProfileId(Number(event.target.value));setPricingApproved(false)}}><option value={0}>Choose a shipping profile</option>{etsyShippingProfiles.map(profile=><option key={profile.id} value={profile.id}>{decodeProfileTitle(profile.title)}</option>)}</select>{etsyShippingProfileId!==Number(activeRecipe?.etsyShippingProfileId)&&<button type="button" className="save-product-default" disabled={!etsyShippingProfileId||savingProductDefault==="shipping"} onClick={()=>void saveProductDefaults({etsyShippingProfileId},"shipping")}>{savingProductDefault==="shipping"?"Saving…":"Save this shipping profile as the default"}</button>}</section><section className="description-default-editor"><div><b>Product description</b><span>Used for every listing unless you customize one later.</span></div><textarea aria-label="Description for this batch" rows={5} value={description} onChange={event=>setDescription(event.target.value)}/>{description.trim()!==String(activeRecipe?.description||"").trim()&&<button type="button" className="save-product-default" disabled={!description.trim()||savingProductDefault==="description"} onClick={()=>void saveProductDefaults({description},"description")}>{savingProductDefault==="description"?"Saving…":"Save this description as the default"}</button>}</section><section className="saved-setting-list"><span><b>Etsy details</b>{Object.keys(activeRecipe?.etsyDefaults||{}).length?`${Object.keys(activeRecipe?.etsyDefaults||{}).length} product facts remembered`:"Goldie will pre-fill them from this product"}</span><span><b>Listing photos</b>{activeRecipe?.printifyImageIndices?.length?`${activeRecipe.printifyImageIndices.length} Printify photos remembered`:"Choose after previews are created"}</span></section><details className="product-connection-details"><summary>Product connection</summary><p>This link stays attached to the saved product. Change it only if you replace the published Printify product.</p><code>{activeRecipe?.templateUrl}</code></details></div></details></div>}

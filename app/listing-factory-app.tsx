...
setPricingApproved(recipeCarriesApprovedPricing({defaultProfitTarget:activeRecipe?.defaultProfitTarget,etsyShippingProfileId:activeRecipe?.etsyShippingProfileId})); return result.product;
  ...
  async function approveActualPricingGroup(group:{key:string;drafts:DraftResult[]},editedPrices?:Record<string,number>){
    ...
    if(failures.length)stopWith("Some final prices were not approved.",failures);else{/* The Printify update and the batch snapshot are one user action. Waiting for the debounced autosave let a product switch or reload restore the pre-approval snapshot even though the button had already said Saved. */setPricingApproved(true);await persistBatchNow(batchIdRef.current,{drafts:nextDrafts,pricingApproved:true});}
  }

    ...
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"}>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="designs"?"mockup-workspace":""}`}>
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          {templateDetails&&productSelected&&<button type="button" className="workflow-next setup-forward" disabled={!complete&&(!selectedColorIds.length||(Boolean(templateDetails?.sizeOptions?.length)&&!selectedSizeIds.length))} onClick={()=>complete?goToStep("finish",false,true):goToStep("designs")}>{/* D383 - This button relabelled itself with whatever was missing: "Pick a
          ...
          {workflowStep==="finish"&&finishPhase==="final"&&stepProductCards(bundleCardStatus("publish"),<>{/* D387 - This banner floated above the product card. It reports on this
              product's listings, so it belongs inside the card with them. */}
              {allCreatedListingsHaveImages()&&<div className="step-success-banner" role="status"><span aria-hidden="true">✓</span><div><b>Listing photos complete</b><small>Every listing has at least one photo and is ready for final review.</small></div></div>}

...
export default function ListingFactoryApp() {
  ...
  useEffect(()=>{const read=()=>{const url=new URL(window.location.href),value=url.searchParams.get("step") as WorkflowStep|null,phase=url.searchParams.get("phase") as FinishPhase|null;if(value&&WORKFLOW_STEPS.some(step=>step.id===value))setWorkflowStep(normalizeStep(value));if(phase&&["details","etsy","mockups","final"].includes(phase))setFinishPhase(phase)};read();window.addEventListener("popstate",read);return()=>window.removeEventListener("popstate",read)},[]);
  ...
  async function useBundle(bundle:ProductBundle,recipeIds:string[]){
    ...
    const adopt=(recipe:Recipe,details:TemplateDetails)=>{
      ...
      setBundleColorProducts(current=>({...current,[recipe.id]:details}));
      setBundleColorChoices(current=>({...current,[recipe.id]:ids}));
    };
        ...
        return <article className={`batch-product-card step-product-card ${open?"is-open":"is-closed"} ${status.tone==="ready"?"is-ready":"needs-setup"} ${many?"in-batch":""}`} key={recipe.id}>
          <header
            ...
            <span className="bundle-product-id">
              ...
              <b>{recipe.name}</b>
              <small>{product?.blueprintTitle||status.label}</small>
            </span>
    ...
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"}>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="designs"?"mockup-workspace":""}`}>
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          {templateDetails&&productSelected&&<div className="saved-product-batch-page"><section className="batch-products" aria-label="Products in this batch">{(()=>{const list=activeBundle&&bundleRecipes.length>1?bundleRecipes:(activeRecipe?[activeRecipe]:[]);const pending=list.filter((recipe,index)=>!((!activeBundle||bundleRecipes.length<2||index===bundleIndex)?templateDetails:bundleColorProducts[recipe.id]));return pending.length?<p className="bundle-product-loading"><span className="goldie-spinner" aria-hidden="true"/>Loading {pending.length} {pending.length===1?"product":"products"}…</p>:null})()}{(activeBundle&&bundleRecipes.length>1?bundleRecipes:(activeRecipe?[activeRecipe]:[])).map((recipe,index)=>{const isActive=!activeBundle||bundleRecipes.length<2||index===bundleIndex;const product=isActive?templateDetails:bundleColorProducts[recipe.id];if(!product)return null;const ready=readinessFor(product,recipe);/* D232 · Colours and sizes are open from the start. They are the two things a
          ...
          {templateDetails&&productSelected&&<button type="button" className="workflow-next setup-forward" disabled={!complete&&(!selectedColorIds.length||(Boolean(templateDetails?.sizeOptions?.length)&&!selectedSizeIds.length))} onClick={()=>complete?goToStep("finish",false,true):goToStep("designs")}>{complete?"Back to finishing your listings":activeRecipe?.setupComplete===false?"Save this product’s defaults to continue":!selectedColorIds.length?"Choose product colors to continue":Boolean(templateDetails?.sizeOptions?.length)&&!selectedSizeIds.length?"Choose product sizes to continue":false?"":bundleKeywordGaps.length?(bundleKeywordGaps.length===1?`Pick a keyword bank for ${bundleKeywordGaps[0]}`:`Pick a keyword bank for ${bundleKeywordGaps.length} products`):"Continue to images"} <span>→</span></button>}
          </BatchPreferencesPortal>
          </div>
...
<div className="final-safety-readiness" aria-label="Final publishing readiness"><span className={files.every(file=>etsyRequiredComplete(file.etsy))&&files.every(file=>!personalizationProblem(file.etsy))?"ready":"needs-review"}>{files.every(file=>etsyRequiredComplete(file.etsy))&&files.every(file=>!personalizationProblem(file.etsy))?"✓":"!"} Etsy details and personalization {files.every(file=>etsyRequiredComplete(file.etsy))&&files.every(file=>!personalizationProblem(file.etsy))?"are ready":"still need review"}</span><span className={allCreatedListingsHaveImages(selectedPublishDrafts())?"ready":"needs-review"}>{allCreatedListingsHaveImages(selectedPublishDrafts())?"✓":"!"} {allCreatedListingsHaveImages(selectedPublishDrafts())?"Every selected listing has at least one photo":"One or more selected listings still need a photo"}</span></div><FinalListingReview drafts={drafts} files={files} selections={printifyImageSelections} defaultIndices={printifyImageIndices} preparedMockupCounts={preparedMockupCounts} batchSizeGuide={sizeGuideName} onRetry={clientId=>{const design=files.find(file=>file.id===clientId);if(design)void runDrafts([design],true)}} onEdit={setFinishPhase}/><div className="publish-live-warning"><b>Only the listings selected above will be published live on Etsy.</b><span>Listings that need another try stay here while successful listings can publish now.</span><small>Etsy charges its standard $0.20 USD listing fee for each listing created. This fee is charged by Etsy and is separate from your Goldie subscription.</small></div><button className="publish-all-button" aria-busy={publishing} disabled={publishing||!allCreatedListingsHaveImages(selectedPublishDrafts())||!selectedPublishDrafts().length} onClick={openPublishConfirmation}>{publishing?"Publishing…":`Publish ${selectedPublishDrafts().length} selected ${selectedPublishDrafts().length===1?"listing":"listings"} live on Etsy`}</button><button className="keep-drafts-button" type="button" disabled={publishing} onClick={()=>{setBatchDisplayName(current=>current||suggestedBatchName());setDraftSaveOpen(true)}}>Keep as Printify drafts for now</button><small className="keep-drafts-note">Nothing will publish to Etsy. Return to this exact batch from Batch History.</small>{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}</>}</div></article>)}
      ...
      {complete && workflowStep==="designs" && <section className="post-draft-workspace">
        ...
        <div className="draft-card-grid">{drafts.map(draft=>{const design=files.find(file=>file.id===draft.clientId),selectedImages=draft.id?(printifyImageSelections[draft.id]??printifyImageIndices):printifyImageIndices;return <article id={`listing-images-${draft.clientId}`} className={`draft-card ${draft.status!=="Created"?"failed":""}`} key={draft.clientId}>
          ...
          {draft.status==="Created"&&design&&draft.id&&<details className="draft-mockups"><summary>Create lifestyle mockups from your Mockup Library (optional)</summary><IntegratedMockups design={design.file} productId={draft.id} productName={activeRecipe?.name||templateDetails?.blueprintTitle} defaultTheme={mockupTheme} referenceUrl={draft.previewUrl} onPrepared={count=>setPreparedMockupCounts(current=>({...current,[draft.id!]:count}))}/><button type="button" className="panel-collapse-foot" onClick={event=>{const box=(event.currentTarget as HTMLElement).closest("details");if(box){(box as HTMLDetailsElement).open=false;box.scrollIntoView({block:"nearest"})}}}><em aria-hidden="true">⌃</em> Close lifestyle mockups</button></details>}
        ...
        <button className="workflow-next mockup-next" disabled={progressGateIssues(8).length>0} title={progressGateIssues(8)[0]} onClick={()=>{const missing=createdListingsMissingImages();if(missing.length){setImageStepError(`${missing.length} ${missing.length===1?"listing needs":"listings need"} at least one photo.`);setMissingPhotoDraftIds(missing.map(draft=>draft.clientId));return}setImageStepError("");setMissingPhotoDraftIds([]);/* D222 · This control used to end the mockups PHASE and jump to final review. Now that photos live on the Images PAGE, that would skip the Listing page entirely — titles, tags, descriptions and Etsy details — and land on Publish. It goes to Listing, which is the next page. */setFinishPhase("details");void goToStep("finish",false,true);window.scrollTo(0,0)}}>Continue to titles <span>→</span></button>{progressGateIssues(8)[0]&&<p className="etsy-preparing-note gate-reason" role="status">{progressGateIssues(8)[0]}</p>}
      </section>}

      {complete && workflowStep==="designs" && <div className="workflow-footer-actions post-draft-footer"><button className="workflow-back" type="button" onClick={goBackOneStep}><span aria-hidden="true">←</span> Back</button><span className="autosave-note"><i aria-hidden="true">✓</i> Saved automatically</span><button className="workflow-next" type="button" onClick={()=>goToStep("finish",false,true)}>Continue to titles <span aria-hidden="true">→</span></button></div>}

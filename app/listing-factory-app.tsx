...
function IndividualSizeGuide({productId,name,batchName,onSaved}:{productId:string;name?:string;batchName?:string;onSaved:(name:string)=>void}){
  const picker=useRef<HTMLInputElement>(null),[status,setStatus]=useState(""),[saving,setSaving]=useState(false);
  async function save(file:File){if(saving)return;setSaving(true);setStatus("Saving size guide…");try{const form=new FormData();form.set("productId",productId);form.set("kind","size-guide");form.set("file",file);const response=await fetch("/api/etsy/images",{method:"POST",body:form}),payload=await response.json() as {error?:string};if(!response.ok)throw new Error(payload.error||"This size guide could not be saved.");onSaved(file.name);setStatus("")}catch(error){setStatus(error instanceof Error?error.message:"This size guide could not be saved.")}finally{setSaving(false)}}
  async function remove(){if(saving)return;setSaving(true);setStatus("Removing size guide…");try{const response=await fetch(`/api/etsy/images?productId=${encodeURIComponent(productId)}&kind=size-guide`,{method:"DELETE"}),payload=await response.json() as {error?:string};if(!response.ok)throw new Error(payload.error||"The size guide could not be removed.");onSaved("");setStatus("Size guide removed.")}catch(error){setStatus(error instanceof Error?error.message:"The size guide could not be removed.")}finally{setSaving(false)}}
  const guide=name??batchName;
  return <div className="individual-size-guide" aria-busy={saving} aria-label="Saving size guide"><div><b>Size guide <em>Optional</em></b><small>{guide?"Selected":"No size guide selected"}</small></div><input ref={picker} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={event=>{const file=event.target.files?.[0];event.target.value="";if(file)void save(file)}}/><div className="size-guide-actions"><button type="button" aria-busy={saving} disabled={saving} onClick={()=>picker.current?.click()}>{saving?"Saving…":guide?"Use a different size guide":"Choose a size guide"}</button>{guide?<button type="button" disabled={saving} onClick={()=>void remove()}>Remove size guide</button>:null}</div>{status&&<p role="status">{status}</p>}</div>
}

function DownloadListingPhotos({productId,name,indices}:{productId:string;name:string;indices:number[]}){const [downloading,setDownloading]=useState(false),[message,setMessage]=useState("");async function download(){if(downloading)return;setDownloading(true);setMessage("");try{const response=await fetch("/api/listing-photos/download",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId,printifyImageIndices:indices})});if(!response.ok){const payload=await response.json() as {error?:string};throw new Error(payload.error||"These listing photos could not be downloaded.")}const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`${name.replace(/[^a-z0-9._-]+/gi,"-").slice(0,90)||"listing"}-listing-photos.zip`;document.body.appendChild(link);link.click();link.remove();URL.revokeObjectURL(url);setMessage("✓ Download ready.")}catch(error){setMessage(error instanceof Error?error.message:"These listing photos could not be downloaded.")}finally{setDownloading(false)}}return <div className="listing-photo-download"><div><b>Download photos <em>Optional</em></b><small>Save a copy for social media or your own use.</small></div><button type="button" aria-busy={downloading} disabled={downloading} onClick={()=>void download()}>{downloading?"Preparing photos…":"Download this listing’s photos"}</button>{message&&<p role="status">{message}</p>}</div>}
...
export default function ListingFactoryApp() {
  ...
  useEffect(()=>{if(localPreview||!complete||preparingEtsy)return;const prepared=files.filter(file=>file.etsy);if(!prepared.length)return;const timer=window.setTimeout(()=>{void runBounded(prepared,2,async file=>{try{await syncPreparedListing(file,file.etsy!);syncedListingSignatures.current.set(file.id,`${file.title}\n${file.tags.join("|")}`);updateDesign(file.id,{etsyError:""})}catch(error){updateDesign(file.id,{etsyError:error instanceof Error?error.message:"The listing changes could not be saved."})}return file},()=>undefined)},1200);return()=>window.clearTimeout(timer);
...
/* D567 - loading="lazy" on an image with no intrinsic size is a deadlock: the
  ...
  function titlesLead(){return <>
      <div className="task-panel-lead"><div><p className="mini-label">BATCH TITLE BUILDER</p><h3>{activeBundle?"Create titles for this product":"Create titles for the whole batch"}</h3><p>Use suggested phrases from your validated bank, or choose the exact phrases yourself. No new keywords are ever added.</p></div><div className="title-builder-choice" role="group" aria-label="How do you want to create batch titles?"><button className={titleBuilderMode==="ai"?"active":""} onClick={()=>setTitleBuilderMode("ai")}><b>Auto-create titles</b><span>Creates a different title for each design</span></button><button className={titleBuilderMode==="manual"?"active":""} onClick={()=>setTitleBuilderMode("manual")}><b>I choose from my bank</b><span>{activeBundle?"Uses your selections for this product":"Uses your selections across the batch"}</span></button></div><div className="title-style-toggle"><span>Title format</span><button className={titleJoiner===", "?"active":""} onClick={()=>changeTitleJoiner(", ")}>With commas</button><button className={titleJoiner===" "?"active":""} onClick={()=>changeTitleJoiner(" ")}>Without commas</button>{/* D413 - Capitalization sat in its own card above the builder, but it is the
                    same decision as the comma style: how the title is formatted. One group. */}<button type="button" className={titleCaps?"active":""} aria-pressed={titleCaps} onClick={()=>changeTitleCaps(!titleCaps)}>{titleCaps?"Capitalized":"Not capitalized"}</button></div>{titleBuilderMode==="ai"?<div className="title-builder-pane"><KeywordBank selectionOnly initialId={autoTitleBankId||activeRecipe?.keywordListId||""} onSelect={list=>{setAutoTitleBank(list);setAutoTitleBankId(list?.id||"");/* D221 · Choosing the bank here IS establishing it for this product, the same as it was on the product card before the picker moved. Without this the choice would apply to this batch only and the next one would ask again. */if(activeRecipe&&list?.id&&list.id!==activeRecipe.keywordListId)void establish(activeRecipe,{keywordListId:list.id})}} title="Choose a keyword bank" copy="Only exact phrases from this bank are used."/><div className="ai-title-disclaimer"><b>Review every generated title.</b><span>Confirm that each title fits its design before continuing.</span></div><button className="ai-title-button" title={batchHeldByAnotherTab?"This batch is open in another tab, so nothing saved here would be kept.":!autoTitleBank?"Choose a keyword bank first.":!files.length?"Upload a design first.":undefined} disabled={titleBuilding||!autoTitleBank||!files.length||batchHeldByAnotherTab} onClick={()=>void buildBatchTitle()}>{titleBuilding?`Creating ${files.length} titles…`:activeBundle?"Auto-create titles for this product":"Auto-create all titles"}</button>{/* D660 · The 1566 crewneck joined a bundle with no
...
{secondaryVersions.map(artwork=>{const compatibleProducts=productsInBatch.filter(recipe=>orderedPrintSides(bundleProductDetails[recipe.id]?.printPositions).some(side=>side.toLocaleLowerCase()===artwork.side.toLocaleLowerCase())),assignedProducts=artwork.productIds?.length?artwork.productIds:(artwork.ownerProductId?[artwork.ownerProductId]:activeRecipe?.id?[activeRecipe.id]:[]);return <section className="artwork-version" key={artwork.id}><img src={artwork.previewUrl} alt=""/><div><b>{printSideLabel(artwork.side)} artwork for this listing design</b>{!artwork.colorIds.length&&<em className="artwork-color-required">Choose at least one {itemNoun} color for this {printSideLabel(artwork.side).toLocaleLowerCase()} print.</em>}{colors.length?<fieldset><legend>Use it on these {activeRecipe?.name||itemNoun} colors</legend>{colors.map(color=><button type="button" key={color.id} className={artwork.colorIds.includes(color.id)?"selected":""} aria-pressed={artwork.colorIds.includes(color.id)} onClick={()=>toggleArtworkColor(file.id,artwork.id,color.id)}><i style={{background:color.swatch||"#ddd"}}/>{color.title}</button>)}</fieldset>:null}{activeBundle&&bundleRecipes.length>1?<fieldset className="bundle-print-products"><legend>Which products get this {printSideLabel(artwork.side).toLocaleLowerCase()} artwork?</legend><small>Only products that support this print area are included. Every other product keeps its primary artwork only.</small>{compatibleProducts.map(recipe=><button type="button" key={recipe.id} className={assignedProducts.includes(recipe.id)?"selected":""} aria-pressed={assignedProducts.includes(recipe.id)} onClick={()=>toggleArtworkProduct(file.id,artwork.id,recipe.id)}><span aria-hidden="true">{assignedProducts.includes(recipe.id)?"✓":""}</span>{recipe.name}</button>)}</fieldset>:null}</div><button type="button" onClick={()=>removeArtworkVersion(file.id,artwork.id)} aria-label={`Remove ${printSideLabel(artwork.side).toLocaleLowerCase()} artwork`}>Remove</button></section>})}
    ...
    footer here too, so the cards report their products and the controls go. */}{/* D387 - This banner floated above the product card. It reports on this
            ...
            {/* D625 · A green "Listing photos complete" banner sat directly under
              ...
              own it. What this step still has to say is where the seller finishes. */}<p className="step-copy">{activeBundle&&bundleRecipes.length>1?`Prepare the saved choices for every product, then send hidden drafts from Printify to Etsy.`:"Prepare your saved choices, then send a hidden draft from Printify to Etsy."}</p>{/* D546 - her words, looking at it: "this whole section doesn't need to be on
...
{/* D559 - her question: "why if this is a hoodie t shirt and crew neck batch
            ...
            <PhotoDeliveryHandoff ref={photoDeliveryRef} onStatusReady={setPhotoDeliveryStatusReady} onReview={(id,photos)=>{const draft=bundlePublishDrafts().find(item=>item.id===id);if(draft)editReviewedListing(photos?"mockups":"details",draft)}} targets={bundlePublishDrafts().filter(draft=>draft.id&&draft.status==="Created").map(draft=>({id:draft.id!,title:draft.title||draft.name||"Listing",indices:bundlePublishSelections()[draft.id!]??printifyImageIndices,shippingProfileId:drafts.some(own=>own.id===draft.id)?etsyShippingProfileId:Number(Object.values(bundleMembers).find(member=>member.drafts.some(own=>own.id===draft.id))?.shippingProfileId)||0}))} beforePrepare={async()=>{await persistBatchNow();await persistRunNow()}}/>
              ...
              <div className="printify-handoff" role="status">
                <b>{handoffBlockers().length?"Finish the indicated items before preparing your Etsy drafts.":"Etsy drafts stay unpublished."}</b><p>Publish in Etsy only after your result says “Etsy draft verified.”</p>
...
{publishRun&&!publishing?"Queuing every listing in this batch…":publishing?(activeBundle&&bundleRecipes.length>1?(()=>{
     ...
     no longer available once publishing has started. */}{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}{publishFailures.length>0&&<section className="publish-failure-panel" role="alert"><p className="mini-label">NOTHING WAS PUBLISHED</p><h3>{publishFailures.length===1?"1 listing could not be published":`${publishFailures.length} listings could not be published`}</h3><p className="publish-failure-lede">Etsy did not create {publishFailures.length===1?"this listing":"these listings"}, so you have not been charged a listing fee for {publishFailures.length===1?"it":"them"}. Here is exactly what Etsy said:</p><ul className="publish-failure-list">{publishFailures.map(failure=>{const draft=drafts.find(item=>item.id===failure.productId);return <li key={failure.productId}><strong>{draft?.title?.slice(0,60)||draft?.name||"Listing"}</strong><span>{failure.error}</span></li>})}</ul><p className="publish-failure-lede">The error was emailed to you and recorded. You can press publish again once it is fixed.</p></section>}</div></div></>}</div></article>{!batchReceipt&&<FactoryFooter status={handoffBlockers()[0]||(!photoDeliveryStatusReady?"Check saved progress above before continuing.":"Creates drafts only. Nothing goes live.")}><button type="button" className="workflow-next" disabled={creatingEtsyDrafts||!photoDeliveryStatusReady||Boolean(handoffBlockers().length)} onClick={async()=>{setCreatingEtsyDrafts(true);try{await photoDeliveryRef.current?.prepare()}finally{setCreatingEtsyDrafts(false)}}}>{creatingEtsyDrafts?"Saving your draft request…":"Save to Etsy Drafts"}</button></FactoryFooter>}</>,false,finalProductOverview()))}
        </div>

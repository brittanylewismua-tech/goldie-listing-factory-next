...
export default function ListingFactoryApp() {
  ...
  function normalizeColorIds(details:TemplateDetails|null|undefined,ids:number[]){
    ...
    for(const id of ids){const option=options.find(color=>(color.ids?.length?color.ids:[color.id]).includes(id));if(option)canonical.add(option.id)}
    return [...canonical];
  }
    ...
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"} data-product-library={showProductLibrary?"true":"false"}>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="designs"?"mockup-workspace":""}`}>
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          {workflowStep==="designs"&&complete&&<div className="step-success-banner" role="status"><span aria-hidden="true">✓</span><div><b>Printify drafts created</b><small>{files.length} {files.length===1?"listing is":"listings are"} ready for listing photos.</small></div></div>}
...
{/* D559 - her question: "why if this is a hoodie t shirt and crew neck batch
            ...
            <FinalListingReview handoffOnly drafts={bundlePublishDrafts()} files={bundlePublishFiles()} selections={bundlePublishSelections()} defaultIndices={printifyImageIndices} preparedMockupCounts={bundlePublishMockupCounts()} batchSizeGuide={sizeGuideName} onRetry={clientId=>{const design=files.find(file=>file.id===clientId);if(design)void runDrafts([design],true)}} onEdit={setFinishPhase} onSelectionChange={setSelectedPublishIds} onSelectionTouched={()=>{sellerChosePublish.current=true}}/>{/* D548 - read as someone about to spend money, this said two untrue things.
              ...
              <div className="printify-handoff" role="status">
                <b>Your listings are ready in Printify.</b>
              ...
              <a className="publish-all-button printify-handoff-button" href="https://printify.com/app/products" target="_blank" rel="noopener noreferrer" onClick={()=>{void persistRunNow()}}>
              ...
              <small className="keep-drafts-note">Nothing is published automatically.</small>
...
<h2 id="preflight-title">{activeBundle&&bundleRecipes.length>1?`Create ${files.length*bundleRecipes.length} product drafts across ${bundleRecipes.length} products?`:`Create ${files.length} product ${files.length===1?"draft":"drafts"}?`}</h2><p className="preflight-timing">This usually takes about 15 seconds per listing. A batch of 20 can take a few minutes.</p><div className="preflight-list"><div><span>{activeBundle&&bundleRecipes.length>1?"Printify products":"Printify product"}</span><b>{activeBundle&&bundleRecipes.length>1?`✓ ${bundleRecipes.map(recipe=>recipe.name).join(", ")}`:`✓ ${templateDetails?.blueprintTitle||"Selected product"}`}</b></div><div><span>Listing designs</span><b>✓ {files.length} ready · one listing per design on each product</b></div><div className="preflight-artwork-plan"><span>Artwork on {activeRecipe?.name||"this product"}</span><b>{files.map(printPlanFor).join(" · ")}</b></div><div><span>Plan allowance</span><b>{planDraftsRemaining===null?"Checking current usage…":`✓ ${requestedListingCount} of ${planDraftsRemaining} remaining listings`}</b></div><div><span>Permanent description</span><b>{description.trim()?"✓ Imported from Printify":"None found. You can add one later"}</b></div><div><span>Starting variants</span><b title={bundleVariantCounts.detail}>✓ Using the colors and sizes saved in Printify{bundleVariantCounts.perProduct.length>1?` · ${bundleVariantCounts.detail}`:""}</b></div><div><span>Pricing + shipping</span><b>Reviewed after Printify calculates the finished drafts</b></div><div><span>Publishing</span><b>Unpublished Printify drafts only</b></div></div>{templateDetails?.hasLabelArtwork?<p className="preflight-note">Inside-label artwork is not copied to new products.</p>:null}<p className="preflight-explainer">{activeBundle&&bundleRecipes.length>1?"Primary artwork makes the listing on every product. Extra print-area artwork goes only to the compatible products you selected above. Back artwork is never used as another product’s front design. ":""}After the drafts are created, review their real previews, then approve colors, sizes, finished pricing, and shipping.</p><div className="preflight-actions"><button className="preflight-cancel" onClick={()=>setPreflightOpen(false)}>Go back</button><button className="preflight-confirm" disabled={running} aria-busy={running} onClick={confirmDrafts}>Create Printify drafts →</button></div></section></div>}

...
/* D567 - loading="lazy" on an image with no intrinsic size is a deadlock: the
  ...
  function titlesLead(){const titled=files.filter(file=>file.title.trim()).length;return <div className="task-panel-lead compact-title-tools">
      <div className="title-tools-heading"><h3>Create titles and tags</h3><span>{titled} of {files.length} ready</span></div>
      {titleBuilderMode==="ai"?<div className="title-builder-pane"><KeywordBank compact selectionOnly initialId={autoTitleBankId||activeRecipe?.keywordListId||""} onSelect={list=>{setAutoTitleBank(list);setAutoTitleBankId(list?.id||"");if(activeRecipe&&list?.id&&list.id!==activeRecipe.keywordListId)void establish(activeRecipe,{keywordListId:list.id})}} title="Keyword bank" copy=""/><button className="ai-title-button" title={batchHeldByAnotherTab?"This batch is open in another tab, so nothing saved here would be kept.":!autoTitleBank?"Choose a keyword bank first.":!files.length?"Upload a design first.":undefined} disabled={titleBuilding||!autoTitleBank||!files.length||batchHeldByAnotherTab} onClick={()=>void buildBatchTitle()}>{titleBuilding?`Creating ${files.length} titles and tags…`:activeBundle?"Create titles and tags for this product":"Create all titles and tags"}</button>{titleBuildMessage&&<p className="title-build-message" role="status">{titleBuildMessage}</p>}<details className="title-builder-options"><summary>Title options</summary>{titleFormatControls()}<button type="button" className="title-mode-switch" onClick={()=>setTitleBuilderMode("manual")}>Build titles manually</button>{activeBundle&&bundleRecipes.length>1&&autoTitleBank&&bundleRecipes.some(recipe=>recipe.id!==activeRecipe?.id&&recipe.keywordListId!==autoTitleBank.id)?<button type="button" className="title-mode-switch" disabled={applyingBankToBundle} onClick={()=>void applyBankToBundle()}>{applyingBankToBundle?"Applying…":`Use this bank for all ${bundleRecipes.length} products`}</button>:null}</details></div>:<div className="title-builder-pane manual-title-builder"><div className="manual-title-heading"><b>Build titles manually</b><button type="button" className="title-mode-switch" onClick={()=>setTitleBuilderMode("ai")}>Use automatic titles</button></div><KeywordBank compact initialId={manualKeywordBankId||activeRecipe?.keywordListId||""} onSelect={list=>setManualKeywordBankId(list?.id||"")} onAdd={addBatchKeyword} title="Keyword bank" copy="Choose phrases in title order."/><div className="selected-batch-keywords"><div><b>Selected keywords</b>{batchKeywords.length>0&&<button onClick={clearBatchKeywords}>Clear all</button>}</div>{batchKeywords.length?<div className="selected-keyword-chips">{batchKeywords.map(keyword=><button key={keyword} onClick={()=>removeBatchKeyword(keyword)}>{keyword}<span>×</span></button>)}</div>:<p>No keywords selected yet.</p>}</div>{batchKeywords.length>0&&<div className="batch-title-preview"><b>Title preview</b><span>{batchKeywords.join(titleJoiner)}</span><small>{activeBundle?"Applied to this product’s listings.":"Applied to every listing."}</small></div>}<details className="title-builder-options"><summary>Title options</summary>{titleFormatControls()}</details></div>}
    </div>}
...
{/* D496 - a held tab has to say so where she is working, not silently stop
    saving. */}
        ...
        {!(complete&&workflowStep==="designs")&&<div className="workflow-footer-actions">{reviewEditing?null:progressIndex>0&&<button className="workflow-back" type="button" onClick={goBackOneStep}><span aria-hidden="true">←</span> Back</button>}<span className="autosave-note"><i aria-hidden="true">{batchAuthenticationRequired||batchSaveConflict||batchHeldByAnotherTab?"!":"✓"}</i> {batchAuthenticationRequired?"Sign in to save":batchSaveConflict?"Saving paused":batchHeldByAnotherTab?"Saving paused in this tab":"Saved automatically"}</span>{/* D776 - the step's own footer (status + forward) lands here, so the bar the seller can see is the bar with the way forward in it. */}<span className="factory-footer-slot"/>{reviewEditing?<button className="workflow-next" type="button" onClick={()=>openFinishedReview(false)}>Back to Review <span aria-hidden="true">→</span></button>:<>{/* D386 - Saving a draft was only reachable from the Publish step, so

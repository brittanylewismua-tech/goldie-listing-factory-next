      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="designs"?"mockup-workspace":""}`}>
        <nav className="workflow-progress" aria-label="Listing Factory progress" style={{"--rail-count":RAIL_STAGES.length} as React.CSSProperties}>
          ...
          {RAIL_STAGES.map((stage,position)=>{
            ...
            const done=progressIndex>Math.max(...stage.covers);
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          />}
          {templateDetails&&productSelected&&<button type="button" className="workflow-next setup-forward" disabled={!complete&&(!selectedColorIds.length||(Boolean(templateDetails?.sizeOptions?.length)&&!selectedSizeIds.length))} onClick={()=>complete?goToStep("finish",false,true):goToStep("designs")}>{complete?"Back to finishing your listings":activeRecipe?.setupComplete===false?"Save this product’s defaults to continue":!selectedColorIds.length?"Choose product colors to continue":Boolean(templateDetails?.sizeOptions?.length)&&!selectedSizeIds.length?"Choose product sizes to continue":false?"":bundleKeywordGaps.length?`Pick a keyword bank for ${bundleKeywordGaps.join(", ")}`:"Continue to designs"} <span>→</span></button>}
          </BatchPreferencesPortal>
          </div>

...
function ProductColorRendering({title,color,artworkUrl,productRenderingUrl}:{title:string;color:string;artworkUrl?:string;productRenderingUrl?:string}){
  ...
  return <div className="product-color-rendering" data-family={family} style={{"--product-glyph-color":color||"#ddd"} as CSSProperties}>
    {productRenderingUrl?<img className="product-color-rendering-base" src={productRenderingUrl} alt=""/>:<ProductGlyph title={title} color={color}/>} 
...
function DraftColorSelector({product,drafts,selected,saving,artworkByDraft,onChange,onArtworkChange}:{product:TemplateDetails;drafts:DraftResult[];selected:number[];saving:boolean;artworkByDraft:Record<string,string>;onChange:(ids:number[])=>void;onArtworkChange:(draft:DraftResult,color:ProductColor,list:FileList|null,reset?:boolean)=>void}){
  ...
  const mainArtwork=artworkByDraft[draft.clientId]||"";
  ...
  function focusColor(id:number){setActiveColor(id);setShowRealPreview(false)}
  function toggle(color:ProductColor){const next=new Set(selectedSet);if(idsFor(color).some(id=>next.has(id))){for(const id of idsFor(color))next.delete(id)}else next.add(color.id);focusColor(color.id);onChange([...next])}
  ...
  return <section ref={selectorRef} className="draft-color-selector" aria-label="Preview and choose product colors">
    ...
    <div className="draft-color-workspace"><div className="draft-color-main">{showRealPreview&&realPreview?<img src={realPreview} alt={`${focused.title} finished Printify preview`}/>:<ProductColorRendering title={product.blueprintTitle} color={focused.swatch} artworkUrl={mainArtwork} productRenderingUrl={productRendering}/>}<b>{focused.title}</b><span>{override?"Using alternate artwork":"Using the main design"}</span><button type="button" className="draft-color-preview" disabled={!realPreview} onClick={()=>setShowRealPreview(value=>!value)}>{showRealPreview?"Back to edit view":"Preview"}</button><div className="draft-color-artwork-action"><label role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.currentTarget.querySelector("input")?.click()}}}>{override?`Change artwork for ${focused.title}`:`Use different artwork for ${focused.title}`}<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" disabled={saving} onChange={event=>{onArtworkChange(draft,focused,event.target.files);event.target.value=""}}/></label>{override?<button type="button" disabled={saving} onClick={()=>onArtworkChange(draft,focused,null,true)}>Use main design</button>:null}</div></div><div className="draft-color-grid">{colors.map(color=>{const included=idsFor(color).some(id=>selectedSet.has(id));return <button type="button" key={color.id} aria-pressed={included} className={included?"selected":""} onMouseEnter={()=>focusColor(color.id)} onFocus={()=>focusColor(color.id)} onClick={()=>toggle(color)}><i className="draft-color-product-glyph" style={{"--product-glyph-color":color.swatch||"#ddd"} as CSSProperties}><ProductGlyph title={product.blueprintTitle} color={color.swatch}/></i><span>{color.title}</span><em>{included?"✓ Included":"Add"}</em></button>})}</div></div>
...
function EtsyDetailsEditor({design,categories,onChange,onCategory,checklist=true}:{design:DesignFile;categories:EtsyCategoryOption[];onChange:(details:EtsyDetails)=>void;onCategory:(taxonomyId:number)=>Promise<void>;checklist?:boolean}){
  ...
  return <details className="etsy-details-editor" open onToggle={event=>{if(!(event.currentTarget as HTMLDetailsElement).open)(event.currentTarget as HTMLDetailsElement).open=true}}><summary><span><b>Etsy details</b><small>{required.length?`${requiredDone.length} of ${required.length} required set`:`${completed.length} added · all optional`}{preview?` · ${preview}`:""}</small></span></summary><div className="factory-listing-grid">{/* D730 - prototype .goldie-listing-grid: the fields on the left, and
      beside them the list of what Etsy still needs. The summary line already
      counted them ("2 of 5 required set"); the checklist names them. Every
      field, handler and validation below is unchanged. */}<div className="etsy-details-editor-fields factory-form-card"><label>Etsy category<small>Current: {details.category||"None chosen"}</small><input type="search" value={query} placeholder="Search Etsy categories" onChange={event=>setQuery(event.target.value)} disabled={loading}/></label>{matches.length?<div className="etsy-category-results" role="listbox" aria-label="Matching Etsy categories">{matches.map(category=><button type="button" key={category.id} onClick={()=>void choose(category.id)}>{category.path}</button>)}</div>:query.trim().length>=2?<small>No matching Etsy categories.</small>:null}{loading&&<small>Loading the exact Etsy options for this category…</small>}<div className="etsy-attribute-list">{properties.map(property=><LazyEtsyProperty key={property.propertyId} property={property} onValue={value=>setProperty(property,value)}/>)}</div><small className="optional-note">Optional fields can stay blank.</small><PersonalizationEditor value={details.personalization} onChange={personalization=>onChange({...details,personalization})}/></div>{checklist?<RequiredDetailsChecklist items={[{key:"category",label:"Etsy category",value:details.category||"",required:true},...properties.filter(property=>property.required||property.value.trim()).map(property=>({key:String(property.propertyId),label:property.label,value:property.value,required:property.required}))]}/>:null}</div></details>;
}

function IndividualSizeGuide({productId,name,batchName,onSaved}:{productId:string;name?:string;batchName?:string;onSaved:(name:string)=>void}){const picker=useRef<HTMLInputElement>(null),[status,setStatus]=useState(""),[saving,setSaving]=useState(false);async function save(file:File){if(saving)return;setSaving(true);setStatus("Saving size guide…");try{const form=new FormData();form.set("productId",productId);form.set("kind","size-guide");form.set("file",file);const response=await fetch("/api/etsy/images",{method:"POST",body:form}),payload=await response.json() as {error?:string};if(!response.ok)throw new Error(payload.error||"This size guide could not be saved.");onSaved(file.name);setStatus("")}catch(error){setStatus(error instanceof Error?error.message:"This size guide could not be saved.")}finally{setSaving(false)}}const guide=name||batchName;return <div className="individual-size-guide"><div><b>Size guide</b><small>{guide?"Selected":"No size guide selected"}</small></div><input ref={picker} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={event=>{const file=event.target.files?.[0];if(file)void save(file)}}/><button type="button" aria-busy={saving} disabled={saving} onClick={()=>picker.current?.click()}>{saving?"Saving…":guide?"Use a different size guide":"Choose a size guide"}</button>{status&&<p role="status">{status}</p>}</div>}
...
/* D567 - loading="lazy" on an image with no intrinsic size is a deadlock: the
  ...
  function titlesLead(){return <>
      <div className="task-panel-lead"><div><p className="mini-label">BATCH TITLE BUILDER</p><h3>Create titles for the whole batch</h3><p>Use suggested phrases from your validated bank, or choose the exact phrases yourself. No new keywords are ever added.</p></div><div className="title-builder-choice" role="group" aria-label="How do you want to create batch titles?"><button className={titleBuilderMode==="ai"?"active":""} onClick={()=>setTitleBuilderMode("ai")}><b>Auto-create titles</b><span>Creates a different title for each design</span></button><button className={titleBuilderMode==="manual"?"active":""} onClick={()=>setTitleBuilderMode("manual")}><b>I choose from my bank</b><span>Uses your selections across the batch</span></button></div><div className="title-style-toggle"><span>Title format</span><button className={titleJoiner===", "?"active":""} onClick={()=>changeTitleJoiner(", ")}>With commas</button><button className={titleJoiner===" "?"active":""} onClick={()=>changeTitleJoiner(" ")}>Without commas</button>{/* D413 - Capitalization sat in its own card above the builder, but it is the
  ...
  function descriptionLead(){return <>
      <div className="task-panel-lead"><div className="batch-description-body"><label>Description for every listing<textarea rows={9} value={description} onChange={event=>setDescription(event.target.value)} placeholder="Add sizing, materials, production, care, and shipping information"/></label><p>This came from your saved product. Edit it here to change every listing in this batch.</p>{/* D232 · "Save this description as the default" went with the settings block. The
...
setPricingApproved(recipeCarriesApprovedPricing({defaultProfitTarget:activeRecipe?.defaultProfitTarget,etsyShippingProfileId:activeRecipe?.etsyShippingProfileId})); return result.product;
  ...
  async function updateDraftColorArtwork(draft:DraftResult,color:ProductColor,list:FileList|null,reset=false){
    ...
    const colorIds=[...new Set([color.id,...(color.ids||[])])],variantIds=printifyVariantIdsForColor(templateDetails.variants,colorIds);
  ...
  async function refreshDraftPhotos(productId:string){try{const response=await fetch("/api/printify/drafts/update",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({productId,refreshImages:true})});const payload=await response.json() as {draft?:DraftResult};if(response.ok&&payload.draft)setDrafts(current=>current.map(item=>item.id===productId?payload.draft!:item))}catch{/* Keep the already loaded photos when Printify is temporarily unavailable. */}}
...
{/* D559 - her question: "why if this is a hoodie t shirt and crew neck batch
            ...
            {!handoffBlockers().length&&<div className="printify-ready-hero"><h2>Your listings are ready in Printify.</h2><p>Review the finished products below, then open My Products when you are ready.</p></div>}
     ...
     no longer available once publishing has started. */}{publishMessage&&<p className="publish-message" role="status">{publishMessage}</p>}{publishFailures.length>0&&<section className="publish-failure-panel" role="alert"><p className="mini-label">NOTHING WAS PUBLISHED</p><h3>{publishFailures.length===1?"1 listing could not be published":`${publishFailures.length} listings could not be published`}</h3><p className="publish-failure-lede">Etsy did not create {publishFailures.length===1?"this listing":"these listings"}, so you have not been charged a listing fee for {publishFailures.length===1?"it":"them"}. Here is exactly what Etsy said:</p><ul className="publish-failure-list">{publishFailures.map(failure=>{const draft=drafts.find(item=>item.id===failure.productId);return <li key={failure.productId}><strong>{draft?.title?.slice(0,60)||draft?.name||"Listing"}</strong><span>{failure.error}</span></li>})}</ul><p className="publish-failure-lede">The error was emailed to you and recorded. You can press publish again once it is fixed.</p></section>}</div></div></>}</div></article>{!batchReceipt&&<FactoryFooter status={handoffBlockers()[0]||"Nothing publishes until you choose it in Printify."}><a className={`workflow-next${handoffBlockers().length?" disabled":""}`} aria-disabled={handoffBlockers().length>0} href={handoffBlockers().length?undefined:"https://printify.com/app/products"} target="_blank" rel="noopener noreferrer" onClick={event=>{const issues=handoffBlockers();if(issues.length){event.preventDefault();stopWith("Finish all sections first.",issues);return}void persistRunNow()}}>Open My Products <span aria-hidden="true">↗</span></a></FactoryFooter>}</>)}
        ...
        {stepProductCards(bundleCardStatus("images"),null,!(workflowStep==="designs")||complete,<aside className={`launch-panel workflow-panel ${workflowStep==="designs"&&!complete?"active-panel":"hidden-panel"}`}>
          ...
          <div className="summary-list">
            ...
            {!(activeBundle&&bundleRecipes.length>1)&&<div><span>Saved product</span><b>{activeRecipe?.name||templateDetails?.blueprintTitle||"Not selected"}</b><button type="button" aria-label="Change saved product" onClick={()=>goToStep("setup")}>Change product</button></div>}
          ...
          {running && (
            <div className="batch-progress" role="status" aria-live="polite">
              ...
              <div className="progress-copy"><b>Creating your Printify drafts</b><span>{preparationMessage || "Keep this page open until the batch is finished."}</span></div>
...
<h2 id="preflight-title">{activeBundle&&bundleRecipes.length>1?`Create ${files.length*bundleRecipes.length} private drafts?`:`Create ${files.length} private ${files.length===1?"draft":"drafts"}?`}</h2><p className="preflight-timing">Nothing publishes yet.</p><div className="preflight-list"><div><span>{activeBundle&&bundleRecipes.length>1?"Products":"Product"}</span><b>{activeBundle&&bundleRecipes.length>1?bundleRecipes.map(recipe=>recipe.name).join(", "):templateDetails?.blueprintTitle||"Selected product"}</b></div><div><span>Designs</span><b>{files.length} · one draft per design{activeBundle&&bundleRecipes.length>1?" on each product":""}</b></div><div><span>Starting options</span><b title={bundleVariantCounts.detail}>Saved Printify colors and sizes</b></div><div><span>Next</span><b>Review previews, prices, and shipping</b></div></div><div className="preflight-actions"><button className="preflight-cancel" onClick={()=>setPreflightOpen(false)}>Go back</button><button className="preflight-confirm" disabled={running} aria-busy={running} onClick={confirmDrafts}>Create Printify drafts →</button></div></section></div>}

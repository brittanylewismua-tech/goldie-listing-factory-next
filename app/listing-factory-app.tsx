        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          <article className={`step-card designs-step workflow-panel ${workflowStep==="setup"?"batch-design-drop":""} ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"?"active-panel":"hidden-panel"}`}>{/* D238 · Choosing the mockup SET lived on Product while the mockups it controls are generated here on Images. Same setting, two pages — the exact split that caused the keyword-bank and shipping duplication. */}
            ...
            <div className="step-content">
              ...
              {files.length>0&&(workflowStep==="setup"||workflowStep==="designs")&&<div className="design-upload-review" aria-label="Review uploaded designs">{files.map(file=>{const colors=(templateDetails?.colorOptions||[]).filter(color=>selectedColorIds.includes(color.id)),positions=templateDetails?.printPositions||[],frontReady=positions.some(position=>/front|chest/i.test(position)),backReady=frontReady&&positions.some(position=>/back/i.test(position));return <article className="design-artwork-card" key={file.id}><div className="design-artwork-primary"><UploadedDesignPreview src={file.previewUrl} name={file.name}/><div><b title={file.name}>{file.name}</b><small>{file.width&&file.height?`${file.width} × ${file.height}px`:"Checking dimensions…"}</small><em>Primary artwork · used wherever no alternate is assigned</em></div><button type="button" onClick={()=>removeDesign(file.id)} aria-label={`Remove ${file.name}`}>Remove</button></div>{((colors.length>1&&frontReady)||backReady)&&<div className="artwork-version-tools"><b>Print artwork options</b><small>Add a light or dark version for specific garment colors, or add separate artwork to the back.</small><div>{colors.length>1&&frontReady?<label className="secondary-action">＋ Artwork for certain colors<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"front",event.target.files);event.target.value=""}}/></label>:null}{backReady?<label className="secondary-action">＋ Back print<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"back",event.target.files);event.target.value=""}}/></label>:frontReady?<span className="print-area-unavailable">Back print becomes available when this saved Printify product has artwork positioned on the back.</span>:null}</div></div>}{(file.artworkVersions||[]).map(artwork=><section className="artwork-version" key={artwork.id}><img src={artwork.previewUrl} alt=""/><div><b>{artwork.side==="back"?"Back print":"Alternate front artwork"}</b><small>{artwork.name}</small>{!artwork.colorIds.length&&<em className="artwork-color-required">Choose at least one garment color to use this artwork.</em>}{colors.length?<fieldset><legend>{artwork.side==="back"?"Print on these garment colors":"Use this artwork on"}</legend>{colors.map(color=><button type="button" key={color.id} className={artwork.colorIds.includes(color.id)?"selected":""} aria-pressed={artwork.colorIds.includes(color.id)} onClick={()=>toggleArtworkColor(file.id,artwork.id,color.id)}><i style={{background:color.swatch||"#ddd"}}/>{color.title}</button>)}</fieldset>:null}</div><button type="button" onClick={()=>removeArtworkVersion(file.id,artwork.id)} aria-label={`Remove ${artwork.name}`}>Remove</button></section>)}</article>})}</div>}

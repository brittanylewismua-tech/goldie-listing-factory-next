...
setSavedRevision(current=>current+1);}catch(error){/* Automatic defaults are a convenience, not a reason to stop the seller's run with a modal. Explicit saves still report the failure. */if(key!=="auto-defaults")stopWith("This default was not saved.",[error instanceof Error?error.message:"Try again in a moment."])}finally{setSavingProductDefault("")}}
  ...
  function reviewListingSectionNav(design?:DesignFile){
    ...
    const editingAllListings=current==="title";
    return <>
      <aside className="review-listing-editor-nav" aria-label={editingAllListings?"Edit titles and tags for all listings":`Edit listing ${position+1}`}>
        {editingAllListings?<div className="review-listing-current is-all-listings"><span><small>Editing all listings</small><b>{files.length} titles &amp; tag sets</b></span></div>:<div className="review-listing-current"><span className="review-listing-thumb">{currentPreview?<img src={currentPreview} alt="" decoding="async"/>:<span aria-hidden="true">{position+1}</span>}</span><span><small>Editing listing {position+1} of {files.length}</small><b>{design.title.trim()||"Untitled"}</b></span></div>}
        {!editingAllListings&&files.length>1&&<div className="review-listing-picker"><button type="button" aria-label="Previous listing" disabled={movablePosition<=0} onClick={()=>move(movableListings[movablePosition-1])}>←</button><label><span>Jump to listing</span><select aria-label="Jump to listing" value={design.id} onChange={event=>{const next=files.find(candidate=>candidate.id===event.target.value);if(next)move(next)}}>{files.map((candidate,index)=>{const available=movableListings.some(item=>item.id===candidate.id);return <option key={candidate.id} value={candidate.id} disabled={!available}>{`Listing ${index+1}${candidate.title.trim()?` — ${candidate.title.trim()}`:""}`}</option>})}</select></label><button type="button" aria-label="Next listing" disabled={movablePosition<0||movablePosition>=movableListings.length-1} onClick={()=>move(movableListings[movablePosition+1])}>→</button></div>}

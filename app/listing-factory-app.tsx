...
const WORKFLOW_HELP = [
  {title:"Connect Printify and Etsy",intro:"Both accounts must be connected before Goldie can build a complete listing.",sections:[{heading:"Printify connection",copy:"Connect the Printify account that contains your saved product. Goldie uses it to read product costs and variants, upload artwork, and create unpublished product drafts."},{heading:"Etsy connection",copy:"Connect the Etsy shop linked to that saved product. Goldie uses Etsy’s real categories, attributes, and shipping profiles to prepare each draft correctly."},{heading:"Nothing publishes here",copy:"This step only verifies access. Goldie never publishes listings to Etsy or triggers Etsy listing fees."},{heading:"Use matching accounts",copy:"Connect the Printify account that contains the saved product and the Etsy shop where that product was published. If the product belongs to a different shop, Goldie will stop and explain the mismatch."},{heading:"Your publishing safeguard",copy:"Goldie creates unpublished Printify drafts. When the batch is ready, you choose what publishes from Printify My Products."}]},
  ...
  {title:"Review the draft plan",intro:"Check exactly what Goldie will create before anything is sent to Printify.",sections:[{heading:"Confirm the scope",copy:"Goldie names the saved product, number of designs, and total private drafts in this run. A product bundle names every included product separately."},{heading:"Confirm the artwork",copy:"Review the primary artwork, any color-specific alternatives, and each additional print area before continuing."},{heading:"Nothing publishes to Etsy",copy:"This action creates private Printify drafts only. Goldie cannot publish them to Etsy."},{heading:"Prices come after the drafts",copy:"Once Printify applies every print location, Goldie reads the finished production costs so you can approve accurate prices and shipping."}]},
  {title:"Create the Printify drafts",intro:"This creates one unpublished Printify product draft for every uploaded design.",sections:[{heading:"What Goldie copies",copy:"Goldie starts with the saved product's variants and placement, applies the uploaded artwork, and creates a private draft so you can judge the real product mockups before finalizing colors and prices."},{heading:"What this does not do",copy:"The products are not published to Etsy at this point. They remain unpublished Printify drafts while you finish colors, prices, titles, Etsy details, and images."},{heading:"Keep the page open",copy:"Large artwork and large batches can take time. Goldie processes the batch safely and shows progress as each draft is completed."},{heading:"If one draft fails",copy:"Goldie keeps successful drafts and identifies the failed design so it can be retried without duplicating the completed products."}]},
  ...
  {title:"Complete the final review",intro:"This is the last checkpoint before you finish publishing in Printify.",sections:[{heading:"Open every listing summary",copy:"Review the title, tags, description, Etsy details, prices, shipping, and selected images. Use the edit buttons to return to any unfinished section."},{heading:"Finish in Printify",copy:"Goldie keeps every product as a private Printify draft. The final action opens Printify My Products, where you select the products from this batch and publish them yourself."},{heading:"No automatic Etsy charge",copy:"Goldie does not publish to Etsy from this screen, so opening Printify cannot create listings or trigger Etsy listing fees."},{heading:"Keep the batch",copy:"Your work remains available in Batch History if you want to return to it before publishing in Printify."}]},
];
...
export default function ListingFactoryApp() {
  ...
  async function removeDesign(id:string){const removed=files.find(file=>file.id===id);if(!removed)return;if(drafts.length&&!await confirmAction({title:`Remove ${removed.name}?`,body:"Its existing Printify draft stays in Printify. This listing is removed from this Goldie batch.",confirmLabel:"Remove listing",destructive:true}))return;const next=files.filter(file=>file.id!==id);URL.revokeObjectURL(removed.previewUrl);for(const artwork of removed.artworkVersions||[])if(artwork.previewUrl)URL.revokeObjectURL(artwork.previewUrl);setFiles(next);setFileError("");setFileNotice(`${removed.name} was removed.`);setComplete(false);setDrafts([]);setProcessed(0);const batchId=batchIdRef.current;if(batchId){if(next.length){void saveBatchFiles(batchId,next.map(file=>file.file)).catch(()=>undefined);persistArtworkFiles(next)}else void clearBatchFiles(batchId)}}
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          <article className={`step-card designs-step workflow-panel ${workflowStep==="setup"?"batch-design-drop":""} ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="setup"&&Boolean(templateDetails)&&productSelected&&!failedBundleNames().length&&!bundleCreationMode)?"active-panel":"hidden-panel"}`}>{/* D238 · Choosing the mockup SET lived on Product while the mockups it controls are generated here on Images. Same setting, two pages — the exact split that caused the keyword-bank and shipping duplication. */}
            ...
            <div className="step-content">
              ...
              <p className="upload-primary-note"><b>Main upload: one {uploadPrimaryLabel} design creates one listing.</b><span>{uploadSecondaryLabel?`This product also supports ${uploadSecondaryLabel} artwork. Add it—or a color-specific alternate—inside that listing’s card after the ${uploadPrimaryLabel} design appears below.`:`Any color-specific alternate artwork is added inside that listing’s card after the ${uploadPrimaryLabel} design appears below.`}</span></p>
              <div className="upload-actions">
              <button className="folder-drop" onClick={() => folderPicker.current?.click()}>
                <span className="upload-icon" aria-hidden="true">↑</span>
                <span><b>{files.length ? designsFinished?"Choose a folder to add more":`Choose a folder · preparing ${designsReady} of ${files.length}` : `Choose a folder of ${uploadPrimaryLabel} designs`}</b><small>{files.length ? `${files.length} design${files.length===1?"":"s"} ready · ${(totalSize / 1024 / 1024).toFixed(1)} MB selected${totalSize>LARGE_BATCH_THRESHOLD?" · will process one at a time":""} · Choose again to add more` : `Each image creates one listing · up to ${batchDesignLimit}`}</small></span>
                <span className="browse-chip">Browse</span>
              </button>
              <button className="folder-drop" onClick={() => imagePicker.current?.click()}>
                <span className="upload-icon" aria-hidden="true">＋</span>
                <span><b>Choose {uploadPrimaryLabel} design images</b><small>Select one listing design or several at once</small></span>
                <span className="browse-chip">Browse</span>
              </button>
              </div>
              ...
              {files.length>0&&(workflowStep==="setup"||workflowStep==="designs")&&<div className="design-upload-review" aria-label="Review uploaded designs">{files.map(file=>{const colors=(templateDetails?.colorOptions||[]).filter(color=>selectedColorIds.includes(color.id)),sides=orderedPrintSides(templateDetails?.printPositions),primarySide=primaryPrintSide(sides),secondarySides=sides.filter(side=>side!==primarySide),itemNoun=productNoun(templateDetails?.blueprintTitle,templateDetails?.brand,templateDetails?.model),canVaryByColor=colors.length>1&&Boolean(primarySide),primaryVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side===primarySide),secondaryVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side!==primarySide);return <article className="design-artwork-card" key={file.id}>
                <div className="design-artwork-primary"><UploadedDesignPreview src={file.previewUrl} name={file.name}/><div><b title={file.name}>{file.name}</b><small>{file.width&&file.height?`${file.width} × ${file.height}px`:"Checking dimensions…"}</small><em>{primarySide?`Main design · ${printSideLabel(primarySide)}`:"Main design"}</em></div><button type="button" className="artwork-remove-action" onClick={()=>removeDesign(file.id)} aria-label={`Remove ${file.name}`}>Remove</button></div>
                {(canVaryByColor||secondarySides.length>0)&&<div className="artwork-version-tools"><b>Optional artwork for this listing</b><small>Add a file only when this listing uses different art on some colors or on another print area shown here.</small><div>{canVaryByColor?<label className="secondary-action" role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.currentTarget.querySelector("input")?.click()}}}>＋ Use different artwork on some colors<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{if(primarySide)void addArtworkVersion(file.id,primarySide,event.target.files);event.target.value=""}}/></label>:null}{secondarySides.map(side=><label className="secondary-action" role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.currentTarget.querySelector("input")?.click()}}} key={side}>＋ Add {printSideLabel(side).toLocaleLowerCase()} artwork to this design<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,side,event.target.files);event.target.value=""}}/></label>)}{sides.length===1?<span className="print-area-unavailable">This saved Printify product has one print area ({printSideLabel(primarySide).toLocaleLowerCase()}).</span>:null}</div></div>}
                {primaryVersions.length>0&&colors.length>0&&primarySide?<section className="artwork-assignment-matrix"><div><b>Artwork by color</b><small>Every selected color gets exactly one {printSideLabel(primarySide).toLocaleLowerCase()} design.</small></div>{colors.map(color=><label key={color.id}><span><i style={{background:color.swatch||"#ddd"}}/>{color.title}</span><select aria-label={`${printSideLabel(primarySide)} artwork for ${color.title}`} value={sideAssignmentFor(primaryVersions,primarySide,color.id)} onChange={event=>assignPrimaryArtwork(file.id,primarySide,color.id,event.target.value)}><option value="primary">Primary · {file.name}</option>{primaryVersions.map(artwork=><option value={artwork.id} key={artwork.id}>{artwork.name}</option>)}</select></label>)}</section>:null}
                {primaryVersions.map(artwork=><section className="artwork-version compact" key={artwork.id}><img src={artwork.previewUrl} alt=""/><div><b>Alternate {printSideLabel(primarySide).toLocaleLowerCase()} artwork</b><small>{artwork.name}</small><em>{artwork.colorIds.length?`${artwork.colorIds.length} ${itemNoun} ${artwork.colorIds.length===1?"color":"colors"} assigned`:"Assign this file in the color table above, or remove it."}</em></div><button type="button" onClick={()=>removeArtworkVersion(file.id,artwork.id)} aria-label={`Remove ${artwork.name}`}>Remove</button></section>)}
                {secondaryVersions.map(artwork=>{const compatibleProducts=productsInBatch.filter(recipe=>orderedPrintSides(bundleProductDetails[recipe.id]?.printPositions).some(side=>side.toLocaleLowerCase()===artwork.side.toLocaleLowerCase())),assignedProducts=artwork.productIds?.length?artwork.productIds:(artwork.ownerProductId?[artwork.ownerProductId]:activeRecipe?.id?[activeRecipe.id]:[]);return <section className="artwork-version" key={artwork.id}><img src={artwork.previewUrl} alt=""/><div><b>{printSideLabel(artwork.side)} artwork for this listing design</b><small>{artwork.name}</small>{!artwork.colorIds.length&&<em className="artwork-color-required">Choose at least one {itemNoun} color for this {printSideLabel(artwork.side).toLocaleLowerCase()} print.</em>}{colors.length?<fieldset><legend>Use it on these {activeRecipe?.name||itemNoun} colors</legend>{colors.map(color=><button type="button" key={color.id} className={artwork.colorIds.includes(color.id)?"selected":""} aria-pressed={artwork.colorIds.includes(color.id)} onClick={()=>toggleArtworkColor(file.id,artwork.id,color.id)}><i style={{background:color.swatch||"#ddd"}}/>{color.title}</button>)}</fieldset>:null}{activeBundle&&bundleRecipes.length>1?<fieldset className="bundle-print-products"><legend>Which products get this {printSideLabel(artwork.side).toLocaleLowerCase()} artwork?</legend><small>Goldie applies it only to products that support this print area. Every other product keeps its primary artwork only.</small>{compatibleProducts.map(recipe=><button type="button" key={recipe.id} className={assignedProducts.includes(recipe.id)?"selected":""} aria-pressed={assignedProducts.includes(recipe.id)} onClick={()=>toggleArtworkProduct(file.id,artwork.id,recipe.id)}><span aria-hidden="true">{assignedProducts.includes(recipe.id)?"✓":""}</span>{recipe.name}</button>)}</fieldset>:null}</div><button type="button" onClick={()=>removeArtworkVersion(file.id,artwork.id)} aria-label={`Remove ${artwork.name}`}>Remove</button></section>})}
              </article>})}</div>}
        ...
        {stepProductCards(bundleCardStatus("images"),null,!(workflowStep==="designs"),<aside className={`launch-panel workflow-panel ${workflowStep==="designs"?"active-panel":"hidden-panel"}`}>
          ...
          {!complete ? (
            ...
            <div className="batch-actions">
              {costReviewGroups().map(group=>{const costs=group.review.variants.filter(variant=>variant.isEnabled).map(variant=>variant.cost),low=costs.length?Math.min(...costs):0,high=costs.length?Math.max(...costs):0,approved=group.drafts.every(draft=>draft.costReview?.approved),verified=group.drafts.every(draft=>draft.costReview?.verified);return <section className={`actual-cost-review ${approved?"approved":""}`} key={group.key}><div><b>{group.productName}</b><small>{group.drafts.length} {group.drafts.length===1?"listing":"listings"} · Final Printify production cost {low===high?`$${(low/100).toFixed(2)}`:`$${(low/100).toFixed(2)}–$${(high/100).toFixed(2)}`}</small><span>{verified?"Read from the finished Printify drafts after every print location was applied.":"Goldie could not verify every finished draft's production costs yet."}</span></div>{approved?<strong>✓ Final prices approved</strong>:<button type="button" disabled={!verified||Boolean(pricingApprovalGroup)} onClick={()=>void approveActualPricingGroup(group)}>{pricingApprovalGroup===group.key?"Saving final prices…":verified?`Approve final prices for ${group.drafts.length} ${group.drafts.length===1?"listing":"listings"}`:"Actual costs unavailable"}</button>}</section>})}
          ...
          <p className="launch-note">This step creates unpublished Printify drafts. Goldie never publishes them to Etsy; you finish from Printify My Products.</p>
        </aside>,false)}

  ...
  return <>{/* D407 - Was open by default, so arriving on Images dropped you into the
    ...
    disclosure, so the shell made a second accordion inside the first. */}
      ...
      {(()=>{
        ...
        const groups=mockupViewGroups(indexed,colors,variants,details),primary=groups.filter(group=>group.primary),other=groups.filter(group=>!group.primary);
...
{/* D559 - her question: "why if this is a hoodie t shirt and crew neck batch
            ...
            <PhotoDeliveryHandoff ref={photoDeliveryRef} targets={bundlePublishDrafts().filter(draft=>draft.id&&draft.status==="Created").map(draft=>({id:draft.id!,title:draft.title||draft.name||"Listing",indices:bundlePublishSelections()[draft.id!]??printifyImageIndices}))} beforePrepare={async()=>{await persistBatchNow();await persistRunNow()}}/>
              ...
              <div className="printify-handoff" role="status">
                <b>{handoffBlockers().length?"Review any unfinished items before publishing in Printify.":"Review and publish in Printify."}</b>
                <span>Publishing and renewal stay under your control in Printify.</span>

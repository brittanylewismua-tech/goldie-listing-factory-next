...
function PricingReview({section="all",variants,pricing,prices,productName,profiles,selectedProfileId,templateShippingProfileId,profilesLoading,profilesError,onReloadProfiles,approved,preserveEdits=false,wholeNumber=false,onWholeNumber,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprovalChange}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;productName:string;section?:"all"|"prices"|"shipping";profiles:EtsyShippingProfile[];selectedProfileId:number;templateShippingProfileId:number;profilesLoading:boolean;profilesError:string;onReloadProfiles?:()=>void;approved:boolean;preserveEdits?:boolean;wholeNumber?:boolean;onWholeNumber?:(value:boolean)=>void;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string,international:InternationalShippingRate[])=>Promise<void>;onApprovalChange:(ready:boolean)=>void}){
  ...
  const manualPriceEdit=useRef(preserveEdits);
  ...
  function changeCostGroupPrice(cost:number,cents:number){manualPriceEdit.current=true;const matching=variants.filter(item=>item.cost===cost),safeCents=Math.max(cents,cost),next={...prices};for(const item of matching)next[String(item.id)]=safeCents;onPrices(next);setRecommendationMessage(`✓ $${(safeCents/100).toFixed(2)} applied to all ${matching.length} ${matching.length===1?optionNoun:optionNouns} with a $${(cost/100).toFixed(2)} Printify cost.`)}
  function changeIndividualPrice(variant:ProductVariant,cents:number){manualPriceEdit.current=true;/* D404 - This never set the flag, so the recalculate effect could snap a hand-typed variant price back to the goal. */onPrices({...prices,[String(variant.id)]:Math.max(cents,variant.cost)});setRecommendationMessage(`✓ ${variant.title} now has its own price. The rest of its cost group was not changed.`)}
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      {profilesError&&<div className="shipping-api-note error"><b>Shipping profiles could not be loaded.</b><span>{profilesError}</span>{onReloadProfiles&&<button type="button" className="secondary-action" disabled={profilesLoading} onClick={onReloadProfiles}>Try loading shipping again</button>}</div>}
      {!profilesLoading&&!profilesError&&profiles.length===0&&<div className="shipping-api-note"><b>No shipping profiles in this Etsy shop yet.</b><span>Create a shipping profile in Etsy Shop Manager under Shipping settings, then reload your profiles here.</span>{onReloadProfiles&&<button type="button" className="secondary-action" onClick={onReloadProfiles}>Reload shipping profiles</button>}</div>}
      {selectedProfile&&<>{selectedProfileNeedsReview&&<div className="shipping-profile-family-warning" role="status"><b>Double-check this profile for {productName}.</b><span>Its name does not clearly match this product type. It has not been changed; confirm the buyer charges below before approving.</span></div>}{/* D232 · Three chips — "Etsy buyer charge", "Printify shipping cost — what you
      ...
      {!selectedProfile&&selectedProfileId>0&&!profilesLoading&&<p className="shipping-profile-missing" role="status">{/* D458 - this claimed a saved profile had been deleted from her shop and told her to choose another "below", while sitting below the picker itself. On a product she had just created there was no saved profile to lose: the id came from Printify and simply does not match anything on the Etsy shop. Say that, and point the right way. */}No matching Etsy shipping profile was found for this product. Pick one above and it will be remembered.</p>}
      ...
      {approved
        ?<p className="pricing-approved-state" role="status"><span aria-hidden="true">✓</span> Pricing target and shipping saved</p>
        :<button type="button" className="pricing-approval-button" disabled={!selectedProfile||customDirty} onClick={()=>onApprovalChange(true)}>{customDirty?"Save or discard your custom profile to continue":!selectedProfile?"Choose a shipping profile to continue":"Save pricing target and shipping"}</button>}
      </section>}
    </section>
  );
}
...
function friendlyUploadError(error: unknown) {
  ...
  if (/image could not be decoded|could not be read|invalidstateerror|source image could not be decoded/i.test(message)) return withReference("This file is visible but its image data cannot be read. Download it fully to your computer, then upload it again as a PNG or JPG.");
  ...
  return message || "This draft could not be created. Retry it when the batch finishes.";
}
...
export default function ListingFactoryApp() {
  ...
  */
  ...
  const [createBundleRequest,setCreateBundleRequest]=useState(0);
  ...
  const [productFormMode,setProductFormMode]=useState(false);
  ...
  const [pricingApprovalGroup,setPricingApprovalGroup]=useState("");
  ...
  const designArtworkReady=(file:DesignFile)=>Boolean(file.width&&file.height&&file.paddingStatus!=="checking"&&(file.artworkVersions||[]).every(artwork=>!artwork.originalUnavailable&&artwork.file?.size&&artwork.colorIds.length>0&&(artwork.productIds?.length??1)>0));
  const designsReady=useMemo(()=>files.filter(designArtworkReady).length,[files]);
  const designsPreparing=Math.max(0,files.length-designsReady);
  const designsFinished=files.length>0&&designsPreparing===0;
  const setupForwardReady=ready&&designsFinished&&!templateError&&!failedBundleNames().length;
  ...
  function normalizeColorIds(details:TemplateDetails|null|undefined,ids:number[]){
    const options=details?.colorOptions||[],canonical=new Set<number>();
    /* Printify can expose the same named colour through several option ids.
       The picker groups them visually, so the variant filter must retain every
       id in that group; keeping only the display id makes valid sizes appear to
       have no matching variant after a catalogue refresh. */
  ...
  function variantsFor(details:TemplateDetails|null|undefined,colorIds:number[],sizeIds:number[]){
    ...
    const byColor=!details?.colorOptions?.length?variants:(()=>{const selected=new Set(normalizeColorIds(details,colorIds));return variants.filter(variant=>variant.colorId==null||selected.has(variant.colorId))})();
  ...
  function publishBlockers(){
    ...
    if(batchHeldByAnotherTab)issues.push("This batch is open in another tab. Take over there or here before publishing, so the receipt is saved.");
    ...
    for(const draft of chosen){
      ...
      else if(draft.costReview?.required&&!draft.costReview.approved)issues.push(`Review the final prices for ${draft.title||draft.name}; Printify's finished production cost has not been approved.`);
    }
  ...
  function snapshotDraft(draft:DraftResult){
    const previews=draft.colorPreviewImageDetails||[],needed=new Set<string>();
    for(const color of templateDetails?.colorOptions||[]){
      const variants=printifyVariantIdsForColor(templateDetails?.variants||[],[color.id,...(color.ids||[])]);
      const src=printifyMockupForColor(previews,variants);if(src)needed.add(src);
    }
    const compactPreviews=needed.size?previews.filter(image=>needed.has(image.src)):previews.slice(0,50);
  ...
  const railInFinish=currentStage.label==="Finish";
  function bundleProductsReady(){
    /* D905 · Before private drafts exist, a bundle product only needs to be a
       real loaded Printify template with an enabled variant. Final prices and
       Etsy shipping are deliberately decided after Printify has rendered the
       design and returned the finished production costs. */
    ...
    return bundleRecipes.every((recipe,index)=>{
      ...
      return Boolean(product.enabledVariants);
    });
  }
  ...
  useEffect(()=>{
    if(restoringBatch||!activeRecipe||drafts.some(draft=>draft.status==="Created"))return;
  ...
  if(["review","finish"].includes(step)){if(!files.length)issues.push("Add at least one finished design.");if(files.some(file=>!file.width||!file.height||file.paddingStatus==="checking"))issues.push("Wait until every design finishes loading and checking.");}/* D298 · Prices and the shipping profile are both set on the PRODUCT page, but
     ...
     owns the decision. */
  if(step==="finish"&&!etsyShippingProfileId)issues.push("Choose the Etsy shipping profile after approving the finished prices.");
  if(step==="finish"){if(!complete)issues.push("Finish the Printify draft run first.");if(!drafts.some(draft=>draft.status==="Created"))issues.push("Create at least one Printify draft before continuing.");if(costReviewDrafts().some(draft=>!draft.costReview?.approved))issues.push("Approve the final prices calculated from the finished Printify drafts.");}return issues}
  ...
  /*
    D1449 - SAY WHICH PRINTIFY STORE THE DRAFTS ARE IN.

    Printify's links carry no store, so its interface opens whichever store
    the member last selected. A member with more than one store clicks
    through to "This listing isn't available in the selected store" on a
    draft that exists and is perfectly fine - and duplicating it fails the
    same way, because the duplicate lands in the wrong store too.
  ...
  */
  ...
  if(typeof window!=="undefined"&&!tabId.current)tabId.current=clientRandomId();
  ...
  useEffect(()=>{if(!connectAnotherOpen)return;const close=(event:KeyboardEvent)=>{if(event.key!=="Escape"||etsyConnecting)return;setConnectAnotherOpen(false);window.setTimeout(()=>connectAnotherOpener.current?.focus(),0)};window.addEventListener("keydown",close);return()=>window.removeEventListener("keydown",close)},[connectAnotherOpen,etsyConnecting]);
  ...
  async function chooseFiles(list: FileList | null) {
    ...
    const replacements=new Map<string,{file:File;previewUrl:string;artworkPreviewUrl:string;contentHash:string}>();
    ...
    for(const file of selected){
      ...
      if(missing){const artworkPreviewUrl=URL.createObjectURL(file);replacements.set(missing.id,{file,previewUrl:artworkPreviewUrl,artworkPreviewUrl,contentHash});existingHashes.add(contentHash);continue}
  ...
  async function removeDesign(id:string){if(draftRunInFlight.current)return;const removed=files.find(file=>file.id===id);if(!removed)return;if(drafts.length&&!await confirmAction({title:`Remove ${removed.name}?`,body:"Its existing Printify draft stays in Printify. This listing is removed from this batch.",confirmLabel:"Remove listing",destructive:true}))return;const next=files.filter(file=>file.id!==id);URL.revokeObjectURL(removed.previewUrl);for(const artwork of removed.artworkVersions||[])if(artwork.previewUrl)URL.revokeObjectURL(artwork.previewUrl);setFiles(next);setFileError("");setFileNotice(`${removed.name} was removed.`);setComplete(false);setDrafts([]);setProcessed(0);const batchId=batchIdRef.current;if(batchId){if(next.length){void saveBatchFiles(batchId,next.map(file=>file.file)).catch(()=>undefined);persistArtworkFiles(next)}else void clearBatchFiles(batchId)}}
  ...
  function recordListingSyncError(id:string,message:string){updateDesign(id,{etsyError:message});if(/Printify draft no longer exists/i.test(message))setDrafts(current=>current.map(draft=>draft.clientId===id?{...draft,status:"Failed",error:message}:draft))}
  ...
  async function selectRecipe(recipe:Recipe):Promise<TemplateDetails|null>{/* D301 · Colours and sizes were persisted per template, but the product
    ...
    /* D842 · Remember this product's flatlay on the recipe. The saved-product
       ...
       product details of their own to draw from. One write when the product is
       opened, instead of one Printify round trip per tile on every render. */
    const flatlay=pickProductPhoto(details);
    if(flatlay&&flatlay!==recipe.previewImage){
      setActiveRecipe(current=>current&&current.id===recipe.id?{...current,previewImage:flatlay}:current);
      void fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:recipe.id,name:recipe.name,templateUrl:recipe.templateUrl,previewImage:flatlay})}).catch(()=>undefined);
      window.dispatchEvent(new CustomEvent("goldie-recipe-photo",{detail:{recipeId:recipe.id,previewImage:flatlay}}));
    }
    const savedDescription=normalizeProductDescription(recipe.description),importedDescription=normalizeProductDescription(details.description);if(savedDescription)setDescription(savedDescription);else if(importedDescription){const updated={...recipe,description:importedDescription};setDescription(importedDescription);setActiveRecipe(updated);void fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:recipe.id,name:recipe.name,templateUrl:recipe.templateUrl,description:importedDescription})}).catch(()=>undefined)}return details}
  async function saveProductDefaults(change:Partial<Recipe>,key:string){if(!activeRecipe)return;const recipeId=activeRecipe.id;setSavingProductDefault(key);try{const response=await fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:activeRecipe.id,name:activeRecipe.name,templateUrl:activeRecipe.templateUrl,...change})});if(!response.ok)throw new Error("This product default could not be saved.");/* D463 - this used to merge into a copy of the recipe captured before the
...
setSavedRevision(current=>current+1);}catch(error){/* Automatic defaults are a convenience, not a reason to stop the seller's run with a modal. Explicit saves still report the failure. */if(key!=="auto-defaults")stopWith("This default was not saved.",[error instanceof Error?error.message:"Try again in a moment."])}finally{setSavingProductDefault("")}}
  ...
  async function useBundle(bundle:ProductBundle,recipeIds:string[]):Promise<boolean|"canceled">{
    ...
    if((files.length>0||drafts.length>0||complete)&&!await confirmAction({title:`Start “${bundle.name}” and clear this batch?`,body:"Your current designs and unfinished work will be removed. Your saved products and keyword banks are untouched.",confirmLabel:"Start this bundle",destructive:true}))return "canceled";
    ...
    const adopt=(recipe:Recipe,details:TemplateDetails)=>{
      ...
      const ids=normalizeColorIds(details,recipe.defaultColorIds||[]).filter(id=>available.has(id));
    ...
    return (recipe:Recipe,index:number):{label:string;tone:"ready"|"attention"|"advice"|"waiting"}=>{
      if(index===bundleIndex){
        ...
        if(step==="listing"){
          ...
          if(tagged<files.length)return {label:`${files.length-tagged} with fewer than 13 tags · optional`,tone:"advice"};
      ...
      if(step==="listing"){
        ...
        if(fullyTagged<summary.designs)return {label:`${summary.designs-fullyTagged} with fewer than 13 tags · optional`,tone:"advice"};
        return {label:"Titles and tags ready",tone:"ready"};
      }
  ...
  useEffect(()=>{
    ...
    void Promise.all(missing.map(async recipe=>{
      const outcome=await fetchWithDeadline("/api/printify",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productUrl:recipe.templateUrl,savedShippingProfileId:Number(recipe.etsyShippingProfileId)||0})},30000)
        ...
        .catch(()=>({error:"Printify could not be reached for this product."}));
  ...
  const [bundleCompletionRevision,setBundleCompletionRevision]=useState(0);
  ...
  useEffect(()=>{
    ...
    await Promise.all(wanted.map(async recipe=>{
      ...
      /* D627 · This returned null, so no summary was ever written for a member
         ...
         "Still reading the other products in this batch". It was not
  ...
  },[activeBundle,bundleRecipes,activeRecipe,bundleBatchIds,bundleCompletionRevision]);
  ...
  function productRows(recipe:Recipe,isActive:boolean):Array<{label:string;value:string;detail?:string;advice?:string;done:boolean;target?:string;task?:string;report?:boolean;optional?:boolean;pending?:boolean}>{
    ...
    const fullyTaggedCount=isActive
      ?files.filter(file=>file.tags.length>=13).length
      :(bundleMembers[recipe.id]?.designs||[]).filter(design=>(design.tags||[]).length>=13).length;
    ...
    return [
      ...
      {label:"Write titles and tags",value:started?(()=>{
        ...
        return `${counts.titled} of ${counts.designs} titles · tags added to ${counts.tagged} of ${counts.designs}`;
        ...
        done:started&&counts.designs>0&&counts.titled===counts.designs,advice:started&&counts.designs>0&&counts.titled===counts.designs&&fullyTaggedCount<counts.designs?`${counts.designs-fullyTaggedCount} of ${counts.designs} have fewer than 13 tags. This is optional and does not block Review batch.`:undefined,task:"titles"},
  ...
  function reviewListingSectionNav(design?:DesignFile){
    ...
    const movableListings=files.filter(candidate=>drafts.some(item=>item.clientId===candidate.id&&item.status==="Created"&&item.id));
    const movablePosition=movableListings.findIndex(candidate=>candidate.id===design.id);
    const currentPreview=draft.previewUrl||design.previewUrl;
    const editingAllListings=current==="title"||current==="description";
    return <>
      <aside className="review-listing-editor-nav" aria-label={editingAllListings?(current==="description"?"Edit descriptions for all listings":"Edit titles and tags for all listings"):`Edit listing ${position+1}`}>
        {editingAllListings?<div className="review-listing-current is-all-listings"><span><small>Editing all listings</small><b>{current==="description"?`${files.length} listing descriptions`:`${files.length} titles & tag sets`}</b></span></div>:<div className="review-listing-current"><span className="review-listing-thumb">{currentPreview?<img src={currentPreview} alt="" decoding="async"/>:<span aria-hidden="true">{position+1}</span>}</span><span><small>Editing listing {position+1} of {files.length}</small><b>{design.title.trim()||"Untitled"}</b></span></div>}
        {!editingAllListings&&files.length>1&&<div className="review-listing-picker">
          <button type="button" aria-label="Previous listing" disabled={movablePosition<=0} onClick={()=>move(movableListings[movablePosition-1])}>←</button>
          <details className="review-listing-chooser">
            <summary aria-label={`Choose listing. Currently listing ${position+1}`}><span>Choose listing</span><b>Listing {position+1}</b></summary>
            <div className="review-listing-choices">{files.map((candidate,index)=>{const available=movableListings.some(item=>item.id===candidate.id),candidateDraft=drafts.find(item=>item.clientId===candidate.id&&item.status==="Created"),preview=candidateDraft?.previewUrl||candidate.previewUrl,title=candidate.title.trim()||"Untitled";return <button type="button" key={candidate.id} disabled={!available} aria-current={candidate.id===design.id?"true":undefined} aria-label={`Open listing ${index+1}: ${title}`} onClick={event=>{move(candidate);event.currentTarget.closest("details")?.removeAttribute("open")}}><span className="review-listing-choice-thumb">{preview?<img src={preview} alt="" decoding="async"/>:<span aria-hidden="true">{index+1}</span>}</span><span><small>Listing {index+1}</small><b>{title}</b></span></button>})}</div>
          </details>
...
/* D567 - loading="lazy" on an image with no intrinsic size is a deadlock: the
  ...
  function designTaskRows(task:string,standing:(design:DesignFile)=>string,inner:(design:DesignFile)=>ReactNode,flags?:(design:DesignFile)=>ListingFlag[],only?:DesignFile,openAll=false){
  ...
  function descriptionFlags(design:DesignFile):ListingFlag[]{
    ...
    return design.descriptionOverride!==undefined?[{tone:"note",label:"Customized"}]:[];
  }
  ...
  function titlesLead(){
    ...
    return <div className="task-panel-lead compact-title-tools">
      ...
      {titleBuilderMode==="ai"?<div className="title-builder-pane">
        ...
        <KeywordBank compact initialId={manualKeywordBankId||activeRecipe?.keywordListId||""} onSelect={list=>setManualKeywordBankId(list?.id||"")} onAdd={addBatchKeyword} title="Keyword bank" copy="Choose phrases in title order."/>
        <div className="selected-batch-keywords"><div><b>Selected keywords</b>{batchKeywords.length>0&&<button onClick={clearBatchKeywords}>Clear all</button>}</div>{batchKeywords.length?<div className="selected-keyword-chips">{batchKeywords.map(keyword=><button key={keyword} onClick={()=>removeBatchKeyword(keyword)}>{keyword}<span>×</span></button>)}</div>:<p>No keywords selected yet.</p>}</div>
        {batchKeywords.length>0&&<div className="batch-title-preview"><b>Title preview</b><span>{batchKeywords.join(titleJoiner)}</span><small>{activeBundle?"Applied to this product’s listings.":"Applied to every listing."}</small></div>}
      </div>}
    </div>
  }
  ...
  function taskPanel(task:string){
    ...
    if(task==="draft-sizes"&&templateDetails){const focused=reviewEditing?drafts.find(draft=>draft.id===reviewEditing.id):undefined,axes=draftVariantAxes(focused);return <><ProductSizeSelector product={templateDetails} selected={axes.sizes} onChange={ids=>void syncDraftVariantChoices(axes.colors,ids,focused)} onRemember={()=>undefined} remembering={savingDraftVariants} remembered inCard scope="listing"/>{(draftVariantLimitError||draftVariantError)&&<p className="field-error" role="alert">{draftVariantLimitError||draftVariantError}</p>}</>}
    ...
    if(task==="draft-pricing"){
      ...
      if(!groups.length)return <div className="post-draft-pricing-panel" aria-label="Edit item prices"><p className="task-panel-empty">Printify is still calculating the finished product costs.</p></div>;
      const selected=selectedPriceGroup(groups,draftPriceGroupKey),group=selected.group;
      ...
      return <div className="post-draft-pricing-panel" aria-label="Edit item prices">{groups.length>1&&<div className="draft-price-group-nav" aria-label="Pricing group navigation"><button type="button" disabled={selected.index===0} onClick={()=>setDraftPriceGroupKey(groups[selected.index-1].key)}>Previous</button><b>Pricing group {selected.index+1} of {groups.length}</b><button type="button" disabled={selected.index===groups.length-1} onClick={()=>setDraftPriceGroupKey(groups[selected.index+1].key)}>Next</button></div>}<section className="editable-draft-pricing" key={group.key}>{groups.length>1&&<h3 className="draft-price-group-title">Prices for {group.drafts.length===1?"listing":"listings"} {group.drafts.map(draft=>drafts.findIndex(item=>item.id===draft.id)+1).join(", ")}</h3>}<PricingReview section="prices" variants={actualVariants} pricing={pricing} prices={groupPrices} productName={group.productName} profiles={etsyShippingProfiles} selectedProfileId={etsyShippingProfileId} templateShippingProfileId={Number(templateDetails?.shippingTemplateId)||0} profilesLoading={shippingProfilesLoading} profilesError={shippingProfilesError} onReloadProfiles={()=>void loadEtsyShippingProfiles()} approved={approved} preserveEdits={Boolean(group.drafts[0].priceEdits)} wholeNumber={activeRecipe?.wholeNumberPricing===true} onPricing={value=>{setPricing(value);setPricingApproved(false)}} onPrices={value=>{const ids=new Set(group.drafts.flatMap(draft=>draft.id?[draft.id]:[]));setDrafts(current=>updateDraftPriceEdits(current,ids,value));setPricingApproved(false)}} onSelectProfile={()=>undefined} onCreateProfile={createCustomShippingProfile} onApprovalChange={()=>undefined}/>{approved?<p className="pricing-approved-state" role="status"><span aria-hidden="true">✓</span> Prices saved</p>:<button type="button" className="save-draft-prices" disabled={savingDraftVariants||!verified||Boolean(pricingApprovalGroup)} onClick={()=>void approveActualPricingGroup(group,groupPrices)}>{savingDraftVariants?"Saving product choices…":pricingApprovalGroup===group.key?"Saving prices…":verified?"Save these prices":"Costs still loading"}</button>}</section></div>;
    }
      ...
      <div className="task-panel-body placement-review-grid">
        ...
        <ArtworkGrid items={visibleListings.filter(({draft})=>draft.status==="Created").map(({draft,design})=>{
          ...
          return {
            ...
            onSelect:!reviewEditing&&draft.id?(checked=>setSelectedPlacementDrafts(current=>checked?[...new Set([...current,draft.id!])]:current.filter(id=>id!==draft.id))):undefined,
          };
        })}/>
      </div>
    </>;
  ...
  function publishReports(){
    ...
    const mine={designs:reportFiles.length,titled:reportFiles.filter(file=>file.title.trim()).length,tagged:reportFiles.filter(file=>file.tags.length>0).length,fullyTagged:reportFiles.filter(file=>file.tags.length>=13).length,drafts:createdReportDrafts.length,described:Boolean(description.trim()),complete,published:Number(batchReceipt?.publishedCount||0),status:"",photos:createdReportDrafts.reduce((sum,draft)=>sum+(draft.id?(reportSelections[draft.id]?.length||0):0)+(reportFiles.find(file=>file.id===draft.clientId)?.sizeGuideName??sizeGuideName?1:0),0),mockups:Object.values(reportMockups).reduce((sum,count)=>sum+count,0)};
    const counts=mine;
    const started=true;
    ...
    const plural=(count:number,word:string)=>`${count} ${count===1?word:`${word}s`}`;
      ...
      const bundlePricingReady=pricingApproved&&createdReportDrafts.every(draft=>!draft.costReview?.required||draft.costReview.approved)&&(!activeBundle||bundleRecipes.length<2||bundleRecipes.every(recipe=>recipe.id===activeRecipe?.id?pricingApproved:(bundleApproved[recipe.id]??false)));
      return [
...
done:started&&counts.designs>0&&counts.titled===counts.designs,advice:started&&counts.designs>0&&counts.titled===counts.designs&&counts.fullyTagged<counts.designs?`${counts.designs-counts.fullyTagged} of ${counts.designs} have fewer than 13 tags. This is optional and does not block Review batch.`:undefined,report:true},
      ...
      <div className="factory-listing-grid">
        <div className="factory-form-card factory-listing-form">
          {/* D793 · Her design filenames run to "ChatGPT Image Aug 28, 2026,
              10_44_01 AM34-gigapixel-standard v2-4x.png", which wrapped the
              heading onto two lines and pushed the first field down the card.
  ...
  function stepProductCards(statusFor:(recipe:Recipe,index:number)=>{label:string;tone:"ready"|"attention"|"advice"|"waiting"},body:ReactNode,hidden=false,footer:ReactNode=null,showCards=true,header:ReactNode=null){
      ...
      {showCards&&list.map((recipe,index)=>{
        ...
        return <article className={`batch-product-card step-product-card ${open?"is-open":"is-closed"} ${status.tone==="ready"||status.tone==="advice"?"is-ready":"needs-setup"} ${many?"in-batch":""}`} key={recipe.id}>
          <header
            ...
            {photo?<img className="bundle-product-photo" src={photo} alt="" decoding="async"/>:<ProductGlyph title={product?.blueprintTitle||recipe.name}/>}
    ...
    return <div className={`batch-product-rows ${grouped?"has-draft-stages":""}`}>
      ...
      return <Fragment key={row.label}><div
          ...
          {open&&!switchingProduct&&<div className="step-product-body">{body}</div>}
  ...
  async function continueBundle(targetIndex=bundleIndex+1){
    const next=bundleRecipes[targetIndex];if(!activeBundle||!next||targetIndex===bundleIndex)return;
    ...
    await persistBatchNow(batchIdRef.current);
    /* D940 · The primary file is the listing concept and belongs on every
       product. Extra artwork is a print-area decision. Preserve it for the
       automatic bundle run, but stamp legacy versions with the product where
       they were created so they cannot silently spill onto the next product. */
    const outgoingProductId=activeRecipe?.id||"";
    const carriedFiles=files.map(file=>({...file,id:crypto.randomUUID(),previewUrl:URL.createObjectURL(file.file),title:"",tags:[],blurb:undefined,descriptionOverride:undefined,sizeGuideName:undefined,etsy:undefined,etsyError:"",artworkVersions:(file.artworkVersions||[]).map(artwork=>artwork.productIds?.length?artwork:{...artwork,ownerProductId:outgoingProductId,productIds:outgoingProductId?[outgoingProductId]:[]})}));
  ...
  async function changeProduct(){
    if(draftRunInFlight.current)return false;
  ...
  function missingPublishFields(){/* D626 - `files` is the open product's designs, so titles, tags and Etsy details on every other product in the bundle went unchecked and could publish incomplete. */const chosen=selectedPublishDrafts(),clientIds=new Set(chosen.map(draft=>draft.clientId)),chosenFiles=bundlePublishFiles().filter(file=>clientIds.has(file.id)),missing:string[]=[];if(!chosen.length)missing.push("Select at least one successful listing");
    ...
    if(bundleProductsStillReading().length)missing.push("Still reading the other products in this batch");if(chosenFiles.some(file=>!file.title.trim()))missing.push("Titles");if(chosenFiles.some(file=>!file.tags.length))missing.push("Tags");if(!description.trim())missing.push("Permanent product description");if(chosenFiles.some(file=>!etsyRequiredComplete(file.etsy)))missing.push("Etsy details");if(chosenFiles.some(file=>personalizationProblem(file.etsy)))missing.push("Personalization settings");if(chosen.length&&!allCreatedListingsHaveImages(chosen))missing.push("At least one image on every selected listing");return missing}
  ...
  async function monitorPublishJob(jobId:string,resuming=false){
    ...
    setPublishing(true);setPublishMessage(resuming?"Resuming the queued batch…":"Publishing your listings…");
    ...
    }catch(error){setPublishMessage(error instanceof Error?error.message:"The queued batch could not be resumed.")}finally{setPublishing(false)}
  }
  ...
  function pricingForDraft(draft:DraftResult){
    if(drafts.some(item=>item.id===draft.id))return pricing;
    const member=Object.values(bundleMembers).find(entry=>entry.drafts.some(item=>item.id===draft.id));
    const recipe=member?bundleRecipes.find(item=>item.id===member.recipeId):undefined;
    return member?(bundlePricing[member.recipeId]||{...pricing,targetProfit:Number(recipe?.defaultProfitTarget)||DEFAULT_PRICING.targetProfit}):pricing;
  }
  function costReviewGroups(){
    const groups=new Map<string,{key:string;productName:string;drafts:DraftResult[];review:DraftCostReview}>();
    for(const draft of costReviewDrafts()){
      const review=draft.costReview!;
      const member=Object.values(bundleMembers).find(entry=>entry.drafts.some(item=>item.id===draft.id));
      const ownerId=member?.recipeId||activeRecipe?.id||"product";
      const productName=draft.productName||member?.productName||activeRecipe?.name||templateDetails?.blueprintTitle||"Product";
      const signature=review.variants.filter(variant=>variant.isEnabled).map(variant=>`${variant.id}:${variant.cost}`).sort().join("|");
      const key=`${ownerId}:${signature}`;
      const existing=groups.get(key);
      if(existing)existing.drafts.push(draft);else groups.set(key,{key,productName,drafts:[draft],review});
    }
  ...
  async function publishAll(){
    ...
    publishInFlight.current=true;setPublishConfirmOpen(false);setPublishing(true);setPublishFailures([]);setPublishMessage(`Queuing ${ids.length} selected ${ids.length===1?"listing":"listings"}…`);setBatchReceipt(null);
    try{
      const response=await fetch("/api/printify/drafts/publish",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({productIds:ids,/* D872 · The run she authorised, so the queue makes one job for the whole bundle rather than one per product record. */runBatchId:runIdRef.current||batchIdRef.current,printifyImageIndices,printifyImageSelections,etsyShippingProfileId,byProduct})}),payload=await response.json() as {job?:{id:string;status:string;total:number;completed:number;failed:number;queued:number;processing:number;finished:Array<{etsyListingId:number;url:string}>;failures?:Array<{productId:string;error:string}>;budget?:{remaining:number}};error?:string};if(!response.ok||!payload.job)throw new Error(payload.error||"The batch could not be queued.");
  ...
  async function disconnectEtsy(){
    if(!await confirmAction({title:"Disconnect this Etsy shop?",body:"Listing preparation for this shop is unavailable until you reconnect it. Any other shop you have connected stays connected, and your existing Etsy listings are not affected.",confirmLabel:"Disconnect",cancelLabel:"Keep connected"}))return;
    setEtsyError("");
    try{
  ...
  function etsyConnectionRow(withHint:boolean){
    if(checkingEtsyConnection)return <div className="connection-row" role="status"><b>Checking Etsy connection…</b></div>;
    return <div className={`connection-row etsy-connection service-row ${etsyConnected?"connected":""}`}>
      ...
      <div><b>{etsyConnected?"Etsy connected":"Etsy"}</b>
        ...
        <small>{etsyConnected?"Connected and verified.":"Required before preparing your listings."}</small>
      </div>
      {etsyConnected
        ? <>{/* D836 · Without this there is no way to accumulate the shops the
            ...
            <button className="secondary-action add-shop-link" aria-busy={etsyConnecting} disabled={etsyConnecting}
              onClick={event=>{connectAnotherOpener.current=event.currentTarget;setConnectAnotherOpen(true)}}>Connect a different Etsy shop</button>
...
setPricingApproved(recipeCarriesApprovedPricing({defaultProfitTarget:activeRecipe?.defaultProfitTarget,etsyShippingProfileId:activeRecipe?.etsyShippingProfileId})); return result.product;
  ...
  async function rememberProductSizes(){if(!activeRecipe||!selectedSizeIds.length)return;setRememberingSizes(true);try{const updated={...activeRecipe,defaultSizeIds:selectedSizeIds};const response=await fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:activeRecipe.id,name:activeRecipe.name,templateUrl:activeRecipe.templateUrl,defaultSizeIds:selectedSizeIds})});if(!response.ok)throw new Error("These size defaults could not be saved.");setActiveRecipe(updated);setSizesRemembered(true)}catch(error){stopWith("These size defaults were not saved.",[error instanceof Error?error.message:"Try again in a moment."])}finally{setRememberingSizes(false)}}

  function persistArtworkFiles(next:DesignFile[]){const batchId=batchIdRef.current;if(!batchId)return;const assets=Object.fromEntries(next.flatMap(design=>(design.artworkVersions||[]).filter(artwork=>!artwork.originalUnavailable&&artwork.file?.size).map(artwork=>[`${design.id}:${artwork.id}`,artwork.file])));void saveBatchArtworkAssets(batchId,assets)}

  async function inspectArtworkVersion(file:File,side:string):Promise<ArtworkVersion>{
    if(!/\.(png|jpe?g)$/i.test(file.name))throw new Error("Choose a PNG or JPG artwork file.");
    if(file.size>MAX_FILE_BYTES)throw new Error(oversizedFileMessage(file.name,file.size));
    const source=await createImageBitmap(file),width=source.width,height=source.height,sampleWidth=Math.min(512,width),sampleHeight=Math.max(1,Math.round(height*(sampleWidth/width))),canvas=document.createElement("canvas");canvas.width=sampleWidth;canvas.height=sampleHeight;const context=canvas.getContext("2d",{willReadFrequently:true})!;context.drawImage(source,0,0,sampleWidth,sampleHeight);source.close();const pixels=context.getImageData(0,0,sampleWidth,sampleHeight).data;let left=sampleWidth,top=sampleHeight,right=-1,bottom=-1,hasTransparency=false;for(let y=0;y<sampleHeight;y++)for(let x=0;x<sampleWidth;x++){const alpha=pixels[(y*sampleWidth+x)*4+3];if(alpha<250)hasTransparency=true;if(alpha>8){left=Math.min(left,x);right=Math.max(right,x);top=Math.min(top,y);bottom=Math.max(bottom,y)}}const visibleBounds=right<0?{left:0,top:0,right:1,bottom:1}:{left:left/sampleWidth,top:top/sampleHeight,right:(right+1)/sampleWidth,bottom:(bottom+1)/sampleHeight};const trimmed=visibleBounds.left>.015||visibleBounds.top>.015||visibleBounds.right<.985||visibleBounds.bottom<.985;
    return {id:crypto.randomUUID(),name:file.name,size:file.size,file,previewUrl:URL.createObjectURL(file),side,colorIds:[...selectedColorIds],ownerProductId:activeRecipe?.id,productIds:activeRecipe?.id?[activeRecipe.id]:[],width,height,visibleBounds,hasTransparency,paddingStatus:trimmed?"trimmed":"full"};
  }

  async function addArtworkVersion(designId:string,side:string,list:FileList|null){if(draftRunInFlight.current)return;const file=list?.[0];if(!file)return;try{const artwork=await inspectArtworkVersion(file,side);setFiles(current=>{const next=current.map(design=>design.id===designId?{...design,artworkVersions:[...(design.artworkVersions||[]),artwork]}:design);persistArtworkFiles(next);return next});setComplete(false);setDrafts([])}catch(error){setFileError(error instanceof Error?error.message:"That artwork version could not be added.")}}
  function toggleArtworkColor(designId:string,artworkId:string,colorId:number){setFiles(current=>{const design=current.find(item=>item.id===designId),target=design?.artworkVersions?.find(item=>item.id===artworkId);if(!design||!target)return current;const choosing=!target.colorIds.includes(colorId);const next=current.map(item=>item.id!==designId?item:{...item,artworkVersions:(item.artworkVersions||[]).map(artwork=>artwork.side!==target.side?artwork:artwork.id===artworkId?{...artwork,colorIds:choosing?[...artwork.colorIds,colorId]:artwork.colorIds.filter(id=>id!==colorId)}:choosing?{...artwork,colorIds:artwork.colorIds.filter(id=>id!==colorId)}:artwork)});persistArtworkFiles(next);return next});setComplete(false);setDrafts([])}
  function assignPrimaryArtwork(designId:string,side:string,colorId:number,artworkId:string){setFiles(current=>{const next=current.map(design=>design.id!==designId?design:{...design,artworkVersions:assignSideColor(design.artworkVersions||[],side,colorId,artworkId)});persistArtworkFiles(next);return next});setComplete(false);setDrafts([])}
  function toggleArtworkProduct(designId:string,artworkId:string,productId:string){setFiles(current=>{const next=current.map(design=>design.id!==designId?design:{...design,artworkVersions:(design.artworkVersions||[]).map(artwork=>{if(artwork.id!==artworkId)return artwork;const owned=artwork.productIds?.length?artwork.productIds:(artwork.ownerProductId?[artwork.ownerProductId]:activeRecipe?.id?[activeRecipe.id]:[]),included=owned.includes(productId);return {...artwork,productIds:included?owned.filter(id=>id!==productId):[...new Set([...owned,productId])]}})});persistArtworkFiles(next);return next});setComplete(false);setDrafts([])}
  function removeArtworkVersion(designId:string,artworkId:string){setFiles(current=>{const removed=current.find(item=>item.id===designId)?.artworkVersions?.find(item=>item.id===artworkId);if(removed?.previewUrl)URL.revokeObjectURL(removed.previewUrl);const next=current.map(design=>design.id===designId?{...design,artworkVersions:(design.artworkVersions||[]).filter(artwork=>artwork.id!==artworkId)}:design);persistArtworkFiles(next);return next});setComplete(false);setDrafts([])}

  async function preparedUpload(design: DesignFile|ArtworkVersion) {
  ...
  async function runDrafts(targetFiles: DesignFile[], keepSuccessful = false, alreadyAdmitted = false) {
    ...
    try {
      ...
      if(createdNow>0){
        ...
        setFinishPhase("details");void goToStep("designs",false,true);
      }else{
        setComplete(false);
        stopWith(
  ...
  async function updateDraftColorArtwork(draft:DraftResult,color:ProductColor,list:FileList|null,reset=false){
    ...
    const side=primaryPrintSide(orderedPrintSides(templateDetails.printPositions));
  ...
  function bundleListingDetailsIssues(){
    if(!activeBundle||bundleRecipes.length<2)return [];
    const issues:string[]=[];
    for(const recipe of bundleRecipes){
      const designs=recipe.id===activeRecipe?.id?files:bundleMembers[recipe.id]?.designs;
      if(!designs?.length){issues.push(`Open ${recipe.name} to check its listing details.`);continue}
      if(designs.some(file=>!file.title.trim()||file.titleError))issues.push(`Finish the titles for ${recipe.name}.`);
      else if(designs.some(file=>!etsyRequiredComplete(file.etsy)||personalizationProblem(file.etsy)))issues.push(`Finish the Etsy details for ${recipe.name}.`);
    }
    return issues;
  }
  async function saveAllEtsyDetails(){
    const bundleIssues=bundleListingDetailsIssues();if(bundleIssues.length)return void stopWith("Finish every product’s listing details.",bundleIssues);
  ...
  useEffect(()=>{
    ...
    if(complete){
      if(bundleIndex+1>=bundleRecipes.length){
        ...
        void persistBatchNow(batchIdRef.current).catch(error=>stopWith("The drafts exist in Printify, but this batch needs to be saved again.",[error instanceof Error?error.message:"Try Save to Batch History."])).finally(()=>{bundleFinishing.current=false;setBundleRun(null)});
        return
      }
  ...
  const reviewEditorHero=reviewEditing?.section==="title"
    ?{eyebrow:"STEP 4 OF 4 · REVIEW",title:"Edit titles and tags",copy:"Update every listing below, then return to Review."}
    :reviewEditing?.section==="description"
      ?{eyebrow:"STEP 4 OF 4 · REVIEW",title:"Edit descriptions",copy:"Update the shared description or any listing below, then return to Review."}
      :{eyebrow:"STEP 4 OF 4 · REVIEW",title:"Edit this listing",copy:"Update any section below, then return to Review."};
  ...
  const workflowHero = {
    ...
    setup: templateDetails&&productSelected&&!showProductLibrary
      ...
      ? { eyebrow: "STEP 1 OF 4", title: files.length?"Review your product and designs":"Add your designs", copy: "" }
      : !listingRunMode
        ? { eyebrow: "STEP 1 OF 4", title: "Start new listings", copy: "Choose one listing or a batch." }
        : listingRunMode==="single"
          ? { eyebrow: "STEP 1 OF 4", title: "Choose your product", copy: "Select the saved product for this listing." }
          : { eyebrow: "STEP 1 OF 4", title: "Choose products", copy: "Select a saved product or product bundle for this batch." },
    designs: complete
      ? reviewEditing
        ? reviewEditorHero
        : { eyebrow: "STEP 2 OF 4", title: "Finish your Printify drafts", copy: "Check artwork, colors, sizes, pricing, shipping, and listing photos." }
      : { eyebrow: "STEP 2 OF 4", title: "Create Printify drafts", copy: "Check the products and designs below, then create the drafts." },
    review: complete?{eyebrow:"STEP 2 OF 4",title:"Your Printify drafts are created",copy:"Continue with the listing text for these saved drafts."}:{ eyebrow: "STEP 2 OF 4", title: "Create Printify drafts", copy: "Review the plan, then create the private drafts." },
    finish: finishPhase==="details" ? (reviewEditing?reviewEditorHero:{eyebrow:"STEP 3 OF 4",title:"Finish your listing details",copy:"Review titles, tags, descriptions, and Etsy details."}) : finishPhase==="etsy" ? {eyebrow:"STEP 3 OF 4",title:"Review Etsy details",copy:"Choose the category, attributes, and personalization for each listing."} : { eyebrow: "STEP 4 OF 4", title: "Review your listings", copy: bundleProductsStillReading().length||!draftAvailabilitySettled?"Checking your saved drafts.":etsyDraftTransferState==="complete"?"Your Etsy drafts were created and verified.":etsyDraftTransferState==="working"?"Your Etsy drafts are being created and checked.":handoffBlockers().length?"Fix the missing items shown on the listing cards.":"Everything is ready. Save the batch to Etsy Drafts." },
  }[workflowStep];
  ...
  return (
    <main className="app-shell" data-product-selected={templateDetails?"true":"false"} data-product-library={showProductLibrary?"true":"false"}>
      ...
      <header className="topbar">
        ...
        <div className="approved-sidebar-footer"><a className="approved-usage" href="/usage"><b>Listings used</b><span>{sidebarUsage?`${sidebarUsage.used.toLocaleString()} / ${sidebarUsage.limit.toLocaleString()} listings`:sidebarUsageFailed?"Couldn't load — reopen to retry":"Loading usage…"}</span><div className="approved-usage-track" aria-hidden="true"><i style={{width:sidebarUsage?`${Math.min(100,sidebarUsage.used/sidebarUsage.limit*100)}%`:"0%"}} /></div></a>{listingGoal&&<a className="listing-goal-side" href="/goals"><span className="listing-goal-caption">Your {listingGoal.period}ly goal</span><b>{goalDaysError?"Progress unavailable":goalDaysLoaded?`${goalDone} of ${listingGoal.target} drafts ready`:"Loading progress…"}</b><span className="listing-goal-note">{goalDaysError?"Try again shortly.":"You set this target in Goals."}</span>{goalDaysLoaded&&<span className="listing-goal-track" aria-hidden="true"><i style={{width:`${Math.min(100,Math.round((goalDone/Math.max(1,listingGoal.target))*100))}%`}}/></span>}</a>}{/* D357 · "Powered by Goldie AI" is the widest line in the sidebar, so it sets
          ...
          <div className="factory-top-right">
            ...
            <div className="factory-account-wrap">
              ...
              {accountMenuOpen&&<div className="factory-account-menu open" role="menu">
                {etsyShops.length>0&&<div className="factory-account-shops" role="group" aria-label="Etsy shop">
                ...
                <a role="menuitem" href="/usage" onClick={event=>guardNavigation(event,"/usage")}>Usage and limits</a>
      ...
      {!returningHome&&<section className={`workspace ${complete&&workflowStep==="designs"?"mockup-workspace":""}`}>
        ...
        <div className={`steps-column ${workflowStep}-column`} data-restoring={restoringBatch?"true":undefined} inert={restoringBatch}>
          ...
          {workflowStep==="setup"&&!listingRunMode&&<section className="listing-mode-chooser" aria-labelledby="listing-mode-title">
            ...
            <div><button type="button" onClick={()=>setListingRunMode("single")}><strong>Individual listing</strong><span>Choose one product and upload one design.</span><i aria-hidden="true">→</i></button>
            <button type="button" onClick={()=>setListingRunMode("batch")}><strong>Batch of listings</strong><span>Use one or more products and upload several designs.</span><i aria-hidden="true">→</i></button></div>

...
function ProductSizeSelector({product,selected,onChange,onRemember,remembering,remembered,inCard}:{product:TemplateDetails;selected:number[];onChange:(ids:number[])=>void;onRemember:()=>void;remembering:boolean;remembered:boolean;inCard?:boolean}){
  ...
  return <section className="product-size-selector" aria-label={`Choose sizes for ${product.blueprintTitle}`}>
    {inCard?<p className="panel-help">{remembered?"From your last batch — change any.":"These changes apply to this batch unless you save them as the product default."}</p>:<div className="size-selector-head"><div><p className="mini-label">SIZES FOR THIS BATCH</p><h3>Sizes</h3><span>{remembered?"From your last batch — change any.":"These changes apply to this batch unless you save them as the product default."}</span></div><b>{selected.length} selected</b></div>}
...
function PricingReview({variants,pricing,prices,productName,profiles,selectedProfileId,profilesLoading,profilesError,approved,onPricing,onPrices,onSelectProfile,onCreateProfile,onApprovalChange}:{variants:ProductVariant[];pricing:Pricing;prices:Record<string,number>;productName:string;profiles:EtsyShippingProfile[];selectedProfileId:number;profilesLoading:boolean;profilesError:string;approved:boolean;onPricing:(value:Pricing)=>void;onPrices:(value:Record<string,number>)=>void;onSelectProfile:(id:number)=>void;onCreateProfile:(baseId:number,charge:number,additional:number,title:string,international:InternationalShippingRate[])=>Promise<void>;onApprovalChange:(ready:boolean)=>void}){
  ...
  return (
    <section className={"variant-pricing "+(approved?"approved":"")}>
      ...
      <button type="button" className={`pricing-approval-button ${approved?"approved":""}`} disabled={!selectedProfile||customDirty} onClick={()=>onApprovalChange(true)}>{approved?"✓ Prices and shipping approved":customDirty?"Save or discard your custom profile to continue":!selectedProfile?"Choose a shipping profile to continue":"Approve prices and shipping"}</button>
      </section>
    </section>
  );
}
...
export default function ListingFactoryApp() {
  ...
  /* D297 · Saving used to end by clearing the spinner and nothing else, so a
     click that worked looked identical to a click that never registered. This
     holds the confirmation long enough to be seen. */
  const [savedProductDefault,setSavedProductDefault]=useState("");
  ...
  useEffect(()=>{
    if(productRestoreAttempted.current||restoringBatch||activeRecipe||signedIn!==true)return;
    if(new URL(window.location.href).searchParams.get("batch"))return;
    let remembered="";
    try{remembered=window.localStorage.getItem("goldie-active-recipe")||""}catch{/* private mode */}
  ...
  },[restoringBatch,activeRecipe,signedIn]);
  ...
  function clearCurrentBatch(clearProduct=true,preserveSavedBatch=false){
    ...
    if(clearProduct){try{window.localStorage.removeItem("goldie-active-recipe")}catch{/* private mode */}}
  ...
  async function saveProductDefaults(change:Partial<Recipe>,key:string){if(!activeRecipe)return;setSavingProductDefault(key);try{const updated={...activeRecipe,...change};const response=await fetch("/api/product-recipes",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(updated)});if(!response.ok)throw new Error("Goldie could not save this product default.");setActiveRecipe(updated);setSavedProductDefault(key);window.setTimeout(()=>setSavedProductDefault(current=>current===key?"":current),2600)}catch(error){stopWith("This default was not saved.",[error instanceof Error?error.message:"Try again in a moment."])}finally{setSavingProductDefault("")}}

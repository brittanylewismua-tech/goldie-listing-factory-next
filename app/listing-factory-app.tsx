...
import { assignFrontColor, frontAssignmentFor } from "./artwork-assignment";
...
type ArtworkVersion = { id:string;name:string;size:number;file:File;previewUrl:string;side:"front"|"back";colorIds:number[];originalUnavailable?:boolean;width?:number;height?:number;visibleBounds?:VisibleBounds;hasTransparency?:boolean;paddingStatus?:"checking"|"trimmed"|"full" };
...
setPricingApproved(recipeCarriesApprovedPricing({defaultProfitTarget:activeRecipe?.defaultProfitTarget,etsyShippingProfileId:activeRecipe?.etsyShippingProfileId})); return result.product;
  ...
  async function inspectArtworkVersion(file:File,side:"front"|"back"):Promise<ArtworkVersion>{
  ...
  function assignFrontArtwork(designId:string,colorId:number,artworkId:string){setFiles(current=>{const next=current.map(design=>design.id!==designId?design:{...design,artworkVersions:assignFrontColor(design.artworkVersions||[],colorId,artworkId)});persistArtworkFiles(next);return next});setComplete(false);setDrafts([])}
      ...
      try {
        ...
        for (let pipelineAttempt = 1; pipelineAttempt <= 3; pipelineAttempt += 1) {
          ...
          try {
            ...
            const assignedFrontColors=new Set(versions.filter(artwork=>artwork.side==="front").flatMap(artwork=>artwork.colorIds));
            const primaryVariantIds=variants.filter(variant=>variant.colorId==null||!assignedFrontColors.has(variant.colorId)).map(variant=>variant.id);
            const artworkAssignments=[...(primaryVariantIds.length?[{position:"front",variantIds:primaryVariantIds,artworkKey:"primary",bounds:design.visibleBounds,maxPlacementScale:isRigidPaperProduct(templateDetails)?1:undefined}]:[]),...versions.map(artwork=>({position:artwork.side,variantIds:variants.filter(variant=>variant.colorId!=null&&artwork.colorIds.includes(variant.colorId)).map(variant=>variant.id),artworkKey:artwork.id,bounds:artwork.visibleBounds,maxPlacementScale:isRigidPaperProduct(templateDetails)?1:undefined})).filter(assignment=>assignment.variantIds.length)];
            ...
            const summary:ArtworkSummary={front:[{name:design.name,colors:(templateDetails?.colorOptions||[]).filter(color=>selectedColorIds.includes(color.id)&&!assignedFrontColors.has(color.id)).map(color=>color.title)}],back:[]};
        ...
        <div className={`steps-column ${workflowStep}-column`}>
          ...
          <article className={`step-card designs-step workflow-panel ${workflowStep==="setup"?"batch-design-drop":""} ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="setup"&&Boolean(templateDetails)&&productSelected&&!failedBundleNames().length)?"active-panel":"hidden-panel"}`}>{/* D238 · Choosing the mockup SET lived on Product while the mockups it controls are generated here on Images. Same setting, two pages — the exact split that caused the keyword-bank and shipping duplication. */}
            ...
            <div className="step-content">
              ...
              {files.length>0&&(workflowStep==="setup"||workflowStep==="designs")&&<div className="design-upload-review" aria-label="Review uploaded designs">{files.map(file=>{const colors=(templateDetails?.colorOptions||[]).filter(color=>selectedColorIds.includes(color.id)),positions=templateDetails?.printPositions||[],frontReady=positions.some(position=>/front|chest/i.test(position)),backReady=frontReady&&positions.some(position=>/back/i.test(position)),frontVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side==="front"),backVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side==="back");return <article className="design-artwork-card" key={file.id}>
                <div className="design-artwork-primary"><UploadedDesignPreview src={file.previewUrl} name={file.name}/><div><b title={file.name}>{file.name}</b><small>{file.width&&file.height?`${file.width} × ${file.height}px`:"Checking dimensions…"}</small><em>Primary front artwork</em></div><button type="button" className="artwork-remove-action" onClick={()=>removeDesign(file.id)} aria-label={`Remove ${file.name}`}>Remove</button></div>
                {((colors.length>1&&frontReady)||backReady)&&<div className="artwork-version-tools"><b>Artwork and print locations</b><small>Assign different artwork to specific garment colors, or add a back print where the product supports one.</small><div>{colors.length>1&&frontReady?<label className="secondary-action" role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.currentTarget.querySelector("input")?.click()}}}>＋ Use a different design on some colors<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"front",event.target.files);event.target.value=""}}/></label>:null}{backReady?<label className="secondary-action" role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.currentTarget.querySelector("input")?.click()}}}>＋ Add a back print<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"back",event.target.files);event.target.value=""}}/></label>:frontReady?<span className="print-area-unavailable">This saved Printify product does not currently expose a back print area.</span>:null}</div></div>}
                {frontVersions.length>0&&colors.length>0?<section className="artwork-assignment-matrix"><div><b>Front artwork by garment color</b><small>Every selected color gets exactly one front design.</small></div>{colors.map(color=><label key={color.id}><span><i style={{background:color.swatch||"#ddd"}}/>{color.title}</span><select aria-label={`Front artwork for ${color.title}`} value={frontAssignmentFor(frontVersions,color.id)} onChange={event=>assignFrontArtwork(file.id,color.id,event.target.value)}><option value="primary">Primary · {file.name}</option>{frontVersions.map(artwork=><option value={artwork.id} key={artwork.id}>{artwork.name}</option>)}</select></label>)}</section>:null}
                {frontVersions.map(artwork=><section className="artwork-version compact" key={artwork.id}><img src={artwork.previewUrl} alt=""/><div><b>Alternate front artwork</b><small>{artwork.name}</small><em>{artwork.colorIds.length?`${artwork.colorIds.length} garment ${artwork.colorIds.length===1?"color":"colors"} assigned`:"Assign this file in the color table above, or remove it."}</em></div><button type="button" onClick={()=>removeArtworkVersion(file.id,artwork.id)} aria-label={`Remove ${artwork.name}`}>Remove</button></section>)}
                {backVersions.map(artwork=><section className="artwork-version" key={artwork.id}><img src={artwork.previewUrl} alt=""/><div><b>Back print</b><small>{artwork.name}</small>{!artwork.colorIds.length&&<em className="artwork-color-required">Choose at least one garment color for this back print.</em>}{colors.length?<fieldset><legend>Print this artwork on the back of</legend>{colors.map(color=><button type="button" key={color.id} className={artwork.colorIds.includes(color.id)?"selected":""} aria-pressed={artwork.colorIds.includes(color.id)} onClick={()=>toggleArtworkColor(file.id,artwork.id,color.id)}><i style={{background:color.swatch||"#ddd"}}/>{color.title}</button>)}</fieldset>:null}</div><button type="button" onClick={()=>removeArtworkVersion(file.id,artwork.id)} aria-label={`Remove ${artwork.name}`}>Remove</button></section>)}
              </article>})}</div>}

  ...
  function reviewListingSectionNav(design?:DesignFile){
    ...
    return <>
      <aside className="review-listing-editor-nav" aria-label={`Edit listing ${position+1}`}>
        <div className="review-listing-current"><span className="review-listing-thumb">{currentPreview?<img src={currentPreview} alt="" decoding="async"/>:<span aria-hidden="true">{position+1}</span>}</span><span><small>Editing listing {position+1} of {files.length}</small><b>{design.title.trim()||"Untitled"}</b></span></div>
        {files.length>1&&<div className="review-listing-picker"><button type="button" aria-label="Previous listing" disabled={movablePosition<=0} onClick={()=>move(movableListings[movablePosition-1])}>←</button><label><span>Jump to listing</span><select aria-label="Jump to listing" value={design.id} onChange={event=>{const next=files.find(candidate=>candidate.id===event.target.value);if(next)move(next)}}>{files.map((candidate,index)=>{const available=movableListings.some(item=>item.id===candidate.id);return <option key={candidate.id} value={candidate.id} disabled={!available}>{`Listing ${index+1}${candidate.title.trim()?` — ${candidate.title.trim()}`:""}`}</option>})}</select></label><button type="button" aria-label="Next listing" disabled={movablePosition<0||movablePosition>=movableListings.length-1} onClick={()=>move(movableListings[movablePosition+1])}>→</button></div>}

...
export default function ListingFactoryApp() {
  ...
  if(["review","finish"].includes(step)){if(!files.length)issues.push("Add at least one finished design.");if(files.some(file=>!file.width||!file.height||file.paddingStatus==="checking"))issues.push("Wait until every design finishes loading and checking.");}/* D298 · Prices and the shipping profile are both set on the PRODUCT page, but
     ...
     owns the decision. */
  if(["designs","review","finish"].includes(step)){if(!etsyShippingProfileId)issues.push("Choose the Etsy shipping profile for this batch.");}
          ...
          <article className={`step-card designs-step workflow-panel ${workflowStep==="setup"?"batch-design-drop":""} ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"?"active-panel":"hidden-panel"}`}>{/* D238 · Choosing the mockup SET lived on Product while the mockups it controls are generated here on Images. Same setting, two pages — the exact split that caused the keyword-bank and shipping duplication. */}
            ...
            <div className="step-content">
              ...
              {files.length>0&&(workflowStep==="setup"||workflowStep==="designs")&&<div className="design-upload-review" aria-label="Review uploaded designs">{files.map(file=>{const colors=(templateDetails?.colorOptions||[]).filter(color=>selectedColorIds.includes(color.id)),positions=templateDetails?.printPositions||[],frontReady=positions.some(position=>/front|chest/i.test(position)),backReady=frontReady&&positions.some(position=>/back/i.test(position)),frontVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side==="front"),backVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side==="back");return <article className="design-artwork-card" key={file.id}>
                ...
                {((colors.length>1&&frontReady)||backReady)&&<div className="artwork-version-tools"><b>Artwork and print locations</b><small>Your product colors are already fixed. Assign the artwork each garment color should receive, then add a back print only where you want one.</small><div><button type="button" className="text-action" onClick={()=>goToStep("setup",false,true)}>Change product colors</button>{colors.length>1&&frontReady?<label className="secondary-action">＋ Add another front colorway<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"front",event.target.files);event.target.value=""}}/></label>:null}{backReady?<label className="secondary-action">＋ Add a back print<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,"back",event.target.files);event.target.value=""}}/></label>:frontReady?<span className="print-area-unavailable">This saved Printify product does not currently expose a back print area.</span>:null}</div></div>}

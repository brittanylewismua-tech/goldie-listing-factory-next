...
/* D567 - loading="lazy" on an image with no intrinsic size is a deadlock: the
  ...
  async function connectEtsy(){setEtsyConnecting(true);setEtsyError("");try{const response=await fetchWithDeadline("/api/etsy",{method:"POST"},25000),result=await response.json() as {authorizeUrl?:string;error?:string};if(!response.ok||!result.authorizeUrl)throw new Error(result.error||"Etsy connection could not start.");window.location.href=result.authorizeUrl}catch(error){setEtsyError(error instanceof Error?error.message:"Etsy connection could not start.");setEtsyConnecting(false)}}

...
type ArtworkVersion = { id:string;name:string;size:number;file:File;previewUrl:string;side:string;colorIds:number[];originalUnavailable?:boolean;width?:number;height?:number;visibleBounds?:VisibleBounds;hasTransparency?:boolean;paddingStatus?:"checking"|"trimmed"|"full" };
...
export default function ListingFactoryApp() {
  ...
  useEffect(()=>{if(!listingGoal)return;
    void fetch("/api/batches").then(response=>response.json()).then((result:{published?:PublishedDay[]})=>{
      setGoalDays(result.published||[]);setGoalDaysLoaded(true)}).catch(()=>undefined);
  ...
  const designArtworkReady=(file:DesignFile)=>Boolean(file.width&&file.height&&file.paddingStatus!=="checking"&&(file.artworkVersions||[]).every(artwork=>!artwork.originalUnavailable&&artwork.file?.size&&artwork.colorIds.length>0));
...
setPricingApproved(recipeCarriesApprovedPricing({defaultProfitTarget:activeRecipe?.defaultProfitTarget,etsyShippingProfileId:activeRecipe?.etsyShippingProfileId})); return result.product;
  ...
  async function inspectArtworkVersion(file:File,side:string):Promise<ArtworkVersion>{
    ...
    return {id:crypto.randomUUID(),name:file.name,size:file.size,file,previewUrl:URL.createObjectURL(file),side,colorIds:[...selectedColorIds],width,height,visibleBounds,hasTransparency,paddingStatus:trimmed?"trimmed":"full"};
  }
  ...
  async function processDesign(design: DesignFile): Promise<DraftResult> {
      ...
      try {
        const versions=(design.artworkVersions||[]).filter(artwork=>artwork.colorIds.length>0);
          ...
          <article className={`step-card designs-step workflow-panel ${workflowStep==="setup"?"batch-design-drop":""} ${files.length ? "done" : ""} ${workflowStep==="finish"?"finish-mode":""} ${workflowStep==="designs"||(workflowStep==="setup"&&Boolean(templateDetails)&&productSelected&&!failedBundleNames().length&&!bundleCreationMode)?"active-panel":"hidden-panel"}`}>{/* D238 · Choosing the mockup SET lived on Product while the mockups it controls are generated here on Images. Same setting, two pages — the exact split that caused the keyword-bank and shipping duplication. */}
            ...
            <div className="step-content">
              ...
              {files.length>0&&(workflowStep==="setup"||workflowStep==="designs")&&<div className="design-upload-review" aria-label="Review uploaded designs">{files.map(file=>{const colors=(templateDetails?.colorOptions||[]).filter(color=>selectedColorIds.includes(color.id)),sides=orderedPrintSides(templateDetails?.printPositions),primarySide=primaryPrintSide(sides),secondarySides=sides.filter(side=>side!==primarySide),itemNoun=productNoun(templateDetails?.blueprintTitle,templateDetails?.brand,templateDetails?.model),canVaryByColor=colors.length>1&&Boolean(primarySide),primaryVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side===primarySide),secondaryVersions=(file.artworkVersions||[]).filter(artwork=>artwork.side!==primarySide);return <article className="design-artwork-card" key={file.id}>
                ...
                {(canVaryByColor||secondarySides.length>0)&&<div className="artwork-version-tools"><b>Artwork and print areas</b><small>Assign different artwork to specific {itemNoun} colors, or print on another area this product supports.</small><div>{canVaryByColor?<label className="secondary-action" role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.currentTarget.querySelector("input")?.click()}}}>＋ Use a different design on some colors<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{if(primarySide)void addArtworkVersion(file.id,primarySide,event.target.files);event.target.value=""}}/></label>:null}{secondarySides.map(side=><label className="secondary-action" role="button" tabIndex={0} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();event.currentTarget.querySelector("input")?.click()}}} key={side}>＋ Add a {printSideLabel(side).toLocaleLowerCase()} print<input className="hidden-picker" type="file" accept=".png,.jpg,.jpeg" onChange={event=>{void addArtworkVersion(file.id,side,event.target.files);event.target.value=""}}/></label>)}{sides.length===1?<span className="print-area-unavailable">This saved Printify product has one print area ({printSideLabel(primarySide).toLocaleLowerCase()}).</span>:null}</div></div>}
                ...
                {secondaryVersions.map(artwork=><section className="artwork-version" key={artwork.id}><img src={artwork.previewUrl} alt=""/><div><b>{printSideLabel(artwork.side)} print</b><small>{artwork.name}</small>{!artwork.colorIds.length&&<em className="artwork-color-required">Choose at least one {itemNoun} color for this {printSideLabel(artwork.side).toLocaleLowerCase()} print.</em>}{colors.length?<fieldset><legend>Print this artwork on the {printSideLabel(artwork.side).toLocaleLowerCase()} of</legend>{colors.map(color=><button type="button" key={color.id} className={artwork.colorIds.includes(color.id)?"selected":""} aria-pressed={artwork.colorIds.includes(color.id)} onClick={()=>toggleArtworkColor(file.id,artwork.id,color.id)}><i style={{background:color.swatch||"#ddd"}}/>{color.title}</button>)}</fieldset>:null}</div><button type="button" onClick={()=>removeArtworkVersion(file.id,artwork.id)} aria-label={`Remove ${artwork.name}`}>Remove</button></section>)}
              </article>})}</div>}

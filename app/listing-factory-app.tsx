...
{/* D559 - her question: "why if this is a hoodie t shirt and crew neck batch
            ...
            <PhotoDeliveryHandoff ref={photoDeliveryRef} targets={bundlePublishDrafts().filter(draft=>draft.id&&draft.status==="Created").map(draft=>({id:draft.id!,title:draft.title||draft.name||"Listing",indices:bundlePublishSelections()[draft.id!]??printifyImageIndices}))} beforePrepare={async()=>{await persistBatchNow();await persistRunNow()}}/>
              ...
              <div className="printify-handoff" role="status">
                <b>{handoffBlockers().length?"Review any unfinished items before publishing in Printify.":"Review and publish in Printify."}</b><p>After publishing, confirm Etsy category, attributes and any personalization in Etsy. Your saved listing details remain here as a reference.</p>

...
export function batchHistoryQuery(params:URLSearchParams){
 ...
 const where="user_id=? AND parent_batch_id IS NULL"+(query?" AND (instr(lower(setup_name),lower(?))>0 OR instr(lower(product_title),lower(?))>0 OR instr(lower(CASE WHEN json_valid(state_json) THEN coalesce(json_extract(state_json,'$.batchDisplayName'),'') || ' ' || coalesce(json_extract(state_json,'$.run.bundleName'),'') || ' ' || coalesce(json_extract(state_json,'$.activeBundle.name'),'') || ' ' || coalesce(json_extract(state_json,'$.bundleRecipes'),'') ELSE '' END),lower(?))>0 OR instr(lower(replace(replace(CASE WHEN json_valid(state_json) THEN coalesce(json_extract(state_json,'$.designs'),'') ELSE '' END,'_',' '),'-',' ')),lower(?))>0)":"");
 ...
 return {query,page,sort,limit,offset,where,searchBindings:query?[query,query,query,query]:[],order,selectSql,countSql};
}

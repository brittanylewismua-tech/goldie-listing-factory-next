...
export default function BatchesPage() {
  ...
  const [page,setPage]=useState(1),[total,setTotal]=useState(0),[sort,setSort]=useState("recent");
  ...
  async function loadHistory(quiet=false){
    ...
    try{const data=await readBatchHistory<Batch>(fetch,25000,{query,page,sort});
      if(version!==requestVersion.current)return;
      setBatches(data.batches);setTotal(data.total??data.batches.length);
      setSelected(current=>current.filter(id=>data.batches.some(batch=>batch.id===id&&batch.status!=="processing")));setError("");
    }catch(value){if(version===requestVersion.current)setError(value instanceof Error?value.message:"Saved history could not be loaded. Try again.")}
    finally{if(version===requestVersion.current)setLoading(false)}
  }
  useEffect(() => {const timer=window.setTimeout(()=>void loadHistory(),250);return()=>{window.clearTimeout(timer);++requestVersion.current}}, [query,page,sort]);
  useEffect(()=>{if(loading||!batches.some(batch=>batch.status==="processing"))return;const timer=window.setInterval(()=>void loadHistory(true),3000);return()=>window.clearInterval(timer)},[loading,query,page,sort,batches.map(batch=>`${batch.id}:${batch.status}:${batch.draft_count||0}`).join("|")]);
  ...
  async function removeSelected(){
    ...
    setDeleting(false);
    if(removed.length){setTotal(n=>Math.max(0,n-removed.length));if(page>1&&removed.length===batches.length)setPage(n=>n-1);else void loadHistory(true)}
  }
  ...
  async function remove(batch:Batch) {
    ...
    if(result.confirmed.length){setBatches(current=>current.filter(item=>item.id!==batch.id));setTotal(n=>Math.max(0,n-1));if(page>1&&batches.length===1)setPage(n=>n-1);else void loadHistory(true)}
    if(result.uncertain)setError("This removal could not be confirmed. Reload history before trying again.");
  }
  const visibleBatches=batches,visibleIds=visibleBatches.filter(batch=>batch.status!=="processing").map(batch=>batch.id),visibleSelected=visibleIds.filter(id=>selected.includes(id));
  ...
  return <FactoryShell active="batches" title="Batch History"><div className="management-page interior-page">
    ...
    <section className="batch-history">
      ...
      <div className="library-controls"><label className="batch-history-search"><span>Find a saved batch</span><input type="search" value={query} placeholder="Search all saved batches" onChange={event=>{setQuery(event.target.value);setPage(1);setSelected([]);setLoading(true)}}/></label><label>Sort batches<select value={sort} onChange={e=>{setSort(e.target.value);setPage(1);setSelected([]);setLoading(true)}}><option value="recent">Recently saved</option><option value="oldest">Oldest saved</option></select></label></div>
      {!loading&&!error&&<p role="status">{total?`${(page-1)*20+1}–${Math.min(page*20,total)} of ${total} ${query.trim()?"matching ":""}batches`:query.trim()?"No matching batches":"No saved batches"}</p>}
      ...
      {!loading&&visibleBatches.length>0&&<div className="batch-history-select">
        ...
        <span>{selected.length?`${selected.length} selected`:"Select this page"}</span></label>
      ...
      {loading ? (
        /* D1607 · a sentence on the grid was the whole loading state, and this
           page reads Printify before it can answer. The shape of what is
           coming is drawn instead, so it does not jump when it arrives. */
        <div className="p-stack" role="status" aria-label="Loading saved batches">
          <div className="p-skeleton p-skeleton-card" />
          <div className="p-skeleton p-skeleton-card" />
          <div className="p-skeleton p-skeleton-card" />
        </div>
      ) : error ? null : !batches.length && !query.trim() ? error?null:<div className="empty-history"><h2>No saved batches yet</h2><p>Your first batch appears here as soon as you add designs.</p><a href="/listing-factory">Start a batch</a></div> : !visibleBatches.length?<div className="empty-history batch-history-no-match"><h2>No matching batches</h2><p>Try a batch name or product name.</p></div>:visibleBatches.map(batch => <article key={batch.id} className={selected.includes(batch.id)?"selected":""}><label className="batch-select"><input type="checkbox" disabled={batch.status==="processing"} checked={selected.includes(batch.id)} onChange={()=>toggleSelected(batch.id)} aria-label={`Select ${batch.display_name||"Untitled batch"}`}/></label>{batch.thumbnail_url&&!failedThumbnails.has(batch.thumbnail_url)?<img className="batch-history-thumbnail" src={batch.thumbnail_url} alt="" onError={()=>setFailedThumbnails(previous=>new Set(previous).add(batch.thumbnail_url))}/>:<span className="batch-history-thumbnail empty" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="9" cy="10" r="1.6"/><path d="M21 16l-5-5-6 6"/></svg></span>}<div className="batch-history-summary"><span className={`batch-status ${batch.status}`}>{statusLabel(batch)}</span><h2>{batch.display_name || "Untitled batch"}</h2><p>{batch.members?.length?batch.product_title:`${batch.product_title || "Custom product"} · ${batch.design_count} ${batch.design_count === 1 ? "design" : "designs"}`}</p>{/* D871 · A bundle run is one row, and its products are its progress - not
            separate jobs with their own name, badge and Resume button. */}
          {batch.members?.length?<ul className="batch-history-members">{batch.members.map(member=><li key={member.batchId||`missing-${member.position}`} className={member.done?"done":""}><span className="member-mark" aria-hidden="true">{member.done?"✓":member.position}</span><b>{member.productName||`Product ${member.position}`}</b><small>{member.published>0?(member.published>=member.expected?`${member.published} published`:`${member.published} of ${member.expected} published`):member.drafts>0?`${member.drafts} Printify ${member.drafts===1?"draft":"drafts"} saved`:batch.status==="processing"?"Creating drafts…":"Not started yet"}</small></li>)}</ul>:null}<small>Last saved {savedLabel(batch.updated_at)}</small></div><div className="batch-history-controls"><span className="batch-row-actions"><button onClick={() => resume(batch)}>{batch.status==="processing"?"View progress":batch.members?.length
      ...
      {total>20&&<nav className="history-pagination" aria-label="Batch history pages"><button className="secondary-action" disabled={loading||page===1} onClick={()=>{setPage(n=>n-1);setSelected([]);setLoading(true)}}>Previous page</button><span>Page {page} of {Math.ceil(total/20)}</span><button className="secondary-action" disabled={loading||page*20>=total} onClick={()=>{setPage(n=>n+1);setSelected([]);setLoading(true)}}>Next page</button></nav>}
    </section>
  </div></FactoryShell>;
}

...
export function promptFor(kind: ProductKind, hasReference: boolean) {
  if (kind === "apparel") return [
    "Edit the first image only. Print the exact design from the second image directly onto the existing garment in the first image.",
    ...
    hasReference ? "Use the third image only to measure the print's relative width, height, center position, and scale. Do not copy the garment or background from the third image." : "Place the design naturally in the centered chest print area.",

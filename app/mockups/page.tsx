...
export default function Home() {
  ...
  const [activeTheme,setActiveTheme]=useState("Pink Dorm");
  ...
  useEffect(()=>{let alive=true;fetch("/api/mockups/library").then(async response=>response.ok?response.json():{templates:[]}).then((payload:{templates?:Template[]})=>{if(!alive||!payload.templates?.length)return;setLibrary(current=>[...current.filter(item=>!item.custom),...payload.templates!]);setSelected(current=>new Set([...current,...payload.templates!.map(item=>item.id)]));}).catch(()=>undefined);return()=>{alive=false};},[]);
  ...
  return <main className="mockupFactory">
    ...
    <section className="mockupWorkspace">
      ...
      <div className="mockupStep"><div className="stepHead"><span>2</span><div><h2>Choose your mockups</h2><p>Select every scene, or choose only the ones you want for this design.</p></div></div>
        <div className="setList">{[...new Set(library.map(t=>t.theme))].map(theme=>{const items=library.filter(t=>t.theme===theme),open=activeTheme===theme;return <div className={`collection ${open?"open":"collapsed"}`} key={theme}><button className="collectionToggle" aria-expanded={open} onClick={()=>setActiveTheme(open?"":theme)}><div><span className="selected">MOCKUP SET</span><h3>{theme}</h3><p>{items.filter(t=>selected.has(t.id)).length} of {items.length} selected</p></div><span className="collectionChevron" aria-hidden="true">⌄</span></button>{open&&<><div className="collectionActions"><button className="selectSet" onClick={()=>{const ids=items.map(t=>t.id);const all=ids.every(id=>selected.has(id));setSelected(s=>{const n=new Set(s);ids.forEach(id=>all?n.delete(id):n.add(id));return n})}}>{items.every(t=>selected.has(t.id))?"Deselect all":"Select all"}</button></div><div className="thumbs">{items.map(t=><button className={`mockChoice ${selected.has(t.id)?"chosen":""}`} aria-pressed={selected.has(t.id)} key={t.id} onClick={()=>toggle(t.id)}><span className="choiceVisual"><img src={t.src} alt={t.name}/>{selected.has(t.id)&&<i className="choiceCheck" aria-hidden="true">✓</i>}</span><span className="choiceName">{t.name}</span>{t.custom&&(t.surfaceKind||"rigid-flat")==="rigid-flat"&&<small onClick={e=>{e.stopPropagation();setPoints([]);setCalibrating(t)}}>Reset product area</small>}</button>)}</div></>}</div>})}</div>

...
type SurfaceKind = "rigid-flat" | "phone-case" | "t-shirt" | "sweatshirt" | "hoodie" | "other-apparel" | "apparel" | "soft-goods" | "curved" | "irregular";
...
function validateSurface(corners: Template["corners"], width:number, height:number) {
  if (corners.some(([x,y])=>!Number.isFinite(x)||!Number.isFinite(y)||x<0||y<0||x>width||y>height)) throw new Error("The Listing Factory could not establish a safe printable boundary for this mockup.");
  const signs=corners.map((p,i)=>Math.sign(cross(p,corners[(i+1)%4],corners[(i+2)%4]))).filter(Boolean);
  if (!signs.length||!signs.every(s=>s===signs[0])||polygonArea(corners)<width*height*.01) throw new Error("The Listing Factory could not establish a dependable printable surface in this mockup.");
}
...
async function foregroundLayers(template: Template) {
  ...
  if (!response.ok) throw new Error(result.error || `The Listing Factory could not safely layer “${template.name}.”`);
...
export default function Home() {
  ...
  const [expandedIndex,setExpandedIndex]=useState<number|null>(null); const [libraryPreview,setLibraryPreview]=useState<Template|null>(null); const [renamingTheme,setRenamingTheme]=useState(""); const [renameValue,setRenameValue]=useState(""); const [renameSurface,setRenameSurface]=useState<SurfaceKind>("rigid-flat"); const [deletingTheme,setDeletingTheme]=useState("");
  ...
  const downloadAll=async()=>{const entries:Record<string,Uint8Array>={};for(const result of results){entries[result.name]=new Uint8Array(await (await fetch(result.url)).arrayBuffer());}const zip=zipSync(entries,{level:0}) as Uint8Array<ArrayBuffer>;const url=URL.createObjectURL(new Blob([zip],{type:"application/zip"}));const a=document.createElement("a");a.href=url;a.download=`goldie-mockups-${new Date().toISOString().slice(0,10)}.zip`;a.style.display="none";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);};
  /* D605 - the target set is passed in, never read from the builder's fields.
     Adding to an existing set has to use THAT set's name and surface, and the
     file dialog spans several renders, so reading it from state here is how the
     photographs end up filed under whatever was last typed. */
  const addMockups=async(e:ChangeEvent<HTMLInputElement>,target?:{theme:string;surfaceKind:string})=>{const incoming=Array.from(e.target.files||[]).filter(f=>/^image\/(png|jpeg|webp)$/.test(f.type));const theme=target?.theme||themeName.trim()||"My mockup set";const kind=target?.surfaceKind||surfaceKind;e.target.value="";const added:Template[]=[];for(const original of incoming){const file=await libraryImage(original),form=new FormData();form.set("image",file);form.set("theme",theme);form.set("name",original.name.replace(/\.[^.]+$/,"").trim()||"Mockup");form.set("surfaceKind",kind);const response=await fetch("/api/mockups/library",{method:"POST",body:form});const payload=await response.json() as {template?:Template;error?:string};if(!response.ok||!payload.template){setGenerationError(payload.error||"This mockup could not be saved.");continue;}added.push(payload.template);setLibraryProgress(added.length);}if(!added.length)return;setLibrary(x=>[...x,...added]);setSelected(new Set());setActiveTheme(theme);/* D468 - the print area is worked out for every photograph as it arrives. A set
...
/* D573 - this was fire-and-forget, so closing the tab or moving on part way
  ...
  /* D610 - the same dialog now corrects the product surface. A set saved under
     the wrong one had no way back short of deleting and re-uploading it. */
  const renameSet=async()=>{const next=renameValue.trim();if(!next||!renamingTheme)return;const sourceTheme=library.find(item=>item.theme===renamingTheme)?.sourceTheme;const was=library.find(item=>item.theme===renamingTheme)?.surfaceKind;const surfaceChanged=Boolean(renameSurface&&renameSurface!==was);const response=await fetch("/api/mockups/library",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({oldTheme:renamingTheme,newTheme:next,sourceTheme,...(surfaceChanged?{surfaceKind:renameSurface}:{})})});const payload=await response.json() as {error?:string};if(!response.ok){setGenerationError(payload.error||"This mockup set could not be updated.");return;}setLibrary(items=>items.map(item=>item.theme===renamingTheme?{...item,theme:next,...(surfaceChanged?{surfaceKind:renameSurface,preparation:undefined,preparationStatus:"queued"}:{})}:item));setActiveTheme(next);setRenamingTheme("");setRenameValue("")};
  ...
  /* D605 - add photographs to a set that already exists. The server has always
     accepted this - POST keys on the theme and only refuses past fifty - so the
  ...
  const chooseMoreForSet=(theme:string)=>{
    ...
    setGenerationError("");
    addToSetInput.current?.click();
  };
  const addMockupsToSet=async(e:ChangeEvent<HTMLInputElement>)=>{
    const target=addToSetTarget.current;
    const count=e.target.files?.length||0;
    if(!target||!count){e.target.value="";return}
  ...
  return <FactoryShell active="mockups" title="Your saved mockups"><div className="management-page mockupFactory managementOnly interior-page">
    ...
    <section className="mockupWorkspace"><div className="mockupStep managementLibrary"><div className="managementLibraryHead"><div><p className="mockupEyebrow">SAVED SETS</p><h2>{libraryLoading?"Loading mockup sets…":library.length?"Saved mockup sets":"Create your first mockup set"}</h2><p>Each set can hold up to 50 blank mockups.</p></div><button className="newSetButton" onClick={()=>setShowAddSet(true)}><span aria-hidden="true">+</span> Add mockup set</button></div>
      ...
      <label className="library-search">Find a mockup set<input type="search" value={setQuery} onChange={e=>setSetQuery(e.target.value)} placeholder="Search set names"/></label>
      {library.length>0&&!library.some(item=>item.theme.toLowerCase().includes(setQuery.trim().toLowerCase()))&&<p role="status">No mockup sets match this search.</p>}
      <div className="setList managementSetList">{[...new Set(library.map(item=>item.theme))].filter(theme=>theme.toLowerCase().includes(setQuery.trim().toLowerCase())).map(theme=>{const items=library.filter(item=>item.theme===theme),open=activeTheme===theme;return <article className={`collection ${open?"open":"collapsed"}`} key={theme}><button className="collectionToggle" aria-expanded={open} onClick={()=>setActiveTheme(open?"":theme)}><div><span className="selected">MOCKUP SET</span><span className="setTitleRow"><h3>{theme}</h3></span><p>{items.length} {items.length===1?"mockup":"mockups"}</p>{!open&&<span className="setPreview">{items.slice(0,5).map(item=><img key={item.id} src={item.src} alt="" loading="lazy" decoding="async"/>)}</span>}</div><span className="collectionChevron">⌄</span></button>{open&&<><div className="collectionActions">{addingTheme===theme
  ...
  :<button className="selectSet addToSet" disabled={libraryBusy} onClick={()=>chooseMoreForSet(theme)}><span aria-hidden="true">+</span> Add mockups</button>}<button className="selectSet" onClick={()=>{setRenamingTheme(theme);setRenameValue(theme);setRenameSurface((items[0]?.surfaceKind as SurfaceKind)||"rigid-flat")}}>Rename set</button><button className="deleteSet" onClick={()=>setDeletingTheme(theme)}>Delete set</button></div><div className="thumbs">{items.map(item=><div className="mockChoice" key={item.id}><button type="button" className="savedMockupPreview" onClick={()=>setLibraryPreview(item)} aria-label={`Enlarge ${item.name}`}><img src={item.src} alt={item.name} loading="lazy" decoding="async"/><span>Enlarge</span></button><span className="choiceName">{item.name}</span>
      ...
      <input ref={addToSetInput} hidden type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={event=>void addMockupsToSet(event)}/>
    </div></section>

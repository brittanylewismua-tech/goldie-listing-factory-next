...
export default function Home() {
  ...
  useEffect(()=>{let alive=true;fetch("/api/mockups/library").then(async response=>response.ok?response.json():{templates:[]}).then((payload:{templates?:Template[]})=>{if(!alive||!payload.templates?.length)return;setLibrary(current=>[...current.filter(item=>!item.custom),...payload.templates!]);}).catch(()=>undefined);return()=>{alive=false};},[]);
  ...
  const toggle=(id:string)=>setSelected(s=>{const n=new Set(s),template=library.find(item=>item.id===id);if(!template)return n;if(n.has(id)){n.delete(id);return n;}const kind=template.surfaceKind||"rigid-flat";for(const selectedId of n){const selectedTemplate=library.find(item=>item.id===selectedId);if((selectedTemplate?.surfaceKind||"rigid-flat")!==kind)n.delete(selectedId);}n.add(id);return n});
    ...
    <section className="mockupWorkspace">
      ...
      <div className="mockupStep"><div className="stepHead"><span>2</span><div><h2>Choose your mockups</h2><p>Select every scene, or choose only the ones you want for this design.</p></div></div>
        <div className="setList">
          <button type="button" className="addSetCard" onClick={()=>addSetRef.current?.scrollIntoView({behavior:"smooth",block:"center"})}><span aria-hidden="true">＋</span><strong>Add your first mockup set</strong><small>Upload and save a reusable group of scenes.</small></button>

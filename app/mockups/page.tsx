...
const SURFACE_LABELS: Record<SurfaceKind,string> = {
  ...
  apparel:"T-shirt, sweatshirt, hoodie or other worn apparel",
...
async function makeMockup(file: File, template: Template): Promise<Rendered> {
  ...
  if(kind!=="rigid-flat") throw new Error(`${SURFACE_LABELS[kind]} mockups require product-aware rendering.`);
...
async function makeProductMockup(file:File,template:Template,reference:File|null):Promise<Rendered>{
  ...
  const payload=await response.json() as {image?:string;error?:string};if(!response.ok||!payload.image)throw new Error(payload.error||`Goldie could not render ${template.name}.`);
...
export default function Home() {
  ...
  const [expandedIndex,setExpandedIndex]=useState<number|null>(null); const [renamingTheme,setRenamingTheme]=useState(""); const [renameValue,setRenameValue]=useState(""); const [deletingTheme,setDeletingTheme]=useState("");
  ...
  const [libraryBusy,setLibraryBusy]=useState(false); const [libraryProgress,setLibraryProgress]=useState(0);
  ...
  return <main className="mockupFactory managementOnly">
    ...
    <section className="mockupWorkspace"><div className="mockupStep managementLibrary"><div className="managementLibraryHead"><div><p className="mockupEyebrow">SAVED SETS</p><h2>{library.some(item=>item.custom)?"Your mockup sets":"Create your first mockup set"}</h2><p>Each set can hold up to 50 blank mockups.</p></div><button className="newSetButton" onClick={()=>setShowAddSet(true)}>＋ Add mockup set</button></div>
      ...
      {libraryBusy&&<div className="librarySaving" role="status"><span className="librarySpinner"/><div><b>Saving {libraryProgress} {libraryProgress===1?"mockup":"mockups"}…</b><small>Please keep this page open until the set appears below.</small></div></div>}
      <div className="setList managementSetList">{[...new Set(library.map(item=>item.theme))].map(theme=>{const items=library.filter(item=>item.theme===theme),open=activeTheme===theme;return <article className={`collection ${open?"open":"collapsed"}`} key={theme}><button className="collectionToggle" aria-expanded={open} onClick={()=>setActiveTheme(open?"":theme)}><div><span className="selected">MOCKUP SET</span><span className="setTitleRow"><h3>{theme}</h3></span><p>{items.length} {items.length===1?"mockup":"mockups"}</p><span className="setPreview">{items.slice(0,3).map(item=><img key={item.id} src={item.src} alt=""/>)}</span></div><span className="collectionChevron">⌄</span></button>{open&&<><div className="collectionActions"><button className="deleteSet" onClick={()=>setDeletingTheme(theme)}>Delete set</button><button className="selectSet" onClick={()=>{setRenamingTheme(theme);setRenameValue(theme)}}>Rename set</button></div><div className="thumbs">{items.map(item=><div className="mockChoice" key={item.id}><span className="choiceVisual"><img src={item.src} alt={item.name}/></span><span className="choiceName">{item.name}</span>{item.custom&&(item.surfaceKind||"rigid-flat")==="rigid-flat"&&<button className="resetArea" onClick={()=>{setPoints([]);setCalibrating(item)}}>Reset product area</button>}</div>)}</div></>}</article>})}</div>
    </div></section>
    {showAddSet&&<div className="confirmOverlay" role="dialog" aria-modal="true" aria-labelledby="add-set-title"><div className="confirmDialog addSetDialog"><button className="closeAddSet" onClick={()=>setShowAddSet(false)} aria-label="Close">×</button><p className="mockupEyebrow">ADD MOCKUP SET</p><h2 id="add-set-title">Build a reusable set</h2><div className="setControls"><label><span>Name this set</span><input value={themeName} onChange={e=>setThemeName(e.target.value)} placeholder="Example: Palm Springs Models"/></label><label><span>Product surface</span><select value={surfaceKind} onChange={e=>setSurfaceKind(e.target.value as SurfaceKind)}>{Object.entries(SURFACE_LABELS).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><button disabled={libraryBusy||!themeName.trim()} onClick={()=>mockupInput.current?.click()}><span>↑</span>{libraryBusy?"Saving…":"Choose blank mockups"}</button><small>PNG, JPG, or WEBP · maximum 50 mockups per set</small><input ref={mockupInput} hidden type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={event=>void addMockupsManaged(event)}/></div></div></div>}
    ...
    <footer className="mockupFooter"><span>GOLDIE MOCKUP SETS</span><p>Manage here. Create inside Listing Factory.</p></footer>
  </main>;

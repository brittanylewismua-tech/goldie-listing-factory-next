...
export default function Home() {
  ...
  const renameSet=async()=>{const next=renameValue.trim();if(!next||!renamingTheme)return;const response=await fetch("/api/mockups/library",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({oldTheme:renamingTheme,newTheme:next})});const payload=await response.json() as {error?:string};if(!response.ok){setGenerationError(payload.error||"This mockup set could not be renamed.");return;}setLibrary(items=>items.map(item=>item.custom&&item.theme===renamingTheme?{...item,theme:next}:item));setActiveTheme(next);setRenamingTheme("");setRenameValue("")};
  const deleteSet=async()=>{if(!deletingTheme)return;const response=await fetch("/api/mockups/library",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({theme:deletingTheme})});const payload=await response.json() as {error?:string};if(!response.ok){setGenerationError(payload.error||"This mockup set could not be deleted.");return;}const deletedIds=new Set(library.filter(item=>item.custom&&item.theme===deletingTheme).map(item=>item.id));setLibrary(items=>items.filter(item=>!deletedIds.has(item.id)));setSelected(current=>new Set([...current].filter(id=>!deletedIds.has(id))));if(activeTheme===deletingTheme)setActiveTheme("");setDeletingTheme("")};

...
export function estimatedProfit(priceCents: number, costCents: number, pricing?: PricingRules) {
  ...
  const shippingCharged = Math.max(0, Number(pricing?.shippingCharged || 0));
  const shippingCost = Math.max(0, Number(pricing?.shippingCost || 0));
  const revenue = priceCents / 100 + shippingCharged;
  const fees = revenue * percent + Number(pricing?.fixedFee || 0) + Number(pricing?.listingFee || 0);
  return revenue - costCents / 100 - shippingCost - fees;
}
...
export function recommendedPrice(costCents: number, pricing?: PricingRules) {
  ...
  const shippingCharged = Math.max(0, Number(pricing.shippingCharged || 0));
  const shippingCost = Math.max(0, Number(pricing.shippingCost || 0));
  const fixed = Number(pricing.fixedFee || 0) + Number(pricing.listingFee || 0) + shippingCost - shippingCharged * (1 - percent);

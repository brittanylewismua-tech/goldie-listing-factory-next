...
export default function WaitProgress({operation,observeTools=false}:{operation:WaitOperation|null;observeTools?:boolean}){
 ...
 useEffect(()=>{if(!active||helpOpen||!dialog.current)return;const opener=document.activeElement instanceof HTMLElement?document.activeElement:null;dialog.current.showModal();return()=>{dialog.current?.close();if(opener?.isConnected)opener.focus({preventScroll:true})}},[Boolean(active),active?.background,helpOpen]);

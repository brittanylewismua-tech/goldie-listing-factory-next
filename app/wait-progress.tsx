...
export function WaitCard({title,detail,started,lastConfirmed,done,total,background=false,onHelp}:{onHelp?:()=>void;title:string;detail?:string;started:number;lastConfirmed?:number;done?:number;total?:number;background?:boolean}){
 ...
 return <div className="goldie-wait-card">
  ...
  {state.long&&<p className="goldie-wait-long">{state.stale?'This is taking longer than expected. Your confirmed work is saved; the results below will show any recovery needed.':background?'Draft creation can take several minutes. Batch History will show each confirmed result.':'Large batches and files can take several minutes. The Listing Factory is still waiting for confirmation. Do not repeat this action; a result or reported error will appear when the request finishes.'}</p>}
...
export default function WaitProgress({operation,observeTools=false}:{operation:WaitOperation|null;observeTools?:boolean}){
 ...
 useEffect(()=>{if(!active||active.background||helpOpen||!dialog.current)return;const opener=document.activeElement instanceof HTMLElement?document.activeElement:null;dialog.current.showModal();return()=>{dialog.current?.close();if(opener?.isConnected)opener.focus({preventScroll:true})}},[Boolean(active),active?.background,helpOpen]);
 ...
 if(active.background){const value=progressValue(active.done,active.total);return createPortal(<aside className="goldie-background-progress" role="status" aria-live="polite"><div><b>{active.title}</b><span>{value!=null?`${value} of ${active.total} finished`:"Working in Printify"}</span></div><progress aria-label={active.title} max={active.total||1} value={value}/><p>{active.detail||"Printify is creating the drafts."}</p><nav aria-label="Background draft actions"><a href="/listing-factory">Start another batch</a><a href="/batches">View Batch History <span aria-hidden="true">→</span></a></nav></aside>,document.body)}

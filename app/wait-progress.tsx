...
export function WaitCard({title,detail,started,lastConfirmed,done,total,background=false,onHelp}:{onHelp?:()=>void;title:string;detail?:string;started:number;lastConfirmed?:number;done?:number;total?:number;background?:boolean}){
 ...
 return <div className="goldie-wait-card">
  ...
  {background&&<nav className="goldie-wait-actions" aria-label="Continue while drafts are created"><a href="/listing-factory">Start another batch</a><a href="/batches">View Batch History</a></nav>}
...
export default function WaitProgress({operation,observeTools=false}:{operation:WaitOperation|null;observeTools?:boolean}){
 ...
 useEffect(()=>{if(!active||helpOpen||!dialog.current)return;const opener=document.activeElement instanceof HTMLElement?document.activeElement:null;dialog.current.showModal();const restoreFocus=containModalFocus(active.title,opener);return()=>{dialog.current?.close();restoreFocus()}},[Boolean(active),active?.background,helpOpen]);
 ...
 return createPortal(<dialog className="goldie-wait-dialog" ref={dialog} tabIndex={-1} aria-modal="true" aria-label={active.title} onCancel={event=>event.preventDefault()}><WaitCard {...active} started={started} onHelp={getHelp}/></dialog>,document.body);
}

...
export default function FinalListingReview({drafts,files,selections,defaultIndices,preparedMockupCounts,batchSizeGuide,onRetry,onEdit}:Props){
  ...
  return <section className="final-listing-review">
    ...
    <div className="final-design-groups">{groups.map(([designName,group])=>{const attention=group.filter(draft=>draft.status!=="Created"||!draft.id||(selections[draft.id]??defaultIndices).length+(preparedMockupCounts[draft.id]||0)===0).length;return <details className="final-design-group" key={designName} open={groups.length<=3||attention>0}><summary><span>{designName}</span><b>{group.length} {group.length===1?"listing":"listings"}</b><em className={attention?"needs-attention":"ready"}>{attention?`${attention} need a look`:"✓ Ready"}</em></summary><div className="final-listing-grid">{group.map(draft=>{const design=files.find(file=>file.id===draft.clientId)||files.find(file=>file.name===draft.name),selectedCount=draft.id?(selections[draft.id]??defaultIndices).length:defaultIndices.length,mockupCount=draft.id?preparedMockupCounts[draft.id]||0:0,hasPhoto=selectedCount+mockupCount>0,ready=draft.status==="Created"&&hasPhoto;return <article className={`final-listing-card ${ready?"":"failed"}`} key={`${draft.productName||"product"}:${draft.clientId}`}>
      ...
      <div><small className="final-product-name">{draft.productName||"Saved product"}</small><b>{design?.title||draft.title||draft.name}</b><small>{(design?.title||draft.title||draft.name).length}/140 characters · {design?.tags?.length||0}/13 tags · {selectedCount+mockupCount} photos{design?.sizeGuideName||batchSizeGuide?" · size guide ready":""}</small><span className={ready?"ready":"needs-attention"}>{ready?"✓ Ready for final publish":draft.status!=="Created"?`! ${draft.error||"Draft needs attention"}`:"! Add at least one listing photo"}</span></div>

...
type Props = { drafts:Draft[]; files:Design[]; selections:Record<string,number[]>; defaultIndices:number[]; preparedMockupCounts:Record<string,number>; batchSizeGuide:string; productName?:string; onRetry?:(clientId:string)=>void; onEdit:(phase:"details"|"mockups"|"pricing",draft:Draft)=>void; onEditProduct?:(stage:"design"|"pricing"|"photos",draft:Draft)=>void; onSelectionChange?:(ids:string[])=>void; onSelectionTouched?:()=>void; handoffOnly?:boolean };
...
export default function FinalListingReview({drafts,files,selections,defaultIndices,preparedMockupCounts,batchSizeGuide,productName,onRetry,onEdit,onEditProduct,onSelectionChange,onSelectionTouched,handoffOnly=false}:Props){
    ...
    return <section className="final-listing-review handoff-only recipe-listing-review">
      <div className="recipe-product-groups">{productGroups.map(([name,items],productIndex)=>{
        return <section className="recipe-product-group" key={name}>
          <header><div><small>{productGroups.length>1?`Product ${productIndex+1} of ${productGroups.length}`:"Saved product"}</small><h4>{name}</h4></div>{onEditProduct&&items[0]?<nav className="recipe-product-settings" aria-label={`Edit ${name} settings`}><span>Product settings</span><button type="button" onClick={()=>onEditProduct("design",items[0])}>Artwork, colors &amp; sizes</button><button type="button" onClick={()=>onEditProduct("pricing",items[0])}>Pricing &amp; shipping</button><button type="button" onClick={()=>onEditProduct("photos",items[0])}>Listing photos</button></nav>:null}</header>

...
import { promptFor, rendererFor, type ProductKind } from "@/app/mockups/product-renderers";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`https://fal.run/${rendererFor(kind)}`, {
      ...
      body: JSON.stringify({ image_urls:[body.scene,body.design,...(body.reference?[body.reference]:[])], prompt:promptFor(kind,Boolean(body.reference)), guidance_scale:1, num_inference_steps:8, acceleration:"regular", negative_prompt:"altered text, misspelled words, changed artwork, extra graphics, warped letters, fake logo, changed person, changed background", enable_safety_checker:true, output_format:"png", num_images:1, lora_scale:1 }),
    });

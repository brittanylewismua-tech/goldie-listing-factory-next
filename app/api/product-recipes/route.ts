...
export async function POST(request: Request) {
  ...
  const description=String(body.description||"").trim().slice(0,12000),defaultMockupTheme=String(body.defaultMockupTheme||"").trim().slice(0,80),defaultProfitTarget=Math.max(0,Math.min(500,Number(body.defaultProfitTarget)||10));
  const mockupIds=Array.isArray(body.mockupIds)?body.mockupIds.map(id=>String(id).trim()).filter(Boolean).slice(0,8):undefined;
  const extras={keywordListId:String(body.keywordListId||""),printifyImageIndicesJson:JSON.stringify((body.printifyImageIndices||[]).filter(Number.isInteger).slice(0,20)),normalizePadding:body.normalizePadding!==false,pricingJson:JSON.stringify({etsyShippingProfileId:Number(body.etsyShippingProfileId)||0,defaultColorIds:(body.defaultColorIds||[]).filter(Number.isInteger),defaultProfitTarget,etsyDefaults,mockupIds,setupComplete:body.setupComplete!==false})};

...
export async function GET(request:Request){const user=await getChatGPTUser();if(!user)return NextResponse.json({error:"Sign in to continue."},{status:401});const database=db();if(!database)return NextResponse.json({error:"Batch history is unavailable."},{status:503});await ensure(database);const url=new URL(request.url),id=url.searchParams.get("id");if(id){const row=await database.prepare("SELECT * FROM listing_batches WHERE id=? AND user_id=?").bind(id,user.userId).first<Record<string,unknown>>();if(!row)return NextResponse.json({error:"That batch was not found."},{status:404});let state=JSON.parse(String(row.state_json||"{}")) as BatchListState;
    const designIds=(state.designs||[]).map(design=>design.id).filter((value):value is string=>Boolean(value));
    if(designIds.length){
      const records=await database.prepare(`SELECT response_json FROM printify_draft_results WHERE user_id=? AND status='succeeded' AND client_id IN (${designIds.map(()=>'?').join(',')})`).bind(user.userId,...designIds).all<{response_json:string}>();
      const authoritative=records.results.flatMap(record=>{try{return [JSON.parse(record.response_json)]}catch{return []}});
...
export async function POST(request:Request){const user=await getChatGPTUser();if(!user)return NextResponse.json({error:"Sign in to continue."},{status:401});const database=db();if(!database)return NextResponse.json({error:"Batch history is unavailable."},{status:503});await ensure(database);const body=await request.json() as {id?:string;status?:string;step?:string;setupName?:string;productTitle?:string;designCount?:number;state?:unknown;parentBatchId?:string};const id=String(body.id||crypto.randomUUID()).replace(/[^a-zA-Z0-9-]/g,"").slice(0,80);const allowedStatus=new Set(["draft","processing","needs_attention","complete"]),allowedStep=new Set(["connect","setup","designs","review","finish"]);const status=allowedStatus.has(String(body.status))?String(body.status):"draft",step=allowedStep.has(String(body.step))?String(body.step):"connect";if(batchDraftIdentityProblem((body.state||{}) as Parameters<typeof batchDraftIdentityProblem>[0]))return NextResponse.json({error:"The draft results do not belong to this product’s designs. Reload the saved batch before continuing."},{status:409});const stateJson=JSON.stringify(body.state??{});if(stateJson.length>1800000)return NextResponse.json({error:"This batch snapshot is too large."},{status:413});/* D871 · A child names its parent run once, when it is first written. The
  ...
  const saved=await database.prepare("INSERT INTO listing_batches (id,user_id,status,step,setup_name,product_title,design_count,state_json,parent_batch_id,updated_at) VALUES (?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(id) DO UPDATE SET parent_batch_id=COALESCE(excluded.parent_batch_id,listing_batches.parent_batch_id),status=excluded.status,step=excluded.step,setup_name=excluded.setup_name,product_title=excluded.product_title,design_count=excluded.design_count,state_json=CASE WHEN length(trim(COALESCE(json_extract(listing_batches.state_json,'$.batchDisplayName'),'')))>0 THEN json_set(excluded.state_json,'$.batchDisplayName',json_extract(listing_batches.state_json,'$.batchDisplayName')) ELSE excluded.state_json END,updated_at=CURRENT_TIMESTAMP WHERE user_id=excluded.user_id AND (json_extract(listing_batches.state_json,'$.activeRecipe.id') IS NULL OR json_extract(listing_batches.state_json,'$.activeRecipe.id')=json_extract(excluded.state_json,'$.activeRecipe.id'))").bind(id,user.userId,status,step,String(body.setupName||"").slice(0,160),String(body.productTitle||"").slice(0,200),Math.max(0,Math.min(20,Number(body.designCount||0))),stateJson,parentBatchId).run();if(!saved.meta.changes)return NextResponse.json({error:"This batch belongs to a different product. Reload before saving."},{status:409});return NextResponse.json({id,saved:true})}

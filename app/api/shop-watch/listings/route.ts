...
export const GET=withErrorLog("shop-watch-listings",async(request:Request)=>{
  ...
  if(offset===0&&cached&&now-cached.refreshed_at<21600){const saved=JSON.parse(cached.payload);if("nextOffset" in saved)return NextResponse.json(saved);}
  try{
    await waitForEtsyCapacity();
    const response=await fetch(`https://openapi.etsy.com/v3/application/shops/${shopId}/listings/active?limit=24&offset=${offset}`,{headers:{"x-api-key":etsyApiCredential()},signal:AbortSignal.timeout(25000)});
    ...
    const payload={listings,asOf:now,total,nextOffset:returned>0&&next<=12000&&(total===null?returned===24:next<total)?next:null};

...
export async function GET() {
  ...
  /* `renders` counted AI-rendered mockups. That feature is gone, so the query
     and the field it fed are gone with it — see app/plan-limits.ts. */
  const [planRow, drafts, sets, billing, publishedToday, publishing, callAverage] = await Promise.all([

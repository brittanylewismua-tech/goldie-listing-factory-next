...
import { printifyCameraMockups } from "@/app/printify-camera-mockups";
...
export async function PATCH(request:Request){
  ...
  const draft=JSON.parse(owned.response_json) as {shopId:number;batchId?:string;blueprintId?:number;providerId?:number;primaryArtworkImageIds?:Record<string,string>;artworkOverrides?:Record<string,{name:string;position:string}>},connection=await env.DB.prepare("SELECT encrypted_token FROM printify_connections WHERE user_id=?").bind(user.userId).first<{encrypted_token:string}>(),secret=(env as unknown as {PRINTIFY_TOKEN_KEY?:string}).PRINTIFY_TOKEN_KEY;
  ...
  const cameraDetails=body.refreshImages&&currentProduct?await printifyCameraMockups({productId,blueprintId,providerId,variants:currentProduct.variants||[]}):[];

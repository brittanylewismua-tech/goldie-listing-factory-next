...
export async function POST(request: Request) {
  ...
  try {
    ...
    const groupedColors=groupProductColors(colorOption?.values||[],availableColorIds,templateColorIds).map(color=>({...color,variantIds:selectableVariants.filter(variant=>(variant.options||[]).some(id=>color.ids.includes(id))).map(variant=>variant.id)}));

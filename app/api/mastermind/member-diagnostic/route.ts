 ...
 const results=[];for(const row of rows.filter(row=>row.status==='needs_attention').slice(0,12)){
  ...
  try{
   const [productCheck,inventory]=await Promise.all([
    ...
    etsyFetch<{products?:{sku:string;offerings:{price:{amount:number;divisor:number};is_enabled:boolean}[];property_values?:{property_id:number;property_name:string;scale_id?:number|null;value_ids?:number[];values?:string[]}[]}[];price_on_property?:number[];quantity_on_property?:number[];sku_on_property?:number[]}>(`/listings/${summary.listingId}/inventory`,etsyToken),
   ]);

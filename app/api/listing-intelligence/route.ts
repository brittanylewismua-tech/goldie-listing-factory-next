...
Avoid duplicate meaning. Do not rewrite, combine, expand, correct, or invent any phrase. Copy each phrase exactly as it appears in the bank. Return only {"design_text":["exact visible line from the design"],"selected_keywords":["exact title phrase copied from the bank"],"tag_keywords":["exact tag phrase copied from the supplied tag candidates"]}.${correction}`})});
    ...
    if(selected.length<minimumTitlePhrases||tags.length<requiredTagCount)return NextResponse.json({error:`Goldie could not build a complete listing for this design after two attempts. It found ${selected.length} of ${minimumTitlePhrases} required title phrases and ${tags.length} of ${requiredTagCount} available Etsy tags. Choose a better-matched keyword bank or build this title yourself.`},{status:422});
    ...
    const chosen=selected,joiner=body.useCommas?", ":" ";let title="";const included:string[]=[];for(const phrase of chosen){const candidate=title?`${title}${joiner}${phrase}`:phrase;if(candidate.length>140)continue;title=candidate;included.push(phrase)}if(!title)return NextResponse.json({error:"Goldie could not build a usable title."},{status:502});const titleWarning=bankMatchesDesignText(titleCandidates,designText)?"":"No phrase in this bank matches this design. Add one, or write the title yourself.";return NextResponse.json({title,keywords:included,tags,titleWarning,designText});
  }

...
Avoid duplicate meaning. Do not rewrite, combine, expand, correct, or invent any phrase. Copy each phrase exactly as it appears in the bank. Return only {"selected_keywords":["exact title phrase copied from the bank"],"tag_keywords":["exact tag phrase copied from the supplied tag candidates"]}.`})});
    const titlePayload=await titleResponse.json() as {output?:string;detail?:string};if(!titleResponse.ok)return NextResponse.json({error:titlePayload.detail||"Goldie could not build this title."},{status:502});const match=titlePayload.output?.match(/\{[\s\S]*\}/);if(!match)return NextResponse.json({error:"Goldie could not read the prepared title."},{status:502});const parsed=JSON.parse(match[0]) as {selected_keywords?:string[];tag_keywords?:string[]},allowedByLower=new Map(keywords.map(keyword=>[keyword.toLocaleLowerCase(),keyword])),selected=[...new Set((parsed.selected_keywords||[]).map(value=>allowedByLower.get(clean(value).toLocaleLowerCase())).filter((value):value is string=>Boolean(value)&&!namesExcludedProduct(value,excludedNouns)))].slice(0,13),tagAllowedByLower=new Map(tagCandidates.map(keyword=>[keyword.toLocaleLowerCase(),keyword])),rankedTags=[...new Set((parsed.tag_keywords||[]).map(value=>tagAllowedByLower.get(clean(value).toLocaleLowerCase())).filter((value):value is string=>Boolean(value)))],tags=[...rankedTags,...tagCandidates.filter(candidate=>!rankedTags.includes(candidate))].slice(0,13);

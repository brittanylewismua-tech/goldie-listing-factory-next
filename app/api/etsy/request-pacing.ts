...
export async function paceEtsyRequest(io:{now():number;read():Promise<{pausedUntil:number;qps:number}>;reserve(now:number,interval:number):Promise<number|null>;wait(ms:number):Promise<void>}){
 let state=await io.read();if(state.pausedUntil>io.now())throw new EtsyRateLimited('Etsy asked The Listing Factory to slow down. Your saved work will continue automatically.');
 const interval=etsyRequestInterval(state.qps),slot=await io.reserve(io.now(),interval);
 if(slot===null)throw new EtsyRateLimited('Your batch is queued behind other saved work. The Listing Factory will continue automatically.');
 const delay=Math.max(0,slot-interval-io.now());
 if(delay>30000)throw new EtsyRateLimited('Your batch is queued behind other saved work. The Listing Factory will continue automatically.');
 if(delay)await io.wait(delay);
 // Another worker may have received a cooldown while this request waited for its slot.
 state=await io.read();if(state.pausedUntil>io.now())throw new EtsyRateLimited('Etsy asked The Listing Factory to slow down. Your saved work will continue automatically.');
}

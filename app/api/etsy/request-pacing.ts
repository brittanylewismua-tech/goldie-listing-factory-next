...
export const RESERVE_ETSY_SLOT_SQL=`UPDATE etsy_request_pacing SET next_at_ms=MAX(next_at_ms,?)+?,updated_at=? WHERE id=1 RETURNING next_at_ms`;
...
export async function paceEtsyRequest(io:{now():number;read():Promise<{pausedUntil:number;qps:number}>;reserve(now:number,interval:number):Promise<number>;wait(ms:number):Promise<void>}){
 ...
 const interval=etsyRequestInterval(state.qps),slot=await io.reserve(io.now(),interval),delay=Math.max(0,slot-interval-io.now());

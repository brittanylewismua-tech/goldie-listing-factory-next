...
export async function POST(request:Request){
  ...
  try{
    const origin=new URL(request.url).origin,redirectUri=`${origin}/api/etsy/callback`,state=base64url(crypto.getRandomValues(new Uint8Array(24))),verifier=base64url(crypto.getRandomValues(new Uint8Array(48))),digest=new Uint8Array(await crypto.subtle.digest("SHA-256",new TextEncoder().encode(verifier))),challenge=base64url(digest);

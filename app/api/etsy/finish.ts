...
type DraftData={id:string;batchId?:string;etsyDetails?:EtsyDetails};
...
async function applyEtsyDetails(token:string,shopId:number,listingId:number,details:EtsyDetails){
  ...
  const listingBody=new URLSearchParams({taxonomy_id:String(match.node.id)});
...
export async function finishEtsyListing(userId:string,draft:DraftData,listingId:number,printifyImageIndices:number[]){
  ...
  await applyEtsyDetails(connection.token,connection.shopId,listingId,draft.etsyDetails);

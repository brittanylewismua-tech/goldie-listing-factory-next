...
export async function GET(){
  ...
  try{
    ...
    const payload=await etsyFetch<{results?:Profile[]}>(`/shops/${connection.shopId}/shipping-profiles`,connection.token,"shipping");
...
export async function POST(request:Request){
  ...
  try{
    ...
    const connection=await etsyConnection(user.userId),base=await etsyFetch<Profile>(`/shops/${connection.shopId}/shipping-profiles/${Number(baseProfileId)}`,connection.token,"shipping"),destinations=base.shipping_profile_destinations||[],domestic=destinations.find(item=>item.destination_country_iso===base.origin_country_iso),submitted=new Map((international||[]).map(rate=>[String(rate.key),rate]));
    if(!domestic)throw new Error("The selected Etsy profile does not contain a domestic destination The Listing Factory can safely copy.");

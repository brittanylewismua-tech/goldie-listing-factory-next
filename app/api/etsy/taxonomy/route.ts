...
type Requested={category?:string;attributes?:Record<string,string>;optional?:Record<string,string>;taxonomyId?:number;product?:{blueprintTitle?:string;brand?:string;model?:string}};
...
function productCategoryScore(product:Requested["product"],path:string){
  const facts=`${product?.blueprintTitle||""} ${product?.brand||""} ${product?.model||""}`.toLowerCase(),candidate=path.toLowerCase();
  if(/poster|art print|canvas/.test(facts))return candidate.includes("art & collectibles › prints ›")?500:candidate.includes("wall")?80:-250;
  if(/t-?shirt|tee|garment-dyed/.test(facts))return candidate.includes("clothing")&&/(t-shirts|tees)/.test(candidate)?500:candidate.includes("clothing")?80:-180;
  if(/sweatshirt|hoodie/.test(facts))return candidate.includes("clothing")&&/(sweatshirt|hoodie)/.test(candidate)?500:candidate.includes("clothing")?80:-180;
  if(/mug|tumbler|wine glass/.test(facts))return /(mug|drinkware|barware)/.test(candidate)?500:candidate.includes("kitchen & dining")?80:-180;
  if(/tote|bag/.test(facts))return candidate.includes("bags & purses")&&candidate.includes("tote")?500:candidate.includes("bags & purses")?80:-180;
  return 0;
}
...
export async function POST(request:Request){
  ...
  try{
    const body=await request.json() as Requested,connection=await etsyConnection(user.userId),tree=await etsyFetch<{results?:TaxonomyNode[]}>("/seller-taxonomy/nodes",connection.token),categories=flatten(tree.results||[]).filter(node=>node.leaf),selected=body.taxonomyId?categories.find(node=>node.id===body.taxonomyId):categories.map(node=>({...node,score:productCategoryScore(body.product,node.path)+score(body.category||"",node.name)+score(body.category||"",node.path)})).sort((a,b)=>b.score-a.score)[0];

...
export function isOwner(user: ChatGPTUser) {
  const ownerEmails = ["beawolfbiz@gmail.com", "brittany@beawolfbiz.com"];
  const normalizedName = (user.fullName ?? user.displayName).trim().toLowerCase();
  return ownerEmails.includes(user.email.trim().toLowerCase()) || normalizedName === "brittany lewis";
}

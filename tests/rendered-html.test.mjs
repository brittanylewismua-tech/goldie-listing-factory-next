...
test("matches Printify editor DPI instead of comparing against template pixel dimensions", async () => {
  ...
  assert.match(page, /Estimated \$\{quality.dpi\} DPI · primary design/);
});
...
test("placement cards contain only the preview, identity, DPI and editor link — D680", async () => {
  ...
  assert.match(placement, /Estimated \$\{quality.dpi\} DPI · primary design/);
...
test("the number on the button is the number that publishes — D561", async () => {
  ...
  assert.equal((app.match(/bundlePublishDrafts\(\)/g) || []).length, 17,

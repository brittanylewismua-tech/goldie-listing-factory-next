...
test('single listing shows one expanded product preview, not a duplicate thumbnail',()=>{
  assert.match(review,/!\(handoffOnly&&group.length===1\)&&\(previews.length\?<ReviewPreviewImage/);
  assert.match(review,/productPreviews.length&&group.length===1\?<div className="final-product-preview"><ReviewPreviewImage/);
});

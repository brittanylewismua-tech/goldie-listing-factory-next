...
test("step 1 names the current action in both states — D322/D946", async () => {
  ...
  assert.match(app,/title: "Start new listings"/);
...
test("the rail digits use the app's own type — D352", async () => {
  ...
  /* D1795 · The app renders Manrope. Inter was declared here and in the
     rail while everything else was Manrope, so two typefaces were on screen
     at once - measured on the live Design Scanner, 29 elements against 21. */
  assert.match(block, /font-family:'Manrope Variable'/, "not a system fallback");
...
test("D379: opening a product card does not reload the page", async () => {
  ...
  assert.match(app, /await persistBatchNow\(batchIdRef\.current\);[\s\S]{0,500}await restoreBatchById\(existing/,

...
test('switching products never exposes the previous product editor under the new task',()=>{
  assert.match(app,/const rowOpen=Boolean\(!switchingProduct&&open&&row\.task&&\(reviewTasks\?reviewTasks\.has\(row\.task\):grouped\?row\.task===effectiveTask:activeTask===row\.task\)\)/);

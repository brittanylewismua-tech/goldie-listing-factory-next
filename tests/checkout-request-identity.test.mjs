...
import {PLANS} from '../app/plan-limits.ts';
...
test('new Pro checkout does not reuse the old Scale label, while old subscribers keep Pro mapping',()=>{
  ...
  assert.equal(prices.priceForPlan('pro'),null);assert.equal(prices.planForPrice('legacy-scale-59'),'pro');assert.equal(prices.priceForPlan('goldie'),'starter-29');
  assert.equal(makePrices(()=>({STRIPE_PRO_PRICE_ID:'dedicated-pro'})).priceForPlan('pro'),'dedicated-pro');
});

test('actual Pro checkout sends the advertised name, price and subscription entitlement',async()=>{
  ...
  assert.equal(result.status,200);const params=h.requests[0].body;assert.equal(params.get('line_items[0][price_data][unit_amount]'),'5900');assert.equal(params.get('line_items[0][price_data][product_data][name]'),'The Goldie Listing Factory — Pro');assert.equal(params.get('subscription_data[metadata][plan_key]'),'pro');
});

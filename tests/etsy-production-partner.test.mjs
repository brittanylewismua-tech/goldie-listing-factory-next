...
test('Printify production partner resolution is exact, case-insensitive, cached and fail-closed',async()=>{
 assert.equal(module.printifyPartnerId({results:[{production_partner_id:77,partner_name:' Printify '}]}),77);
 assert.throws(()=>module.printifyPartnerId({results:[{production_partner_id:2,partner_name:'My print shop'}]}),/Add Printify as a production partner/);

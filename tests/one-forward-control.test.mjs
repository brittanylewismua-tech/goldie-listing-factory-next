...
test("connect remains scoped and Product exposes only its gated continuation", () => {
  ...
  assert.match(setupPanel,/files\.length===0&&!bundleCreationMode&&!productFormMode&&<FactoryFooter status=\{`\$\{missingRequirement\} to continue`\}><button className="workflow-next" type="button" disabled>\{missingRequirement\}<\/button>/);
});

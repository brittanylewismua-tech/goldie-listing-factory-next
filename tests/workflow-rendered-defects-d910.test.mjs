...
test("D911: Step 1 keeps a visible disabled continuation before upload",()=>{
  assert.match(app,/workflowStep==="setup"&&files\.length===0&&!bundleCreationMode&&!productFormMode&&<FactoryFooter status="Add at least one design to continue"><button className="workflow-next" type="button" disabled>Add at least one design<\/button>/);
});

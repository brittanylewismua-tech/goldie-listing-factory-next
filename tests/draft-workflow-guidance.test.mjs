...
const {unfinishedDraftTask,draftTaskSummary}=await import('data:text/javascript;base64,'+Buffer.from(ts.transpile(read('app/draft-workflow-guidance.ts'),{module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022})).toString('base64'));
...
test('compact product summary distinguishes reading, unfinished, and ready work',()=>{
 ...
 assert.equal(draftTaskSummary([task('photos',true),task('extra',false,{optional:true})]),'Sections ready to review');

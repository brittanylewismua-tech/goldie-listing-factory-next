...
test("D1313: the Etsy draft action names its required status check",()=>{
  ...
  assert.match(button,/aria-busy=\{creatingEtsyDrafts\|\|!photoDeliveryStatusReady\}/);
  assert.match(button,/!photoDeliveryStatusReady\?"Checking saved Etsy drafts…":"Save to Etsy Drafts"/);
  assert.match(button,/disabled=\{creatingEtsyDrafts\|\|!photoDeliveryStatusReady\|\|Boolean\(handoffBlockers\(\)\.length\)\}/);
});

...
test("D672 — every open task row closes from the whole row", () => {
  assert.match(page, /const rowOpen=Boolean\(!switchingProduct&&open&&row\.task&&\(reviewTasks\?reviewTasks\.has\(row\.task\):grouped\?row\.task===effectiveTask:activeTask===row\.task\)\)/);
  ...
  assert.match(page, /setActiveTask\(current=>current===task\?"__closed":task\)/);
});

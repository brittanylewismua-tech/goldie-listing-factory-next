...
D1138 deployedc3928e031f80d500d545d22cff7395a9b078ff16, version response matched. Reloaded exclusion test: heading correctly “Create 2 private drafts?”, details “2 uploaded · 2 drafts after exclusions.” Submitted; usage133→135, included active design count1. Found sibling summary still0drafts while active1 and totalheader1, because sibling loader only reran on product/ID changes, not server completion. D1139 adds a dedicated completion revision after final member saves (not general autosave polling). Processing snapshot also exposed the separate upload-review controls; D1139 makes that wrapper inert too. Browser denied subsequent product-switch action because its admin-policy verification was unavailable. No bypass attempted. These last fixes require post-deployment live confirmation; full matrix not signed off.
...
Source follow-through found that excluded files remained in the current member after admission, letting autosave reintroduce files without jobs. Child design IDs also changed without remapping resolution approvals. D1137 derives included member designs and matching approval keys together, persists them together, and updates the active files only after whole-submission acceptance. Three executable regressions cover product-specific exclusions, cloned and stable IDs, stale keys, and input immutability; integration assertions cover acceptance ordering. Live exclusion and in-flight mutation-lock checks remain required. This is not a whole-matrix sign-off.

D1135 deployed `f1dfd8990da53d5dce019e7102a96189a4c35b0b` / Cloudflare `845ca49e-a75a-44fa-8a32-f7ca7f16a67c`; public version matched. Reloaded Chrome production; switched tee→hoodie→tee and remained on Drafts, with no repeated preflight. Save dialog visually verified and actual save succeeded. Fresh single phone case: batch `dd72ff39-c75b-42fd-8eda-c62b8b85b4a8`, draft `6a9c83b2e5fa61628009a6a2`, real pink Books design. Submission click21:03:40.256Z; background acceptance observed at3.552s; complete observed by19.940s (observation bound, not exact completion latency). Usage132→133. Correct actual phone-case mockup appeared, no apparel colors panel. Edited price29.49 and displayed item profit14.99, then saved. Initial auto-recalculation briefly rendered the previous input value beside updated profit; D1136 synchronizes that input before paint. Also found Add/Remove/Change controls still exposed during submission; D1136 makes upload/product sections inert during processing and guards mutation handlers. Photo catalogue comparison and complete single-product finishing flow still in progress; not a full launch sign-off.

...
import {markDeliveryInterrupted} from '../app/api/listing-photos/delivery/interrupted';
import {WorkflowEntrypoint,type WorkflowEvent,type WorkflowStep} from 'cloudflare:workers';
import {runDeliveryTick,deliveryStatus,deliveryEnv,type DeliveryEnv} from '../app/api/listing-photos/delivery/service';
...
export class PhotoDeliveryWorkflow extends WorkflowEntrypoint<DeliveryEnv,{id:string;owner:string}>{
 async run(event:WorkflowEvent<{id:string;owner:string}>,step:WorkflowStep){
  ...
  await step.do('end-bounded-delivery',()=>deliveryStatus(id,owner,'expired','Automatic checking ended. Prepare photo delivery again when ready.'));
   ...
   await step.do('record-interrupted-delivery',{retries:{limit:2,delay:'10 seconds',backoff:'constant'},timeout:'30 seconds'},()=>markDeliveryInterrupted(deliveryEnv().DB,id,owner,logError));
  }
 }
}
